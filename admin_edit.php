...
if ($admin_id!="") {
	if(!($admin_db = mysqli_query($link,"select * from users where id = '$admin_id'"))){
		printf("%s", sprintf("internal error %d:%s\n", mysqli_errno($link), mysqli_error($link)));
		exit();
	}
	$administrator = mysqli_fetch_array($admin_db);
}
...
if($_POST['SubmitAdmin']){
	...
	if($_POST['pass']!="") {
		...
		$passencrypt = $administrator['pass'];
	}
...
if($admin_id!="") {
	if(!($admin_update = mysqli_query($link,"UPDATE users SET email='$email',pass='$passencrypt',firstname='$firstname',lastname='$lastname',site='$site',address='$address',city='$city',state='$state',zip='$zip',office_phone='$officephone',cell_phone='$cellphone',level='$level',active='$active' WHERE id = '$admin_id'"))){
		printf("%s", sprintf("internal error %d:%s\n", mysql_errno($link), mysql_error($link)));
		exit();
	}
} else {
	if(!($admin_update = mysqli_query($link,"INSERT INTO users SET email='$email',pass='$passencrypt',firstname='$firstname',lastname='$lastname',site='$site',address='$address',city='$city',state='$state',zip='$zip',office_phone='$officephone',cell_phone='$cellphone',level='$level',active='$active'"))){
		printf("%s", sprintf("internal error %d:%s\n", mysql_errno($link), mysql_error($link)));
		exit();
	}	
}	
	...
	if($_FILES['uphoto']['name'] != NULL){ 
		$file_name = $_FILES['uphoto']['name'];
      	...
      	if(in_array($file_ext,$expensions)=== false){
          	header("Location: admin_edit.php?admin=".$photo_id."&flag=notvalid");
		  	exit;
      	}
	...
	header("Location: admin_edit.php?admin=".$photo_id."&flag=".$flag."");
	exit();
}
?>
...
<html>
	<head>
		...
		<title>Manage Admin | Look2 Home Marketing</title>
	...
	<div id="container">
		...
		<div id="right_column">
			...
			<div id="rc_content_container">
				<form action="admin_edit.php" method="post" name="adminedit" id="adminedit" enctype="multipart/form-data">
					<div class="section_container">
						<div class="section_header">
							<svg xmlns="http://www.w3.org/2000/svg" class="section_icon" viewBox="0 0 36 36" preserveAspectRatio="xMidYMid meet"><circle cx="14.67" cy="8.3" r="6"/><path d="M16.44 31.82a2.15 2.15 0 0 1-.38-2.55l.53-1-1.09-.33a2.14 2.14 0 0 1-1.5-2.1v-2.05a2.16 2.16 0 0 1 1.53-2.07l1.09-.33-.52-1a2.17 2.17 0 0 1 .35-2.52 18.92 18.92 0 0 0-2.32-.16A15.58 15.58 0 0 0 2 23.07v7.75a1 1 0 0 0 1 1h13.44zm17.26-8.36l-2-.6a6.73 6.73 0 0 0-.58-1.42l1-1.86a.35.35 0 0 0-.07-.43l-1.45-1.46a.38.38 0 0 0-.43-.07l-1.85 1a7.74 7.74 0 0 0-1.43-.6l-.61-2a.38.38 0 0 0-.36-.25h-2.08a.38.38 0 0 0-.35.26l-.6 2a6.85 6.85 0 0 0-1.45.61l-1.81-1a.38.38 0 0 0-.44.06l-1.47 1.44a.37.37 0 0 0-.07.44l1 1.82a7.24 7.24 0 0 0-.65 1.43l-2 .61a.36.36 0 0 0-.26.35v2.05a.36.36 0 0 0 .26.35l2 .61a7.29 7.29 0 0 0 .6 1.41l-1 1.9a.37.37 0 0 0 .07.44L19.16 32a.38.38 0 0 0 .44.06l1.87-1a7.09 7.09 0 0 0 1.4.57l.6 2.05a.38.38 0 0 0 .36.26h2.05a.38.38 0 0 0 .35-.26l.6-2.05a6.68 6.68 0 0 0 1.38-.57l1.89 1a.38.38 0 0 0 .44-.06L32 30.55a.38.38 0 0 0 .06-.44l-1-1.88a6.92 6.92 0 0 0 .57-1.38l2-.61a.39.39 0 0 0 .27-.35v-2.07a.4.4 0 0 0-.2-.36zm-8.83 4.72a3.34 3.34 0 1 1 3.33-3.34 3.34 3.34 0 0 1-3.33 3.34z"/><path fill-opacity="0" d="M0 0h36v36H0z"/></svg><h1>Manage Admin</h1>
						</div>
						...
						<div class="section_result_row">
							...
							<div class="section_form_36">
								<input name="fname" type="text" id="fname" value="<?php echo $administrator['firstname']; ?>" size="32" />
							</div>
							<div class="section_form_14">
								Address:
							</div>
							<div class="section_form_36">
								<input name="address" type="text" id="address" value="<?php echo $administrator['address']; ?>" size="32" />
							</div>
							<div class="clear"></div>
							<div class="section_form_14">
								Last Name:
							...
							<div class="section_form_36">
								<input name="lname" type="text" id="lname" value="<?php echo $administrator['lastname']; ?>" size="32" />
							</div>
							<div class="section_form_14">
								City:
							</div>
							<div class="section_form_36">
								<input name="city" type="text" id="city" value="<?php echo $administrator['city']; ?>" size="32" />
							</div>
							<div class="clear"></div>
							<div class="section_form_14">
								&nbsp;
							...
							<div class="section_form_14">
								<input name="state" type="text" id="state" value="<?php echo $administrator['state']; ?>" size="4" maxlength="2" />
							</div>
							<div class="section_form_10">
								Zip:
							</div>
							<div class="section_form_14">
								<input name="zip" type="text" id="zip" value="<?php echo $administrator['zip']; ?>" size="10" maxlength="5" />
							</div>
							<div class="clear"></div>
							<div class="section_form_14">
								Email Address:
							...
							<div class="section_form_36">
								<input name="email" type="text" id="email" value="<?php echo $administrator['email']; ?>" size="32" />
							</div>
							<div class="clear"></div>
							<div class="section_form_14">
								Cell Phone:
							...
							<div class="section_form_36">
								<input name="cell" type="text" id="cell" value="<?php echo $administrator['cell_phone']; ?>" size="24" maxlength="12" /><br>ex. 555-555-5555
							</div>
							<div class="section_form_14">
								Office Phone:
							</div>
							<div class="section_form_36">
								<input name="officephone" type="text" id="officephone" value="<?php echo $administrator['office_phone']; ?>" size="24" maxlength="12" /><br>ex. 555-555-5555
							</div>
							<div class="clear"></div>
							<div class="section_form_14">
								Dealer Photo:
							...
							<div class="section_form_36">
								<span class="agent_pic_order_80">
									<?php if(file_exists("floorplan/profile/".$admin_id.".jpg")){ ?><img src="floorplan/profile/<?php echo $admin_id; ?>.jpg" width="80px" alt="<?php echo $administrator['firstname']." ". $administrator['lastname']; ?>"><?php } ?>
								</span>
							</div>
							<div class="clear"></div>
							<div class="section_form_14">
							...
							<div class="section_form_36">
								<select name="active" id="active">
                    				...
                    				<option value="n" <?php if($administrator['active']=='n') echo "selected" ?>>No</option>
                  				</select>
							</div>
							<div class="clear"></div>
						</div>
						<div class="section_footer">
							<div class="section_navigate">
								<input name="admin" type="hidden" value="<?php echo $admin_id; ?>" /><input name="level" type="hidden" value="admin" />
							</div>
							<div class="section_submit">
								<input class="section_submit_button_md" name="SubmitAdmin" type="submit" id="SubmitAdmin" value="Submit" />
							</div>
							<div class ="clear"></div>
						</div>
					</div>
