...
if($_GET['st']!="") { 
	$staff_st = $_GET['st'];	
	$letter_menu = '<a href="staff.php?st='.$staff_st.'">All</a>';
	if(!($letter_num = mysqli_query($link,"SELECT * FROM users WHERE lastname REGEXP '^[0-9]' AND state = '$staff_st' AND level = 'staff'"))){
	...
	if($num_count!="0") { 
		$letter_menu .= '<a href="staff.php?st='.$staff_st.'&letter=num">#</a>'; 
	} else { 
...
foreach($letter_sort as $letter){
	if($_GET['st']!="") { 
		if(!($letter_result = mysqli_query($link,"Select * FROM users WHERE lastname LIKE '".$letter."%' AND state = '$staff_st' AND level = 'staff'"))){
			printf("%s", sprintf("internal error %d:%s\n", mysqli_errno($link), mysqli_error($link)));
			exit();
		}
		...
		if($letter_count!="0") { 
			$letter_menu .= '<a href="staff.php?st='.$staff_st.'&letter='.$letter.'">'.$letter.'</a>'; 
		} else { 
...
if ($_GET['task'] == "remstaff"){
	if(!($staff_rem = mysqli_query($link,"DELETE FROM users WHERE id = '$staff_id'"))){
				printf("%s", sprintf("internal error %d:%s\n", mysqli_errno($link), mysqli_error($link)));
				exit();
			}
...
<html>
	<head>
		...
		<title>Staff | Look2 Home Marketing</title>
		...
		<div id="right_column">
			...
			<div id="rc_content_container">
				<div class="section_full_ra">
					<a href="staff_edit.php">Add New Staff</a>	
				</div>
				<div class="section_container">
					...
					<div class="section_header">
						<svg xmlns="http://www.w3.org/2000/svg" class="section_icon" viewBox="0 0 477.655 477.655"><path d="M440.367 440.415l-10.173-29.91c-19.102-56.262-70.83-94.605-129.763-97.121-5.187 4.03-10.655 7.493-16.322 10.521-8.449 22.185-36.836 28.195-53.468 11.205-19.676-1.738-37.69-9.511-53.422-21.725-58.933 2.508-110.647 40.851-129.763 97.121L37.3 440.415c-2.936 8.603-1.522 18.084 3.774 25.469 5.279 7.391 13.821 11.771 22.906 11.771h349.693c9.083 0 17.626-4.379 22.906-11.771 5.294-7.385 6.707-16.866 3.788-25.469zM277.758 290.619c34.212-24.047 58.141-77.151 58.141-128.491 0-145.907-194.133-145.752-194.133 0 0 62.397 35.33 127.303 81.546 139.556 4.456-12.626 16.382-21.757 30.515-21.757 9.504-.001 17.983 4.168 23.931 10.692zM99.169 223.042c4.813 18.906 31.044 13.704 31.044-3.805 0-70.178 3.354-76.731-6.041-84.348 21.507-132.528 206.774-131.534 229.323.015-9.381 7.641-6.025 14.163-6.025 84.333 0 5.489 2.95 10.095 7.189 12.952 0 54.594-22.145 51.402-88.736 69.052-10.295-11.174-28.683-3.899-28.683 11.173 0 18.876 27.053 23.293 32.302 4.318 53.762-14.256 101.018-18.752 101.018-72.484v-11.027c3.991-2.066 6.817-5.729 7.951-10.179 51.822-1.056 51.838-78.719 0-79.775-1.072-4.24-3.711-7.703-7.423-9.815 1.336-15.902-1.94-36.805-11.057-56.985-63.405-130.835-250.676-79.643-253.609 47.155-.404 4.294-.078 7.338.17 9.83-3.712 2.112-6.351 5.575-7.423 9.815-21.71.419-39.212 18.084-39.212 39.888s17.502 39.467 39.212 39.887z"/></svg><h1>Staff Accounts - Select State</h1>
					</div>
					<?php 
						if(!($state_sel = mysqli_query($link,"SELECT distinct(state) FROM users WHERE level = 'staff' ORDER BY state ASC"))){
							printf("%s", sprintf("internal error %d:%s\n", mysqli_errno($link), mysqli_error($link)));
							exit();
							
						}
						while($staff_states=mysqli_fetch_array($state_sel)){ 
					?>
					<div class="section_result_row">
						<a href="staff.php?st=<?php echo $staff_states['state']; ?>">
							<div class="section_full">
								<?php $st = strtoupper($staff_states['state']); echo $state_list[$st]; ?>
							</div>
						</a>
					</div>
					<?php } } else if($_GET['st']!="") { $staff_st = $_GET['st']; ?>
					<div class="section_header">
						<svg xmlns="http://www.w3.org/2000/svg" class="section_icon" viewBox="0 0 477.655 477.655"><path d="M440.367 440.415l-10.173-29.91c-19.102-56.262-70.83-94.605-129.763-97.121-5.187 4.03-10.655 7.493-16.322 10.521-8.449 22.185-36.836 28.195-53.468 11.205-19.676-1.738-37.69-9.511-53.422-21.725-58.933 2.508-110.647 40.851-129.763 97.121L37.3 440.415c-2.936 8.603-1.522 18.084 3.774 25.469 5.279 7.391 13.821 11.771 22.906 11.771h349.693c9.083 0 17.626-4.379 22.906-11.771 5.294-7.385 6.707-16.866 3.788-25.469zM277.758 290.619c34.212-24.047 58.141-77.151 58.141-128.491 0-145.907-194.133-145.752-194.133 0 0 62.397 35.33 127.303 81.546 139.556 4.456-12.626 16.382-21.757 30.515-21.757 9.504-.001 17.983 4.168 23.931 10.692zM99.169 223.042c4.813 18.906 31.044 13.704 31.044-3.805 0-70.178 3.354-76.731-6.041-84.348 21.507-132.528 206.774-131.534 229.323.015-9.381 7.641-6.025 14.163-6.025 84.333 0 5.489 2.95 10.095 7.189 12.952 0 54.594-22.145 51.402-88.736 69.052-10.295-11.174-28.683-3.899-28.683 11.173 0 18.876 27.053 23.293 32.302 4.318 53.762-14.256 101.018-18.752 101.018-72.484v-11.027c3.991-2.066 6.817-5.729 7.951-10.179 51.822-1.056 51.838-78.719 0-79.775-1.072-4.24-3.711-7.703-7.423-9.815 1.336-15.902-1.94-36.805-11.057-56.985-63.405-130.835-250.676-79.643-253.609 47.155-.404 4.294-.078 7.338.17 9.83-3.712 2.112-6.351 5.575-7.423 9.815-21.71.419-39.212 18.084-39.212 39.888s17.502 39.467 39.212 39.887z"/></svg><h1>Staff Accounts - Select Staff</h1>
					</div>
					...
					<?php 
						if($_GET['letter']!="") {
							$staff_letter = $_GET['letter'];
							if($staff_letter!="num"){
								if(!($staff_sel = mysqli_query($link,"SELECT id, firstname, lastname, cell_phone, email FROM users WHERE lastname LIKE '".$staff_letter."%' AND state = '$staff_st' AND level = 'staff' ORDER BY lastname ASC"))){
									printf("%s", sprintf("internal error %d:%s\n", mysqli_errno($link), mysqli_error($link)));
									exit();
								} 
							} else {
								if(!($staff_sel = mysqli_query($link,"SELECT id, firstname, lastname, cell_phone, email FROM users WHERE lastname REGEXP '^[0-9]' AND state = '$staff_st' AND level = 'staff' ORDER BY lastname ASC"))){
									printf("%s", sprintf("internal error %d:%s\n", mysqli_errno($link), mysqli_error($link)));
									exit();
								} 	
							}
						} else {
							if(!($staff_sel = mysqli_query($link,"SELECT id, firstname, lastname, cell_phone, email FROM users WHERE level = 'staff' AND state = '$staff_st' ORDER BY lastname ASC"))){
								printf("%s", sprintf("internal error %d:%s\n", mysqli_errno($link), mysqli_error($link)));
								exit();
							}		
						}
						$num_staff = mysqli_num_rows($staff_sel);														
						while($select_staff=mysqli_fetch_array($staff_sel)) { 
					?>
					<div class="section_result_row">
						<a href="staff_edit.php?staff=<?php echo $select_staff['id']; ?>">
							<div class="section_sel_company">
								<?php echo $select_staff['lastname'].", ".$select_staff['firstname']; ?>
							</div>
							<div class="section_phone">
								<?php echo $select_staff['cell_phone']; ?>
							</div>							
						</a>
						<div class="section_phone">
							&nbsp;
						...
						<div class="section_edit">
							<a class="remove_rec" href="staff.php?staff=<?php echo $select_staff['id']; ?>&task=remstaff" onClick="confirmCleanUp('staff - <?php echo $select_staff["lastname"].", ".$select_staff["firstname"]; ?>'); return returnvalue"><svg xmlns="http://www.w3.org/2000/svg" class="section_icon" viewBox="0 0 800 800"><path d="M800 400c0 220.9-179.1 400-400 400S0 620.9 0 400 179.1 0 400 0s400 179.1 400 400zm-560-40c-22.1 0-40 17.9-40 40s17.9 40 40 40h320c22.1 0 40-17.9 40-40s-17.9-40-40-40H240z" fill-rule="evenodd"/></svg>Remove</a>
						</div>
						<div class="clear"></div>
					</div>
					<?php } } if($num_staff=="0") { ?>
