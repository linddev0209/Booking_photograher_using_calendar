...
if ($_GET['task'] == "remsqft"){ 
	$area_id = $_GET['area_id'];
	if(!($sqft_delete = mysqli_query($link,"DELETE FROM area_calc WHERE id='$area_id'"))){
		printf("%s", sprintf("internal error %d:%s\n", mysqli_errno($link), mysqli_error($link)));
		exit();
	}
...
if($_POST['Submit']){
	...
	$a_sqft = $_POST['sqft'];
	$a_sort = $_POST['sort'];
	$order_a = $_POST['order_id'];
	if(!($area_update = mysqli_query($link,"INSERT INTO area_calc SET order_id='$order_a',area='$area',sqft='$a_sqft',sort='$a_sort'"))){
		printf("%s", sprintf("internal error %d:%s\n", mysqli_errno($link), mysqli_error($link)));
		exit();
	}
	header("Location: complete.php?order_id=".$order_a);
	exit();
}

if($_POST['UpdateSQFT']){
	$sqft_id = $_POST['order_id'];
	if(!($squp_sort = mysqli_query($link,"SELECT * FROM area_calc where order_id = '$sqft_id' ORDER BY sort ASC"))){
		printf("%s", sprintf("internal error %d:%s\n", mysqli_errno($link), mysqli_error($link)));
		exit();
	}
	while($sortsqft=mysqli_fetch_array($squp_sort)){
		$sqid = $sortsqft['id'];
		$sort = $_POST['sort'.$sqid];
		if($sort!=""){
			if(!($rec_sqftsort = mysqli_query($link,"UPDATE area_calc set sort = '$sort' WHERE id = '$sqid'"))){
				printf("%s", sprintf("internal error %d:%s\n", mysqli_errno($link), mysqli_error($link)));
				exit();
			}
		}
	}
	header("Location: complete.php?order_id=".$sqft_id);
	exit();
}

if($_POST['SubmitPD']){
	$order_id = $_POST['order_id'];
	$disp_ext = $_POST['exterior'];
	$disp_int = $_POST['interior']; 
	$beds = $_POST['beds']; 
	$baths = $_POST['baths'];
	$h_baths = $_POST['h_baths'];
	$offices = $_POST['offices'];
	$fpby = $_POST['drawn'];
	$fpdone = $_POST['fp_done'];
	$photodone = $_POST['photos_done'];
	$efpdone = $_POST['efp_done'];
	$tpdone = $_POST['tp_done'];
	$fixdone = $_POST['fix_done'];
	$rush_id = $_POST['rush_id'];

	if(!($pd_db = mysqli_query($link,"SELECT order_id FROM order_data WHERE order_id = '$order_id'"))){
		printf("%s", sprintf("internal error %d:%s\n", mysqli_errno($link), mysqli_error($link)));
		exit();
	}
	if(mysqli_num_rows($pd_db)!=""){
		if(!($pd_update = mysqli_query($link,"UPDATE order_data SET beds='$beds', offices='$offices', baths='$baths', h_bath='$h_baths', internal='$disp_int', external='$disp_ext', drawn_by='$fpby', fp_done='$fpdone', efp_done='$efpdone', photos_done='$photodone', tp_done='$tpdone', fix_done='$fixdone', rush_done='$rush_id' WHERE order_id = '$order_id'"))){
			printf("%s", sprintf("internal error %d:%s\n", mysqli_errno($link), mysqli_error($link)));
			exit();
		}
	}else{
		if(!($dba = mysqli_query($link,"INSERT INTO order_data SET order_id = '$order_id', beds='$beds', offices='$offices', baths='$baths', h_bath='$h_baths', internal='$disp_int', external='$disp_ext', drawn_by='$fpby', fp_done='$fpdone', efp_done='$efpdone', photos_done='$photodone', tp_done='$tpdone', fix_done='$fixdone', rush_done='$rush_id'"))){
			printf("%s", sprintf("internal error %d:%s\n", mysqli_errno($link), mysqli_error($link)));
		exit();
		}
	}
...
if($_POST['SubmitPub']){
	$pub_id = $_POST['order_id'];
	
	if(!($check_comp = mysqli_query($link,"SELECT status FROM orders WHERE status='complete' AND id = '$pub_id'"))){
		printf("%s", sprintf("internal error %d:%s\n", mysqli_errno($link), mysqli_error($link)));
		exit();
	}
	if(mysqli_num_rows($check_comp)==0){
	$process_date = date("Y-m-d H:i:00");
		if(!($comp_up = mysqli_query($link,"UPDATE orders SET status='complete', processed='$process_date', processed_by='$mem_id' WHERE id = '$pub_id'"))){
		printf("%s", sprintf("internal error %d:%s\n", mysqli_errno($link), mysqli_error($link)));
				exit();
		}
	}
	
	if(!($sel_oac = mysqli_query($link,"SELECT orders.*, users.firstname AS agent_firstname, users.lastname  AS agent_lastname, users.cell_phone AS agent_cell, users.email AS agent_email, users.assist_name  AS assist_name, users.assist_email  AS assist_email, users.assist_notify  AS assist_notify, companies.site_email AS site_email, companies.admin_email AS admin_email, companies.site_notify AS site_notify, companies.admin_notify AS admin_notify, companies.designed FROM orders, users, companies WHERE orders.id = '$pub_id' AND orders.agent_id = users.id AND users.company_id = companies.id"))){
			printf("%s", sprintf("internal error %d:%s\n", mysqli_errno($link), mysqli_error($link)));
			exit();
	}
	$list_aoc = mysqli_fetch_array($sel_oac);
	if($list_aoc['street_num']=="0") { $street_num=""; } else { $street_num=$list_aoc['street_num']; }
	
	$message_body = "Dear ".$list_aoc['agent_firstname']." ".$list_aoc['agent_lastname'].", Your order for ".$street_num." ".$list_aoc['street_dir']." ".$list_aoc['street_name']." ".$list_aoc['unit']." has been completed.";
	
	$message_body.= "
	Property Information:
	MLS#:".$list_aoc['mls']."
	".$street_num." ".$list_aoc['street_dir']." ".$list_aoc['street_name']." ".$list_aoc['unit']."
	".$list_aoc['city'].", ".$list_aoc['state']." ".$list_aoc['zip']."
 
 	Ordered:
	";
	if($list_aoc['fp']=="y") $message_body.= "Interactive Floor Plan, "; 
	if($list_aoc['draw_only']=="y") $message_body.= "Drawing Only Floor Plan, ";
	if($list_aoc['hdr_only']=="y") $message_body.= "TruView Photos, ";
	if($list_aoc['hdr_photos']=="y") $message_body.= "TruView Photos, ";
	if($list_aoc['twilight']=="y") $message_body.= "Twilight Photos, ";
	if($list_aoc['aerial_pics']=="y") $message_body.= "Aerial Photos, ";
	if($list_aoc['photos']=="y") $message_body.= "Standard Photo Gallery Only, ";
	if($list_aoc['vid']=="y") $message_body.= "Video, ";
	if($list_aoc['cine_vid']=="y") $message_body.= "Cinematic Video, ";
	if($list_aoc['avid']=="y") $message_body.= "Aerial Video, ";
	if($list_aoc['areacalc']=="y") $message_body.= "Don't Display Area Calculations "; 
	if($list_aoc['rush']=="y") $message_body.= "Rush Order "; 
	if($list_aoc['sr18']=="y") $message_body.= "Blank Sign Rider 18in"; 
    if($list_aoc['sr24']=="y") $message_body.= "Blank Sign Rider 24in";
   	if($list_aoc['sr30']=="y") $message_body.= "Blank Sign Rider 30in";
    if($list_aoc['sticker']=="y") $message_body.= "QR Code Stickers (Qty 2 - 1 for each side of sign)";
	if($list_aoc['realtor_com']=="y") $message_body.= "Add to Realtor.com";
	if($list_aoc['addy_url']=="y") $message_body.= "Add Address URL";
	if($list_aoc['disp_price']=="y") $message_body.= "Don't Show Price Online"; 
	
	if($list_aoc['vid']=="y") $message_body.= "\n"."Video Narration: ".$list_aoc['vid_nar']."\n";
		
	$message_body.= '
		
	Branded Version: 
	<a href="https://tours.look2homes.com/index/'.$pub_id.'">https://tours.look2homes.com/index/'.$pub_id.'</a>

	';
	
	$message_body.='Unbranded Version (required by some MLS boards):
	<a href="https://tours.look2homes.com/unbranded/index/'.$pub_id.'">https://tours.look2homes.com/unbranded/index/'.$pub_id.'</a>
	...
	Look2 Home Marketing
	http://www.look2homes.com

	";

	if($list_aoc['fp']=="y") { $ord_fp= "Interactive Floor Plan w/ TruView Photos, "; } else { $ord_fp= ""; }
	if($list_aoc['draw_only']=="y") { $ord_draw= "Drawing Only Floor Plan, "; } else { $ord_draw= ""; }
	if($list_aoc['hdr_photos']=="y") { $ord_hdr= "TruView Photos Upgrade, "; } else { $ord_hdr= ""; }
	if($list_aoc['hdr_only']=="y") { $ord_hdr_o= "TruView Photos Only, "; } else { $ord_hdr_o= ""; }
	if($list_aoc['twilight']=="y") { $ord_tlight= "Twilight Photos, "; } else { $ord_tlight= ""; }
	if($list_aoc['aerial_pics']=="y") { $ord_aphotos= "Aerial Photos, "; } else { $ord_aphotos= ""; }
	if($list_aoc['photos']=="y") { $ord_s_photo= "Standard Photo Gallery Only, "; } else { $ord_s_photo= ""; }
	if($list_aoc['vid']=="y") { $ord_vid= "Video"; } else { $ord_vid= ""; }
	if($list_aoc['cine_vid']=="y") { $ord_cvid= "Cinematic Video"; } else { $ord_cvid= ""; }
	if($list_aoc['avid']=="y") { $ord_avid= "Aerial Video"; } else { $ord_avid= ""; }
	if($list_aoc['areacalc']=="y") { $ord_disp_area= "Do not Display Area Calculations, "; } else { $ord_disp_area= ""; }
	if($list_aoc['rush']=="y") { $ord_rush= "Rush Order, "; } else { $ord_rush= ""; }
	if($list_aoc['sr18']=="y") { $ord_sr18= "Blank Sign Rider 18in, "; } else { $ord_sr18= ""; }
	if($list_aoc['sr24']=="y") { $ord_sr24= "Blank Sign Rider 24in, "; } else { $ord_sr24= ""; }
	if($list_aoc['sr30']=="y") { $ord_sr30= "Blank Sign Rider 30in, "; } else { $ord_sr30= ""; }
	if($list_aoc['sticker']=="y") { $ord_sticker= "QR Code Stickers (Qty 2 - 1 for each side of sign), "; } else { $ord_sticker= ""; }
	if($list_aoc['realtor_com']=="y") { $ord_realcom= "Add to Realtor.com, "; } else { $ord_realcom= ""; }
	if($list_aoc['addy_url']=="y") { $ord_addyurl= "Add Address URL, "; } else { $ord_addyurl= ""; }
	if($list_aoc['disp_price']=="y") { $ord_disp_price= "Do not Show Price Online, "; } else { $ord_disp_price= ""; }
	
	 $html_data = '<html><head><title>Look2 Home Marketing - '.$street_num.' '.$list_aoc['street_dir'].' '.$list_aoc['street_name'].' '.$list_aoc['unit'].'</title></head><body bgcolor="#EEEEEE" style="background: #EEE top center repeat-x; margin: 0px; padding: 0px 12px 6px 12px; FONT-FAMILY: Arial, Helvetica, sans-serif; "><table width="100%" cellspacing="0" cellpadding="0"><tbody><tr><td><table width="700" align="center" cellspacing="0" cellpadding="0"><tr><td height="118" valign="top" cellpadding="0" align="right"><img src="http://www.look2homes.com/l2fp/html_email/images/look2hm_logo-2.jpg" width="275" height="118" alt="Look2 Home Marketing"></td></tr><tr cellpadding="0" cellspacing="0"><td style="margin:6px; padding:10px; FONT-SIZE: 12px; border:#BBBBBB solid 1px;" bgcolor="#FFFFFF" cellpadding="0" cellspacing="0"><br><strong style="FONT-SIZE: 28px; COLOR: #6633CC;">Order Completed</strong><hr width="100%" color="#BBB" size="1">'.$list_aoc['agent_firstname'].' '.$list_aoc['agent_lastname'].',<br>Your order for '.$street_num.' '.$list_aoc['street_dir'].' '.$list_aoc['street_name'].' '.$list_aoc['unit'].' has been completed. <br><br><strong>Property Address:</strong><br>MLS#:'.$list_aoc['mls'].'<br>'.$street_num.' '.$list_aoc['street_dir'].' '.$list_aoc['street_name'].' '.$list_aoc['unit'].'<br>'.$list_aoc['city'].', '.$list_aoc['state'].' '.$list_aoc['zip'].'<br><br><strong>ORDERED:</strong><br>'.$ord_fp.' '.$ord_draw.' '.$ord_hdr.' '.$ord_hdr_o.' '.$ord_tlight.' '.$ord_aphotos.' '.$ord_s_photo.' '.$ord_disp_area.' '.$ord_rush.' '.$ord_sr18.' '.$ord_sr24.' '.$ord_sr30.' '.$ord_sticker.' '.$ord_realcom.' '.$ord_addyurl.' '.$ord_disp_price.' '.$ord_vid.' '.$ord_cvid.' '.$ord_avid.'<br><br>Branded Version: <br><A style="COLOR: #6633CC;" href="https://tours.look2homes.com/index/'.$pub_id.'">https://tours.look2homes.com/index/'.$pub_id.'</A><br><br>Unbranded Version (required by some MLS boards):<br><A style="COLOR: #6633CC;" href="https://tours.look2homes.com/unbranded/index/'.$pub_id.'">https://tours.look2homes.com/unbranded/index/'.$pub_id.'</A><br><br>Download Files: <br><A style="COLOR: #6633CC;" href="https://downloads.look2homes.com/file_download.php?order_id='.$pub_id.'">https://downloads.look2homes.com/file_download.php?order_id='.$pub_id.'</A><br><br>For questions or support please contact us at <strong>(803) 847-0006</strong><br /></td></tr><tr cellpadding="0" cellspacing="0"><td height="16" valign="top" cellpadding="0" cellspacing="0" align="right"><img src="http://www.look2homes.com/l2fp/html_email/images/bot_shade.jpg" width="700" height="16" Border="0" alt="Order"></td></tr><tr><td style="FONT-SIZE: 12px;" >Thank you for your business!<br>Sincerely,<br><br><strong>Look2 Home Marketing</strong><br><br><A style="COLOR: #000" href="http://www.look2homes.com">http://www.look2homes.com</A><br><A style="COLOR: #000" href="mailto: orders@look2homes.com">orders@look2homes.com</A></td></tr></table></td></tr></tbody></table></body></html>';
		
		$mailsubject = "Completed Order - ".$street_num." ".$list_aoc['street_dir']." ".$list_aoc['street_name']." ".$list_aoc['unit'];
		$agent_email = $list_aoc['agent_email'];
		
		SendEmail("orders@look2homes.com", "Look2 Home Marketing", $list_aoc['agent_email'], $list_aoc['agent_firstname']." ".$list_aoc['agent_lastname'], $mailsubject, nl2br($message_body), $html_data);
		if($list_aoc['assist_email']!="" && $list_aoc['assist_notify']=="y") { 
		SendEmail("orders@look2homes.com", "Look2 Home Marketing", $list_aoc['assist_email'], $list_aoc['assist_name'], $mailsubject, nl2br($message_body), $html_data);
		}
		
		SendEmail("orders@look2homes.com", "Look2 Home Marketing", "orders@look2homes.com", "Look2 Orders", $mailsubject, nl2br($message_body), $html_data);	
	
		header("Location: complete.php?order_id=".$pub_id."&flag=complete");
		exit();	
}
?>
...
<html>
	<head>
		...
		<title>Complete Order | Look2 Home Marketing</title>
		...
		<div id="right_column">
			...
			<div id="rc_content_container">
				<div class="section_full_ra">
					<a href="schedule.php?order_id=<?php echo $order_id; ?>">Set Appointment</a>&nbsp;&nbsp;&nbsp;|&nbsp;&nbsp;&nbsp;<a href="photos.php?order_id=<?php echo $order_id; ?>">Upload Photos</a>&nbsp;&nbsp;&nbsp;|&nbsp;&nbsp;&nbsp;<a href="photos_edit.php?order_id=<?php echo $order_id; ?>">Edit Photos</a>&nbsp;&nbsp;&nbsp;|&nbsp;&nbsp;&nbsp;<a href="fplan.php?order_id=<?php echo $order_id ?>">Upload Floor Plan(s)</a>&nbsp;&nbsp;&nbsp;|&nbsp;&nbsp;&nbsp;<a href="complete.php?order_id=<?php echo $order_id; ?>">Complete Order</a>
				</div>
				<div class="section_container">
					<div class="section_header">
						<svg xmlns="http://www.w3.org/2000/svg" shape-rendering="geometricPrecision" image-rendering="optimizeQuality" class="section_icon" viewBox="0 0 512 512" fill-rule="evenodd"><path d="M156.8 112.5h198.3c2.8 0 5.3-2.3 5.3-5.3V49.3c0-2.8-2.3-5.3-5.3-5.3l-52.3-.5C299.6 19 279.6 0 255.5 0c-24.2 0-44.4 19.2-47.4 44h-51.5c-2.8 0-5.3 2.3-5.2 5.3v58c.2 2.8 2.5 5.2 5.4 5.2zM255 22.6a24.8 24.8 0 0 1 24.8 24.8c0 13.7-11.1 24.9-24.8 24.9a24.8 24.8 0 0 1-24.8-24.8c0-13.8 11.1-24.9 24.8-24.9zm105.4 248.7H200.8c-7.9 0-14.4 6.5-14.4 14.4s6.5 14.4 14.4 14.4h159.4c7.9 0 14.4-6.5 14.4-14.4 0-8-6.3-14.4-14.2-14.4zm-159.6-87.9c-7.9 0-14.4 6.5-14.4 14.4s6.5 14.4 14.4 14.4h159.4c7.9 0 14.4-6.5 14.4-14.4s-6.5-14.4-14.4-14.4H200.8z"/><circle cx="152.8" cy="285.5" r="16"/><path d="M445.3 79.9a46.49 46.49 0 0 0-32.9-13.6h-26.7v27.2h26.7c5.3 0 10.2 2.2 13.7 5.7 3.5 3.6 5.7 8.4 5.7 13.7v50.6l-.1.3v301.7c0 5.3-2.2 10.2-5.7 13.7-3.6 3.5-8.4 5.7-13.7 5.7H259.4l-.4.8c0-.3-.1-.5-.1-.8H99.6c-5.3 0-10.2-2.2-13.7-5.7-3.5-3.6-5.7-8.4-5.7-13.7V112.9c0-5.3 2.2-10.2 5.7-13.6 3.5-3.5 8.3-5.7 13.6-5.7h26.7V66.3H99.5c-12.8 0-24.6 5.2-33 13.6s-13.6 20.1-13.6 33v352.4c0 12.8 5.3 24.5 13.8 33 8.4 8.5 20.1 13.7 32.9 13.7h145.2 22.4 145.2c12.7 0 24.5-5.3 32.9-13.7 8.5-8.5 13.8-20.1 13.8-33V316.8v-204c-.1-12.6-5.4-24.4-13.8-32.9z"/><circle cx="152.8" cy="197.8" r="16"/><path d="M152.8 389.6c8.8 0 16-7.2 16-16.2 0-8.8-7.1-16-16-16s-16 7.2-16 16c-.1 8.9 7.2 16.2 16 16.2zM360.4 359H200.8c-7.9 0-14.4 6.5-14.4 14.4s6.5 14.4 14.4 14.4h159.4c7.9 0 14.4-6.5 14.4-14.4.1-7.9-6.3-14.4-14.2-14.4z"/></svg>
						<h1>Order # <?php echo $order_id; ?></h1>
					</div>
					<?php if($_GET['flag']=="complete"){ ?> 
						<div class="section_flag_row">
							<div class="section_full">
								<?php echo "<strong>Order Published Successfully!</strong>&nbsp;&nbsp;-&nbsp;&nbsp;"; ?><a href="http://www.look2homes.com/l2fp/floorplan/unbranded/index/<?php echo $order_id;?>" target="_blank"><svg xmlns="http://www.w3.org/2000/svg" class="section_icon" viewBox="0 0 800 800"><path d="M0 697.8l34-69.3 47-96.4 42.3-87.3c3.7-7.7 9-10.9 17.6-10.8h95.8c7.5-.1 13.4 1.9 18.5 7.8 12.4 14.5 25.3 28.5 39 43.9h-8.1c-36.8 0-73.7.2-110.5-.1-10.3-.1-16.5 3.9-20.9 13.2L88.3 635.4l-12.1 25.2h647.4L713 638.4l-68.4-140.6c-4.1-8.5-10-12.3-19.6-12.2l-111.2.1h-8.5l17.8-20.2c7.7-8.6 15.1-17.4 23.3-25.4 3.2-3.1 8.4-5.6 12.7-5.7l102.2-.3c7.1 0 11.5 3.6 14.4 9.6l38.9 79.7 53 109 32 65.3v7.7c-4.4 6.8-11.1 7.5-18.5 7.5-157.3-.1-314.6 0-471.9 0H17.4c-7.2 0-13.6-1-17.8-7.5.4-2.5.4-5.1.4-7.6h0zm581-436.4c-.6 38.5-6 66.3-18.6 92.4-9.7 20-22.5 38-37 54.8l-65.1 75.2c-19.2 23.2-34.5 49-46.2 76.8-1 2.3-2.1 4.7-3.5 6.8-5.1 7.7-15.7 8-20.9.3-2.7-4-4.5-8.7-6.5-13.1-17.6-39.5-43.8-73.1-72.5-105-21.5-23.8-43.9-46.9-61.2-74.2-26.3-41.5-35.7-86.4-27.3-134.9 7.2-41.5 26-76.8 56.6-105.4 30.5-28.6 66.9-44.3 109-47 36.8-2.4 71.1 5.4 102.8 23.8 30.6 17.7 53.9 42.5 70.3 73.8 14 26.6 19.9 55.3 20.1 75.7h0zm-181.4-88.9c-52.7.2-95.7 43.5-95.5 96.2.2 52.3 43.5 95.4 95.8 95.3 52.8-.1 96.4-43.7 96-95.8-.5-53-43.7-95.9-96.3-95.7h0z"/></svg>Unbranded Tour</a>
							</div>
						</div>
					<?php } ?>
					<div class="section_titles">
						<div class="section_full"?>
							<h1><?php if($orders['street_num']!="0") { echo $orders['street_num']; } ?> <?php if($orders['street_dir']!="") { echo $orders['street_dir']; } ?> <?php echo $orders['street_name']; ?> <?php if($orders['unit']!="") { echo $orders['unit']; } ?> - <?php echo $orders['city'].", ".$orders['state']." ".$orders['zip']; ?></h1>
						</div>
					</div>
					<form name="form1" id="form1" method="post" action="complete.php" >
						<div class="section_titles">
							<div class="section_form_25"><strong>Enter Area Sizes:</strong></div>
							<div class="clear"></div>
						</div>
						<div class="section_result_row">
							<div class="section_form_25">Area:</div>
							<div class="section_form_25">Sqft:</div>
							<div class="section_form_25">Sort:</div>
							<div class="section_form_25">&nbsp;</div>
							<div class="clear"></div>
							<?php
								if(!($area_order = mysqli_query($link,"SELECT * FROM area_calc where order_id = '$order_id' ORDER BY sort ASC"))){
									printf("%s", sprintf("internal error %d:%s\n", mysqli_errno($link), mysqli_error($link)));
									exit();
								}
								while($disp_area = mysqli_fetch_array($area_order)){
				 			?>
							<div class="section_form_25"><?php echo $disp_area['area']; ?></div>
							<div class="section_form_25"><?php echo $disp_area['sqft']; ?></div>
							<div class="section_form_25"><input name="sort<?php echo $disp_area['id']; ?>" type="text" id="sort<?php echo $disp_area['id']; ?>" value="<?php echo $disp_area['sort']; ?>" size="4" /></div>
							<div class="section_form_25"><a class="remove_rec" href="complete.php?order_id=<?php echo $order_id ?>&amp;area_id=<?php echo $disp_area['id']; ?>&amp;task=remsqft"><svg xmlns="http://www.w3.org/2000/svg" class="section_icon" viewBox="0 0 800 653.3" ><path d="M398.6 314.9L584.1 131c6.1-6.1 16-6 22.1.1L790 316.7c6.1 6.1 6 16-.1 22.1l-44.4 44.1c-6.1 6.1-16 6-22.1-.1l-71.7-72.3c4.3 89.1-28.1 179.5-97 246.9C427 682 224 680.1 98.6 553.1-26.6 426.2-25.5 221.9 101.2 96.4 160.5 37.6 236.9 6.5 314.3 3c9-.4 16.5 6.7 16.4 15.7l-.3 62.6c0 8.2-6.5 15.1-14.7 15.5-53.9 3.1-107 25.1-148.5 66.2-90.1 89.3-90.7 234.4-1.4 324.5s234.4 90.7 324.5 1.4c54.2-53.7 76-127.6 65.4-197.5l-91 90.2c-6.1 6.1-16 6-22.1-.1l-44.1-44.4c-6.1-6.2-6.1-16.1.1-22.2h0z"/></svg>Remove</a></div>
							<div class="clear"></div>
							<?php } ?>
							<div class="section_form_25"><input name="area" type="text" id="area" size="30" /></div>
							<div class="section_form_25"><input name="sqft" type="text" id="sqft" size="10" /></div>
							<div class="section_form_25"><input name="sort" type="text" id="sort" size="4" /></div>
							<div class="section_form_25">&nbsp;</div>
							<div class="clear"></div>
							<div class="section_form_25"><input class="section_submit_button" name="Submit" type="submit" value="Add Area" /><input name="order_id" type="hidden" id="order_id" value="<?php echo $order_id; ?>" /></div>
							<div class="section_form_25">&nbsp;</div>
							<div class="section_form_25"><input class="section_submit_button" name="UpdateSQFT" type="submit" value="Update Sort" /><input name="order_id" type="hidden" id="order_id" value="<?php echo $order_id; ?>" /></div>
							<div class="section_form_25">&nbsp;</div>
							<div class="clear"></div>
						</div>
					</form>
					<form name="form1" id="form1" method="post" action="complete.php" >
						<div class="section_titles">
							<div class="section_form_25"><strong>Property Details:</strong></div>
							<div class="clear"></div>
						</div>
						<div class="section_result_row">
							<?php 
								if(!($pd_sel = mysqli_query($link,"SELECT * FROM order_data WHERE order_id = '$order_id'"))){
									printf("%s", sprintf("internal error %d:%s\n", mysqli_errno($link), mysqli_error($link)));
									exit();
								} 
								$show_pd = mysqli_fetch_array($pd_sel); 
							?>
							<div class="section_form_25"># of Bedrooms:</div>
							<div class="section_form_25"><input name="beds" type="text" id="beds" size="4" value="<?php echo $show_pd['beds']; ?>" /></div>
							<div class="section_form_25"># of Bathrooms:</div>
							<div class="section_form_25"><input name="baths" type="text" id="baths" size="4" value="<?php echo $show_pd['baths']; ?>" /></div>
							<div class="clear"></div>
							<div class="section_form_25">&nbsp;</div>
							<div class="section_form_25">&nbsp;</div>
							<div class="section_form_25"># of Half Baths:</div>
							<div class="section_form_25"><input name="h_baths" type="text" id="h_baths" size="4" value="<?php echo $show_pd['h_bath']; ?>" /></div>
							<div class="clear"></div>
							<div class="section_form_25"># of Offices:</div>
							<div class="section_form_25"><input name="offices" type="text" id="offices" size="4" value="<?php echo $show_pd['offices']; ?>" /></div>
							<div class="section_form_25">&nbsp;</div>
							<div class="section_form_25">&nbsp;</div>
							<div class="clear"></div>
							<div class="section_form_25">&nbsp;</div>
							<div class="clear"></div>
							<div class="section_form_50">SqFt calculated using <em>Exterior</em> dimensions:&nbsp;&nbsp;&nbsp;<input name="exterior" type="checkbox" id="exterior" value="y" <?php if($show_pd['external']=="y") { echo "checked"; } ?>/></div>
							<div class="section_form_50">SqFt calculated using <em>Interior</em> demensions:&nbsp;&nbsp;&nbsp;<input name="interior" type="checkbox" id="interior" value="y" <?php if($show_pd['internal']=="y") { echo "checked"; } ?> /></div>
							<div class="clear"></div>
							<div class="section_form_25">Floor drawn by:</div>
							<div class="section_form_25"><input name="drawn" type="text" id="drawn" value="<?php echo $show_pd['drawn_by']; ?>" size="24" /></div>
							<div class="clear"></div>
							<div class="section_form_25">&nbsp;</div>
							<div class="clear"></div>
							<div class="section_form_25">Interior Only FP  Drawn by Look2:&nbsp;&nbsp;<input name="fp_done" type="checkbox" id="fp_done" value="y" <?php if($show_pd['fp_done']=="y") { echo "checked"; } ?> /></div>
							<div class="section_form_25">&nbsp;</div>
							<div class="section_form_25">Standard Photos processed by Look2:&nbsp;&nbsp;<input name="photos_done" type="checkbox" id="photos_done" value="y" <?php if($show_pd['photos_done']=="y") { echo "checked"; } ?> /></div>
							<div class="section_form_25">&nbsp;</div>
							<div class="clear"></div>
							<div class="section_form_25">Complete FP  Drawn by Look2:&nbsp;&nbsp;<input name="efp_done" type="checkbox" id="efp_done" value="y" <?php if($show_pd['efp_done']=="y") { echo "checked"; } ?> /></div>
							<div class="section_form_25">&nbsp;</div>
							<div class="section_form_25">TruView Photos processed by Look2:&nbsp;&nbsp;<input name="tp_done" type="checkbox" id="tp_done" value="y" <?php if($show_pd['tp_done']=="y") { echo "checked"; } ?> /></div>
							<div class="section_form_25">&nbsp;</div>
							<div class="clear"></div>
							<div class="section_form_25">Order Fix:&nbsp;&nbsp;<input name="fix_done" type="checkbox" id="fix_done" value="y" <?php if($show_pd['fix_done']=="y") { echo "checked"; } ?> /></div>
							<div class="section_form_25">&nbsp;</div>
							<div class="section_form_25">Rush processed by:&nbsp;&nbsp;<?php if(!($rush_db = mysqli_query($link,"SELECT * FROM users WHERE active = 'y' AND (level = 'admin' OR level = 'staff' OR level = 'dealer')  ORDER BY lastname ASC"))){
											printf("%s", sprintf("internal error %d:%s\n", mysqli_errno($link), mysqli_error($link)));
											exit();
									} ?>
                  					<select name="rush_id" id="rush_id" onChange='loadSubCats(this.value)'>
									<option value="0">No Rush</option>
                   					<?php while($sel_rush=mysqli_fetch_array($rush_db)){	?>
                						<option value="<?php echo $sel_rush['id']; ?>"  <?php if($show_pd['rush_done']==$sel_rush['id']) { ?>selected<?php } ?>><?php echo $sel_rush['firstname'].'&nbsp;'.$sel_rush['lastname']; ?></option> 
               					 	<?php } ?>
                 					</select>
							</div>
							<div class="section_form_25">&nbsp;</div>
							<div class="clear"></div>
							<div class="section_form_25"><input class="section_submit_button" name="SubmitPD" type="submit" value="Insert" /><input name="order_id" type="hidden" id="order_id" value="<?php echo $order_id; ?>" /></div>
							<div class="clear"></div>
						</div>
					</form>
					<form name="form1" id="form1" method="post" action="complete.php" >
						<div class="section_titles">
							<div class="section_form_25"><strong>Publish:</strong></div>
							<div class="clear"></div>
						</div>
						<div class="section_result_row">
							<div class="section_form_25">Complete and send emails:</div>
							<div class="section_form_25"><input name="SubmitPub" type="submit" value="Publish" class="section_submit_button_md" /><input name="order_id" type="hidden" id="order_id" value="<?php echo $order_id; ?>" /></div>
							<div class="clear"></div>
						</div>
					</form>
				</div>
