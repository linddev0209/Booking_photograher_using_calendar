...
if($_GET['st']!="" && $_GET['company']=="") { 
	$company_st = $_GET['st'];	
	$letter_menu = '<a href="agents.php?st='.$company_st.'">All</a>';
	if ($_SESSION['SESS_LEVEL']=="dealer") {
		if(!($letter_num = mysqli_query($link,"SELECT * FROM companies WHERE name REGEXP '^[0-9]' AND state = '$company_st' AND location = '$dealer_id'"))){
			printf("%s", sprintf("internal error %d:%s\n", mysqli_errno($link), mysqli_error($link)));
			exit();
		}
	} else {
		if(!($letter_num = mysqli_query($link,"SELECT * FROM companies WHERE name REGEXP '^[0-9]' AND state = '$company_st'"))){
			printf("%s", sprintf("internal error %d:%s\n", mysqli_errno($link), mysqli_error($link)));
			exit();
		}
	}
	...
	if($num_count!="0") { 
		$letter_menu .= '<a href="agents.php?st='.$company_st.'&letter=num">#</a>'; 
	} else { 
		$letter_menu .= '<span class="letter_no_result">#</span>'; 
	} 
} else if($_GET['st']!="" && $_GET['company']!="") { 
	$company_st = $_GET['st'];	
	$company_id = $_GET['company'];	
	$letter_menu = '<a href="agents.php?st='.$company_st.'&company='.$company_id.'">All</a>';
	if(!($letter_num = mysqli_query($link,"SELECT * FROM users WHERE lastname REGEXP '^[0-9]' AND company_id = '$company_id'"))){
		printf("%s", sprintf("internal error %d:%s\n", mysqli_errno($link), mysqli_error($link)));
		exit();
	}
	$num_count = mysqli_num_rows($letter_num);
	...
	if($num_count!="0") { 
		$letter_menu .= '<a href="agents.php?st='.$company_st.'&company='.$company_id.'&letter=num">#</a>'; 
	} else { 
		$letter_menu .= '<span class="letter_no_result">#</span>'; 
	} 
}
...
foreach($letter_sort as $letter){
	if($_GET['st']!="" && $_GET['company']=="") { 
		if ($_SESSION['SESS_LEVEL']=="dealer") {
			if(!($letter_result = mysqli_query($link,"Select * FROM companies WHERE name LIKE '".$letter."%' AND state = '$company_st' AND location = '$dealer_id'"))){
				printf("%s", sprintf("internal error %d:%s\n", mysqli_errno($link), mysqli_error($link)));
				exit();
			}
		} else {
			if(!($letter_result = mysqli_query($link,"Select * FROM companies WHERE name LIKE '".$letter."%' AND state = '$company_st'"))){
				printf("%s", sprintf("internal error %d:%s\n", mysqli_errno($link), mysqli_error($link)));
				exit();
			}
		}
		...
		if($letter_count!="0") { 
			$letter_menu .= '<a href="agents.php?st='.$company_st.'&letter='.$letter.'">'.$letter.'</a>'; 
		} else { 
			$letter_menu .= '<span class="letter_no_result">'.$letter.'</span>'; 
		} 
		
	} else if($_GET['st']!="" && $_GET['company']!="") { 
		if(!($letter_result = mysqli_query($link,"SELECT * FROM users WHERE lastname LIKE '".$letter."%' AND company_id = '$company_id'"))){
			printf("%s", sprintf("internal error %d:%s\n", mysqli_errno($link), mysqli_error($link)));
			exit();
		}
		$letter_count = mysqli_num_rows($letter_result);
		...
		if($letter_count!="0") { 
			$letter_menu .= '<a href="agents.php?st='.$company_st.'&company='.$company_id.'&letter='.$letter.'">'.$letter.'</a>'; 
		} else { 
			$letter_menu .= '<span class="letter_no_result">'.$letter.'</span>'; 
		} 
	}
...
if ($_GET['task'] == "remage"){
	if(!($agent_rem = mysqli_query($link,"DELETE FROM users WHERE id = '$agent_id'"))){
				printf("%s", sprintf("internal error %d:%s\n", mysqli_errno($link), mysqli_error($link)));
				exit();
			}
...
<html>
	<head>
		...
		<title>Agents | Look2 Home Marketing</title>
		...
		<div id="right_column">
			...
			<div id="rc_content_container">
				<div class="section_full_ra">
					<?php if($_GET['company']!="") { $company_id = $_GET['company'];  ?>
						<a href="agent_edit.php?company_id=<?php echo $company_id; ?>">Add New Agent</a>	
					<?php } else { ?>
						<a href="agent_edit.php">Add New Agent</a>	
					<?php } ?>
				</div>
				<div class="section_container">
					...
					<div class="section_header">
						<svg xmlns="http://www.w3.org/2000/svg" class="section_icon" xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="0 0 364 364"><defs><path id="A" d="M0 0h364v364H0z"/></defs><clipPath id="B"><use xlink:href="#A"/></clipPath><path d="M278.8 194.8l-92.5 92.5-92.5-92.5c-27 12.7-49.8 42.1-49.8 69.8v200.8c0 11.4 5.8 21.4 14.6 27.4l-20.7 97.5-64.9-13.9c-5.4-1.2-10.7 2.3-11.9 7.7v.1L-76.3 760c-1.2 5.4 2.3 10.7 7.7 11.9l32.3 6.9c.1-2 .2-4 .7-6.1 4.8-22.4 26.8-36.8 49.3-32 6.5 1.4 12.2 4.4 17.1 8.2l30.6-143.6 22.8-107.1h15.9l12.6 268.4c0 19.4 15.7 35.2 35.2 35.2h38.4 38.4c19.4 0 35.2-15.8 35.2-35.2l12.6-268.4h23.1c18.3 0 33-14.8 33-33V264.6c-.1-27.7-22.9-57.1-49.8-69.8h0zM187 152.7c41.6 0 75.3-33.7 75.3-75.3C262.4 35.7 228.7 2 187 2c-41.6 0-75.3 33.7-75.3 75.3s33.7 75.4 75.3 75.4zm-21 31.7v59.2l20.3 27.1 20.3-27.1v-59.2h-20.3H166zM9.3 761.3c-11.1-2.4-22 4.7-24.4 15.8s4.7 22 15.9 24.4c11.1 2.4 22-4.7 24.4-15.8 2.3-11.1-4.8-22-15.9-24.4z" clip-path="url(#B)"/></svg><h1>Agent Accounts - Select State</h1>
					</div>
					<?php 
						if ($_SESSION['SESS_LEVEL']=="dealer") {
							if(!($state_sel = mysqli_query($link,"SELECT distinct(state) FROM companies WHERE location = '$dealer_id' ORDER BY state ASC"))){
							printf("%s", sprintf("internal error %d:%s\n", mysqli_errno($link), mysqli_error($link)));
							exit();
							}
						} else {
							if(!($state_sel = mysqli_query($link,"SELECT distinct(state) FROM companies ORDER BY state ASC"))){
								printf("%s", sprintf("internal error %d:%s\n", mysqli_errno($link), mysqli_error($link)));
								exit();
							}
						}
					...
					<div class="section_result_row">
						<a href="agents.php?st=<?php echo $comp_states['state']; ?>">
							<div class="section_full">
								<?php $st = strtoupper($comp_states['state']); echo $state_list[$st]; ?>
							</div>
						</a>
					</div>
					<?php } } else if($_GET['st']!="" && $_GET['company']=="") { $company_st = $_GET['st']; ?>
					<div class="section_header">
						<svg xmlns="http://www.w3.org/2000/svg" class="section_icon" xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="0 0 364 364"><defs><path id="A" d="M0 0h364v364H0z"/></defs><clipPath id="B"><use xlink:href="#A"/></clipPath><path d="M278.8 194.8l-92.5 92.5-92.5-92.5c-27 12.7-49.8 42.1-49.8 69.8v200.8c0 11.4 5.8 21.4 14.6 27.4l-20.7 97.5-64.9-13.9c-5.4-1.2-10.7 2.3-11.9 7.7v.1L-76.3 760c-1.2 5.4 2.3 10.7 7.7 11.9l32.3 6.9c.1-2 .2-4 .7-6.1 4.8-22.4 26.8-36.8 49.3-32 6.5 1.4 12.2 4.4 17.1 8.2l30.6-143.6 22.8-107.1h15.9l12.6 268.4c0 19.4 15.7 35.2 35.2 35.2h38.4 38.4c19.4 0 35.2-15.8 35.2-35.2l12.6-268.4h23.1c18.3 0 33-14.8 33-33V264.6c-.1-27.7-22.9-57.1-49.8-69.8h0zM187 152.7c41.6 0 75.3-33.7 75.3-75.3C262.4 35.7 228.7 2 187 2c-41.6 0-75.3 33.7-75.3 75.3s33.7 75.4 75.3 75.4zm-21 31.7v59.2l20.3 27.1 20.3-27.1v-59.2h-20.3H166zM9.3 761.3c-11.1-2.4-22 4.7-24.4 15.8s4.7 22 15.9 24.4c11.1 2.4 22-4.7 24.4-15.8 2.3-11.1-4.8-22-15.9-24.4z" clip-path="url(#B)"/></svg><h1>Agent Accounts - Select Company</h1>
					</div>
					...
					<?php 
						if($_GET['letter']!="") {
							$company_letter = $_GET['letter'];
							if($company_letter!="num"){														
								if ($_SESSION['SESS_LEVEL']=="dealer"){
									if(!($company_sel = mysqli_query($link,"SELECT * FROM companies WHERE name LIKE '".$company_letter."%' AND location = '$dealer_id' AND state = '$company_st' ORDER BY name ASC"))){
										printf("%s", sprintf("internal error %d:%s\n", mysqli_errno($link), mysqli_error($link)));
										exit();
									}
								} else {
									if(!($company_sel = mysqli_query($link,"SELECT * FROM companies WHERE name LIKE '".$company_letter."%' AND state = '$company_st' ORDER BY name ASC"))){
										printf("%s", sprintf("internal error %d:%s\n", mysqli_errno($link), mysqli_error($link)));
										exit();
									}
								}
								...
								if ($_SESSION['SESS_LEVEL']=="dealer"){
									if(!($company_sel = mysqli_query($link,"SELECT * FROM companies WHERE name REGEXP '^[0-9]' AND location = '$dealer_id' AND state = '$company_st' ORDER BY name ASC"))){
										printf("%s", sprintf("internal error %d:%s\n", mysqli_errno($link), mysqli_error($link)));
										exit();
									}
								} else {
									if(!($company_sel = mysqli_query($link,"SELECT * FROM companies WHERE name REGEXP '^[0-9]' AND state = '$company_st' ORDER BY name ASC"))){
										printf("%s", sprintf("internal error %d:%s\n", mysqli_errno($link), mysqli_error($link)));
										exit();
									}
								}
							...
							if ($_SESSION['SESS_LEVEL']=="dealer"){
								if(!($company_sel = mysqli_query($link,"SELECT * FROM companies WHERE location = '$dealer_id' AND state = '$company_st' ORDER BY name ASC"))){
									printf("%s", sprintf("internal error %d:%s\n", mysqli_errno($link), mysqli_error($link)));
									exit();
								}
							} else {
								if(!($company_sel = mysqli_query($link,"SELECT * FROM companies WHERE state = '$company_st' ORDER BY name ASC"))){
									printf("%s", sprintf("internal error %d:%s\n", mysqli_errno($link), mysqli_error($link)));
									exit();
								}
							}
						}
						while($select_comp=mysqli_fetch_array($company_sel)) { 
					?>
					<div class="section_result_row">
						<a href="agents.php?st=<?php echo $select_comp['state']; ?>&company=<?php echo $select_comp['id']; ?>">
							<div class="section_sel_state">
								<?php echo $select_comp['name']; ?>
							</div>
							<div class="section_city">
								<?php echo $select_comp['city'].", ".$select_comp['state']; ?>
							</div>
							<div class="clear"></div>
						</a>
					</div>
					<?php } } else if($_GET['st']!="" && $_GET['company']!="") { $company_st = $_GET['st']; $company_id = $_GET['company']; ?>
					<div class="section_header">
						<svg xmlns="http://www.w3.org/2000/svg" class="section_icon" xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="0 0 364 364"><defs><path id="A" d="M0 0h364v364H0z"/></defs><clipPath id="B"><use xlink:href="#A"/></clipPath><path d="M278.8 194.8l-92.5 92.5-92.5-92.5c-27 12.7-49.8 42.1-49.8 69.8v200.8c0 11.4 5.8 21.4 14.6 27.4l-20.7 97.5-64.9-13.9c-5.4-1.2-10.7 2.3-11.9 7.7v.1L-76.3 760c-1.2 5.4 2.3 10.7 7.7 11.9l32.3 6.9c.1-2 .2-4 .7-6.1 4.8-22.4 26.8-36.8 49.3-32 6.5 1.4 12.2 4.4 17.1 8.2l30.6-143.6 22.8-107.1h15.9l12.6 268.4c0 19.4 15.7 35.2 35.2 35.2h38.4 38.4c19.4 0 35.2-15.8 35.2-35.2l12.6-268.4h23.1c18.3 0 33-14.8 33-33V264.6c-.1-27.7-22.9-57.1-49.8-69.8h0zM187 152.7c41.6 0 75.3-33.7 75.3-75.3C262.4 35.7 228.7 2 187 2c-41.6 0-75.3 33.7-75.3 75.3s33.7 75.4 75.3 75.4zm-21 31.7v59.2l20.3 27.1 20.3-27.1v-59.2h-20.3H166zM9.3 761.3c-11.1-2.4-22 4.7-24.4 15.8s4.7 22 15.9 24.4c11.1 2.4 22-4.7 24.4-15.8 2.3-11.1-4.8-22-15.9-24.4z" clip-path="url(#B)"/></svg><h1>Agent Accounts - Select Agent</h1>
					</div>
					<div class="section_titles">
						<div class="letter_select">
							<strong>Filter By:</strong>
					...
					<?php 
						if($_GET['letter']!="") {
							$agent_letter = $_GET['letter'];
							if($agent_letter!="num"){
								if(!($agent_sel = mysqli_query($link,"SELECT id, firstname, lastname, cell_phone, email FROM users WHERE lastname LIKE '".$agent_letter."%' AND company_id = '$company_id' ORDER BY lastname ASC"))){
									printf("%s", sprintf("internal error %d:%s\n", mysqli_errno($link), mysqli_error($link)));
									exit();
								} 
							} else {
								if(!($agent_sel = mysqli_query($link,"SELECT id, firstname, lastname, cell_phone, email FROM users WHERE lastname REGEXP '^[0-9]' AND company_id = '$company_id' ORDER BY lastname ASC"))){
									printf("%s", sprintf("internal error %d:%s\n", mysqli_errno($link), mysqli_error($link)));
									exit();
								} 	
							}
					...
					?>
					<div class="section_result_row">
						<a href="agent_edit.php?agent=<?php echo $select_agent['id']; ?>">
							<div class="section_sel_company">
								<?php echo $select_agent['lastname'].", ".$select_agent['firstname']; ?>
							</div>
							<div class="section_phone">
								<?php echo $select_agent['cell_phone']; ?>
							</div>							
						</a>
						<div class="section_phone">
							&nbsp;
						...
						<div class="section_edit">
							<a class="remove_rec" href="agents.php?a_id=<?php echo $select_agent['id']; ?>&task=remage" onClick="confirmCleanUp('agent - <?php echo $select_agent["lastname"].", ".$select_agent["firstname"]; ?>'); return returnvalue"><svg xmlns="http://www.w3.org/2000/svg" class="section_icon" viewBox="0 0 800 800"><path d="M800 400c0 220.9-179.1 400-400 400S0 620.9 0 400 179.1 0 400 0s400 179.1 400 400zm-560-40c-22.1 0-40 17.9-40 40s17.9 40 40 40h320c22.1 0 40-17.9 40-40s-17.9-40-40-40H240z" fill-rule="evenodd"/></svg>Remove</a>
						</div>
						<div class="clear"></div>
					</div>
					<?php } } if($num_sgents=="0") { ?>
