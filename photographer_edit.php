...
if ($_SESSION['SESS_LEVEL']=="dealer"){
	$d_id = $_SESSION['SESS_MEMBER_ID'];
} else {
	$d_id = $_GET['dealer_id'];
}

$da_id = $_GET['photographer'];

if ($d_id!="") {
	if(!($dealer_info = mysqli_query($link,"SELECT * FROM users where id = '$d_id'"))){
		printf("%s", sprintf("internal error %d:%s\n", mysqli_errno($link), mysqli_error($link)));
		exit();
	}
	$dealer_assign = mysqli_fetch_array($dealer_info);
} 

if ($da_id!="") {
	if(!($photographer_info = mysqli_query($link,"SELECT * FROM users where id = '$da_id'"))){
		printf("%s", sprintf("internal error %d:%s\n", mysqli_errno($link), mysqli_error($link)));
		exit();
	}
	$photographer_profile = mysqli_fetch_array($photographer_info);
	$dealer_id = $photographer_profile['primary_deal'];
}

if($_POST['SubmitPhotographer']){
	$da_id = $_POST['photographer'];
	if($da_id!="") {
		if(!($photographer_db = mysqli_query($link,"select * from users where id = '$da_id'"))){
			printf("%s", sprintf("internal error %d:%s\n", mysqli_errno($link), mysqli_error($link)));
			exit();
		}
		$pger = mysqli_fetch_array($photographer_db);
	}
	if($_POST['pass']!="") {
		$passencrypt = SHA1($_POST['pass']);
	} else {
		$passencrypt = $pger['pass'];
	}
	...
	if($da_id!="") {
		if(!($photographer_update = mysqli_query($link,"UPDATE users SET email='$email',pass='$passencrypt',firstname='$firstname',lastname='$lastname',address='$address',city='$city',state='$state',zip='$zip',cell_phone='$cellphone',level='$level',primary_deal='$deal_id',active='$active',d_fp='$d_fp',d_photo='$d_photo',d_aerial='$d_aerial',d_twilight='$d_twilight',d_3d='$d_3d' WHERE id = '$da_id'"))){
			printf("%s", sprintf("internal error %d:%s\n", mysqli_errno($link), mysqli_error($link)));
			exit();
		}
	} else {
		if(!($photographer_update= mysqli_query($link,"INSERT INTO users SET email='$email',pass='$passencrypt',firstname='$firstname',lastname='$lastname',address='$address',city='$city',state='$state',zip='$zip',cell_phone='$cellphone',level='$level',primary_deal='$deal_id',active='$active',d_fp='$d_fp',d_photo='$d_photo',d_aerial='$d_aerial',d_twilight='$d_twilight',d_3d='$d_3d'"))){
			printf("%s", sprintf("internal error %d:%s\n", mysqli_errno($link), mysqli_error($link)));
			exit();
		}
		$new_da_id = mysqli_insert_id($link);
	}
	...
	if($_FILES['uphoto']['name'] != NULL){ 
		$file_name = $_FILES['uphoto']['name'];
      	...
      	if(in_array($file_ext,$expensions)=== false){
          	header("Location: photographer_edit.php?photographer=".$photo_id."&flag=notvalid");
		  	exit;
      	}
	...
	if($da_id!="") {
		$flag = "updated";
		header("Location: photographer_edit.php?photographer=".$da_id."&flag=".$flag."");
		exit();	
	 } else {
		$flag = "created";
		header("Location: photographer_edit.php?photographer=".$new_da_id."&flag=".$flag."");
		exit();
	}
}
?>
...
<html>
	<head>
		...
		<title>Manage Photographer | Look2 Home Marketing</title>
	...
	<div id="container">
		...
		<div id="right_column">
			...
			<div id="rc_content_container">
				<form action="photographer_edit.php" method="post" name="pgeredit" id="pgeredit" enctype="multipart/form-data">
					<div class="section_container">
						<div class="section_header">
							<svg xmlns="http://www.w3.org/2000/svg" class="section_icon" viewBox="0 0 512 512"><path d="M256 0C114.509 0 0 114.498 0 256c0 141.491 114.498 256 256 256 141.491 0 256-114.498 256-256C512 114.509 397.502 0 256 0zm0 478.609c-122.746 0-222.609-99.862-222.609-222.609S133.254 33.391 256 33.391 478.609 133.254 478.609 256 378.746 478.609 256 478.609zM60.433 299.546l162.176-93.633-125.15-72.255c-37.44 48.407-49.61 109.334-37.026 165.888zm391.134-87.092l-162.176 93.633 125.148 72.255c37.441-48.406 49.612-109.334 37.028-165.888zM195.928 447.143V259.875L70.709 332.17c22.514 54.566 68.448 97.092 125.219 114.973zm33.391 7.424c59.128 7.906 118.14-10.677 162.076-51.031l-162.076-93.575v144.606zm86.753-389.71v187.27l125.219-72.295c-22.514-54.568-68.448-97.094-125.219-114.975zm-33.391-7.424c-59.128-7.906-118.14 10.679-162.076 51.031l162.076 93.575V57.433z"/></svg><h1>Manage Photographer</h1>
						</div>
						...
						<div class="section_result_row">
							...
							<div class="section_form_36">
								<input name="fname" type="text" id="fname" value="<?php echo $photographer_profile['firstname']; ?>" size="32" />
							</div>
							<div class="section_form_14">
								Address:
							</div>
							<div class="section_form_36">
								<input name="address" type="text" id="address" value="<?php echo $photographer_profile['address']; ?>" size="32" />
							</div>
							<div class="clear"></div>
							<div class="section_form_14">
								Last Name:
							...
							<div class="section_form_36">
								<input name="lname" type="text" id="lname" value="<?php echo $photographer_profile['lastname']; ?>" size="32" />
							</div>
							<div class="section_form_14">
								City:
							</div>
							<div class="section_form_36">
								<input name="city" type="text" id="city" value="<?php echo $photographer_profile['city']; ?>" size="32" />
							</div>
							<div class="clear"></div>
							<div class="section_form_14">
								&nbsp;
							...
							<div class="section_form_14">
								<input name="state" type="text" id="state" value="<?php echo $photographer_profile['state']; ?>" size="4" maxlength="2" />
							</div>
							<div class="section_form_10">
								Zip:
							</div>
							<div class="section_form_14">
								<input name="zip" type="text" id="zip" value="<?php echo $photographer_profile['zip']; ?>" size="10" maxlength="5" />
							</div>
							<div class="clear"></div>
							<div class="section_form_14">
								Email Address:
							...
							<div class="section_form_36">
								<input name="email" type="text" id="email" value="<?php echo $photographer_profile['email']; ?>" size="32" />
							</div>
							<div class="section_form_14">
								Cell Phone:
							</div>
							<div class="section_form_36">
								<input name="cell" type="text" id="cell" value="<?php echo $photographer_profile['cell_phone']; ?>" size="24" maxlength="12" /><br>ex. 555-555-5555
							</div>
							<div class="clear"></div>
							<div class="section_form_14">
								Photographer Photo:
							...
							<div class="section_form_36">
								<span class="agent_pic_order_80">
									<?php if(file_exists("floorplan/profile/".$da_id.".jpg")){ ?><img src="floorplan/profile/<?php echo $da_id; ?>.jpg" width="80px" alt="<?php echo $photographer_profile['firstname']." ". $photographer_profile['lastname']; ?>"><?php } ?>
								</span>
							</div>
							<div class="clear"></div>
							<div class="section_form_14">
							...
							<div class="section_form_50">
								<?php 
									if ($d_id!="") { 
										echo $dealer_assign['lastname'].', '.$dealer_assign['firstname'].' <input type="hidden" name="deal_id" id="deal_id" value="'.$d_id.'" />'; 
									} else {								
										if(!($deal_db = mysqli_query($link,"SELECT * FROM users WHERE level = 'dealer' ORDER BY lastname ASC"))){
											printf("%s", sprintf("internal error %d:%s\n", mysqli_errno($link), mysqli_error($link)));
											exit();
										} 
								?>
                  					<select name="deal_id" id="deal_id" >
                   					<?php while($sel_deal=mysqli_fetch_array($deal_db)){	?>
                						<option value="<?php echo $sel_deal['id']; ?>" <?php if ($sel_deal['id']==$dealer_id) echo "selected"; ?>><?php echo $sel_deal['lastname'].', '.$sel_deal['firstname']; ?></option> 
               					 	<?php } ?>
                 					</select>
								<?php } ?>
							</div>
							...
							<div class="section_form_36">
								<select name="active" id="active">
                    				...
                    				<option value="n" <?php if($photographer_profile['active']=='n') echo "selected" ?>>No</option>
                  				</select>
							</div>
							<div class="clear"></div>
							<div class="section_form_14">
							...
							<?php if ($photographer_profile['g_token']=="") { ?>
							<div class="section_form_36">
						...
						<div class="section_result_row">
							<div class="order_additional_sub">
								<div class="d_fp">
									<input type="checkbox" name="d_fp" id="d_fp" value="y" <?php if($photographer_profile['d_fp']=="y") echo "checked"; ?> /><label for="d_fp"></label><div class="clear"></div>
								</div>
								<span>
									Floor Plans: 
							...
							<div class="order_additional_sub">
								<div class="d_photo">
									<input type="checkbox" name="d_photo" id="d_photo" value="y" <?php if($photographer_profile['d_photo']=="y") echo "checked"; ?> /><label for="d_photo"></label><div class="clear"></div>
								</div>
								<span>
									Photography: 
							...
							<div class="order_additional_sub">
								<div class="d_aerial">
									<input type="checkbox" name="d_aerial" id="d_aerial" value="y" <?php if($photographer_profile['d_aerial']=="y") echo "checked"; ?> /><label for="d_aerial"></label><div class="clear"></div>
								</div>
								<span>
									Aerials: 
							...
							<div class="order_additional_sub">
								<div class="d_twilight">
									<input type="checkbox" name="d_twilight" id="d_twilight" value="y" <?php if($photographer_profile['d_twilight']=="y") echo "checked"; ?> /><label for="d_twilight"></label><div class="clear"></div>
								</div>
								<span>
									Twilight: 
							...
							<div class="order_additional_sub">
								<div class="d_3d">
									<input type="checkbox" name="d_3d" id="d_3d" value="y" <?php if($photographer_profile['d_3d']=="y") echo "checked"; ?> /><label for="d_3d"></label><div class="clear"></div>
								</div>
								<span>
									3D: 
						...
						<div class="section_footer">
							<div class="section_navigate">
								<input name="photographer" type="hidden" value="<?php echo $da_id; ?>" /><input name="level" type="hidden" value="d_assist" />
							</div>
							<div class="section_submit">
								<input class="section_submit_button_md" name="SubmitPhotographer" type="submit" id="SubmitPhotographer" value="Submit" />
							</div>
							<div class ="clear"></div>
						</div>
					</div>
