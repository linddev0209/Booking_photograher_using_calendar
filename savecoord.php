...
if(!($res = mysqli_query($link,"SELECT * FROM coordinates WHERE order_id = '$order_id' AND fp_id = '$fp_id' AND photo_id = '$photo_id'"))){
...
if(mysqli_num_rows($res) > 0 ) {
	if(!($res = mysqli_query($link,"UPDATE coordinates SET xcoord = '$x', ycoord='$y' WHERE order_id = '$order_id' AND fp_id = '$fp_id' AND photo_id = '$photo_id'"))){
		printf("%s", sprintf("internal error %d:%s\n", mysqli_errno($link), mysqli_error($link)));
		exit();
	}
} else {
	if(!($res = mysqli_query($link,"INSERT into coordinates (fp_id, photo_id, order_id, xcoord, ycoord) VALUES ($fp_id, $photo_id, $order_id, '$x', '$y')"))){
		printf("%s", sprintf("internal error %d:%s\n", mysqli_errno($link), mysqli_error($link)));
		exit();
	}
}
