...
if ($_GET['task'] == "remove"){ 
	$fp_id = $_GET['fp_id'];
	if(!($fp_delete = mysqli_query($link,"DELETE FROM floorplans WHERE id='$fp_id'"))){
		printf("%s", sprintf("internal error %d:%s\n", mysqli_errno($link), mysqli_error($link)));
		exit();
	}
	$del_path = 'floorplan/'. $order_id .'/'; 
	$del_ext = ".jpg" ;
	$del_file = $del_path . $order_id . "_".$fp_id . $del_ext ;
	unlink($del_file);
	header("Location: fplan.php?order_id=".$order_id);
	exit();
}

if ($_GET['task'] == "reminfo"){ 
	$icon_id = $_GET['icon_id'];
	if(!($info_delete = mysqli_query($link,"DELETE FROM info_cont WHERE id='$icon_id'"))){
		printf("%s", sprintf("internal error %d:%s\n", mysqli_errno($link), mysqli_error($link)));
		exit();
	}
	header("Location: fplan.php?order_id=".$order_id);
...
if($_POST['UpdateINFO']){
	$info_id = $_POST['order_id'];
	if(!($icup_sort = mysqli_query($link,"SELECT * FROM info_cont where order_id = '$info_id' ORDER BY sort ASC"))){
		printf("%s", sprintf("internal error %d:%s\n", mysqli_errno($link), mysqli_error($link)));
		exit();
	}
	while($sortinfo=mysql_fetch_array($icup_sort)){
		$icid = $sortinfo['id'];
		$sort = $_POST['sort'.$icid];
		if($sort!=""){
			if(!($rec_sqftsort = mysqli_query($link,"UPDATE info_cont set sort = '$sort' WHERE id = '$icid'"))){
				printf("%s", sprintf("internal error %d:%s\n", mysqli_errno($link), mysqli_error($link)));
				exit();
			}
		}
	}
	header("Location: fplan.php?order_id=".$info_id);
	exit();
}
...
if($_POST['SubmitINFO']){
	...
	$i_sort = $_POST['sort'];
	$fp_i = $_POST['fp_id'];
	$order_i = $_POST['order_id'];
	if(!($info_update = mysqli_query($link,"INSERT INTO info_cont SET fp_id='$fp_i',order_id='$order_i',label='$label',content='$i_content',sort='$i_sort'"))){
		printf("%s", sprintf("internal error %d:%s\n", mysqli_errno($link), mysqli_error($link)));
		exit();
	}
	header("Location: fplan.php?order_id=".$order_i);
	exit();
}

if($_POST['Submit']){
	$order_fp = $_POST['order_id'];
	$fp_label = $_POST['label'];
	$fp_sort = $_POST['sort'];
	
	if($_FILES['newfp']['type'] != "image/jpeg") {
	header("Location: photos.php?order_id=".$order_photo."&flag=notvalid");
    exit;
}
	$verifyimg = getimagesize($_FILES['newfp']['tmp_name']);
	...
	if(!($fp_update = mysqli_query($link,"INSERT INTO floorplans SET order_id='$order_fp',label='$fp_label',sort='$fp_sort'"))){
		printf("%s", sprintf("internal error %d:%s\n", mysqli_errno($link), mysqli_error($link)));
		exit();
	}
	$photo_id = mysqli_insert_id($link); 
	$extension = strrchr($_FILES['newfp']['name'],'.');
	$extension = strtolower($extension);
	$save_path = 'floorplan/'. $order_fp .'/'; 
	if(!is_dir($save_path)) mkdir($save_path);
	$filename = $save_path . $order_fp ."_". $photo_id . $extension; 
	move_uploaded_file($_FILES['newfp']['tmp_name'],$filename);
	header("Location: fplan.php?order_id=".$order_fp);
	exit();
}

if($_POST['fp_rep']){
	$order_fp = $_POST['order_id'];
	if(!($rep_fp = mysqli_query($link,"SELECT * FROM floorplans where order_id = '$order_fp' ORDER BY sort ASC"))){
		printf("%s", sprintf("internal error %d:%s\n", mysqli_errno($link), mysqli_error($link)));
		exit();
	}
	while($uprep_fp = mysqli_fetch_array($rep_fp)){
		$fpr_id = $uprep_fp['id'];
		if(!empty($_FILES['replace_ufp'.$fpr_id]['name'])){
			if($_FILES['replace_ufp'.$fpr_id]['type'] != "image/jpeg") {
			header("Location: fplan.php?order_id=".$order_fp."&flag=notvalid");
    		exit;
			}
			$verifyimg = getimagesize($_FILES['replace_ufp'.$fpr_id]['tmp_name']);
			...
			$extension = strrchr($_FILES['replace_ufp'.$fpr_id]['name'],'.');
			$extension = strtolower($extension);
			$save_path = 'floorplan/'. $order_fp .'/'; 
			if(!is_dir($save_path)) mkdir($save_path);
			$filename = $save_path . $order_fp ."_". $fpr_id . $extension; 
			move_uploaded_file($_FILES['replace_ufp'.$fpr_id]['tmp_name'],$filename);
			header("Location: fplan.php?order_id=".$order_fp);
		}
	}
}
...
<html>
	<head>
		...
		<title>Upload Floor Plan(s) | Look2 Home Marketing</title>
		...
		<div id="right_column">
			...
			<div id="rc_content_container">
				...
				<div class="section_container">
					...
					<form name="form1" id="form1" method="post" action="fplan.php" >
						<div class="section_titles">
							<div class="section_form_25"><strong>Add Info Icon:</strong></div>
							<div class="clear"></div>
						</div>
						<div class="section_result_row">
							<div class="section_form_25">Label:</div>
							<div class="section_form_25">Content:</div>
							<div class="section_form_25">Sort:</div>
							<div class="section_form_25">Floor Plan:</div>
							<div class="clear"></div>
							<?php
								if(!($ic_order = mysqli_query($link,"SELECT * FROM info_cont where order_id = '$order_id' ORDER BY sort ASC"))){
									printf("%s", sprintf("internal error %d:%s\n", mysqli_errno($link), mysqli_error($link)));
									exit();
								}
								while($disp_icons = mysqli_fetch_array($ic_order)){
									$position=36;
									$fp_assigned = $disp_icons['fp_id'];
				 			?>
							<div class="section_form_25"><?php echo $disp_icons['label']; ?></div>
							<div class="section_form_25"><?php $content = substr($disp_icons['content'], 0, $position); echo $content; echo "..."; ?></div>
							<div class="section_form_25"><input name="sort<?php echo $disp_icons['id']; ?>" type="text" id="sort<?php echo $disp_icons['id']; ?>" value="<?php echo $disp_icons['sort']; ?>" size="4" /></div>
							<div class="section_form_25">
								<?php 
									if(!($fp_db = mysqli_query($link,"SELECT label FROM floorplans where id = '$fp_assigned'"))){
										printf("%s", sprintf("internal error %d:%s\n", mysqli_errno($link), mysqli_error($link)));
										exit();
									} 
									$fpl_row = mysqli_fetch_array($fp_db);  echo "<strong>".$fpl_row['label']."</strong>";
								?>&nbsp;-&nbsp;<a class="remove_rec" href="fplan.php?order_id=<?php echo $order_id ?>&amp;icon_id=<?php echo $disp_icons['id']; ?>&amp;task=reminfo"><svg xmlns="http://www.w3.org/2000/svg" class="section_icon" viewBox="0 0 800 653.3" ><path d="M398.6 314.9L584.1 131c6.1-6.1 16-6 22.1.1L790 316.7c6.1 6.1 6 16-.1 22.1l-44.4 44.1c-6.1 6.1-16 6-22.1-.1l-71.7-72.3c4.3 89.1-28.1 179.5-97 246.9C427 682 224 680.1 98.6 553.1-26.6 426.2-25.5 221.9 101.2 96.4 160.5 37.6 236.9 6.5 314.3 3c9-.4 16.5 6.7 16.4 15.7l-.3 62.6c0 8.2-6.5 15.1-14.7 15.5-53.9 3.1-107 25.1-148.5 66.2-90.1 89.3-90.7 234.4-1.4 324.5s234.4 90.7 324.5 1.4c54.2-53.7 76-127.6 65.4-197.5l-91 90.2c-6.1 6.1-16 6-22.1-.1l-44.1-44.4c-6.1-6.2-6.1-16.1.1-22.2h0z"/></svg>Remove</a></div>
							<div class="clear"></div>
							<?php } ?>
							<div class="section_form_25"><input name="label" type="text" id="label" size="24" /></div>
							<div class="section_form_25"><textarea name="content" cols="24" id="content"></textarea></div>
							<div class="section_form_25"><input name="sort" type="text" id="sort" size="4" /></div>
							<div class="section_form_25">
								<?php 
								if(!($floorplan_db = mysqli_query($link,"SELECT DISTINCT * FROM floorplans where order_id = '$order_id' ORDER BY id ASC"))){
									printf("%s", sprintf("internal error %d:%s\n", mysqli_errno($link), mysqli_error($link))); 
									exit();
								} ?>
                  				<select name="fp_id" id="fp_id">
                  				<?php while($sel_fp=mysqli_fetch_array($floorplan_db)){	?>
                					<option value="<?php echo $sel_fp['id']; ?>" ><?php echo $sel_fp['label']; ?></option> 
               					<?php } ?>
                  				</select><input name="order_id" type="hidden" id="order_id" value="<?php echo $order_id; ?>" />
							</div>
							<div class="clear"></div>
							<div class="section_form_25"><input name="SubmitINFO" type="submit" id="SubmitINFO" value="Add Information" class="section_submit_button" /></div>
							<div class="section_form_25">&nbsp;</div>
							<div class="section_form_25"><input name="UpdateINFO" type="submit" id="UpdateINFO" value="Update Sort" class="section_submit_button" /></div>
							<div class="section_form_25">&nbsp;</div>
							<div class="clear"></div>
						</div>
					</form>
					<form action="fplan.php" method="post" enctype="multipart/form-data">
						<div class="section_titles">
							<div class="section_form_25"><strong>Upload Floor Plans:</strong></div>
							<div class="clear"></div>
						</div>
						<div class="section_result_row">
							<div class="section_form_25">Floor Plan:</div>
							<div class="section_form_25"><input name="newfp" type="file" id="newfp" size="18" /></div>
							<div class="section_form_25">Sort: 
							...
							<div class="section_form_25">&nbsp;</div>
							<div class="clear"></div>
							<div class="section_form_25"><input name="Submit" type="submit" id="Submit" value="Upload" class="section_submit_button"/><input name="order_id" type="hidden" id="order_id" value="<?php echo $order_id; ?>" /></div>
							<div class="clear"></div>
						</div>
					</form>
					<form action="fplan.php" method="post" enctype="multipart/form-data">
						<div class="section_titles">
							<div class="section_form_25"><strong>Floor Plan(s):</strong></div>
							<div class="clear"></div>
						</div>
						<div class="section_result_row">
							<?php
								if(!($e_fp = mysqli_query($link,"SELECT * FROM floorplans where order_id = '$order_id' ORDER BY sort ASC"))){
									printf("%s", sprintf("internal error %d:%s\n", mysqli_errno($link), mysqli_error($link)));
									exit();
								}
								while($list_fp = mysqli_fetch_array($e_fp)){
									$agent_id = $new_order['agent_id'];
									$order_date = date("m/d/y g:i A", strtotime($new_order['ordered']));
				 			?>
								<div class="section_form_75"><img src="floorplan/<?php echo $order_id; ?>/<?php echo $order_id; ?>_<?php echo $list_fp['id']; ?>.jpg" width="600" alt="Order#<?php echo $order_id; ?>" /></div>
								<div class="section_form_25"><strong><?php echo $list_fp['label']; ?></strong> - Sort: <?php echo $list_fp['sort']; ?><br /><a href="fp_builder.php?order_id=<?php echo $order_id; ?>&amp;fp_id=<?php echo $list_fp['id']; ?>"><svg xmlns="http://www.w3.org/2000/svg" class="section_icon" viewBox="0 0 512 512"><path d="M497.14 111.38l-81.09 80.84-48.58-48.41L448.56 63c-45.22-22-108.65-22.09-146.2 15.35-35.32 35.2-39.73 90.61-22.54 134.2L99.57 391.37a12 12 0 0 0 0 17l52.27 52.11a12 12 0 0 0 17 0l180-180.5c43.16 16.21 98 11.64 132.74-23 37.5-37.45 37.42-100.34 15.56-145.6zM365.45 308.62l-71.83 72 75.53 79.92a10.88 10.88 0 0 0 15.65.21l60-60.46a11 11 0 0 0-.24-15.69zM119 212c0-4.87-4-9.33-7.45-12.78l-.25-.24-1.54-1.47a1.06 1.06 0 0 1-.26-.8 16.16 16.16 0 0 1 9.52-2c1.27.13 5.91.9 12.4 4.91 3.38 2.09 32.63 30.23 43.93 40.7a11 11 0 0 0 .14 15.35l7.43 7.86 65.66-65.17-8.25-7.84a10.87 10.87 0 0 0-15.31-.06c-23-24.68-29-35.45-31-42.45-4.42-15.47 4.14-28 14-36 5.84-4.62 17.88-8.08 29-9a52.72 52.72 0 0 1 11.61.6c3.47.5 6.3 1.14 7.39 1.4a68.51 68.51 0 0 1 11 4l12-19a88.38 88.38 0 0 0-13.4-17.7 115.05 115.05 0 0 0-5.19-5.1c-7.78-7.15-28-19.2-54.59-19.2a117.38 117.38 0 0 0-44.77 8.82c-37.44 15.34-61.88 36.25-73.11 47.35l-.07.07A219.55 219.55 0 0 0 67 128.56c-5.35 7.53-4.77 15.84-4.38 21.34 0 .32 0 .67.07 1a18.41 18.41 0 0 0-10.78-3.5A18 18 0 0 0 39 152.73L2 189.62a6.79 6.79 0 0 0 0 9.6L65 262a6.72 6.72 0 0 0 9.5 0l37.06-37c3.44-3.44 7.44-8.14 7.44-13z"/></svg>Build Floor Plan</a>&nbsp;&nbsp;&nbsp;|&nbsp;&nbsp;&nbsp;<a class="remove_rec" href="fplan.php?order_id=<?php echo $order_id; ?>&amp;fp_id=<?php echo $list_fp['id']; ?>&amp;task=remove"><svg xmlns="http://www.w3.org/2000/svg" class="section_icon" viewBox="0 0 800 653.3" ><path d="M398.6 314.9L584.1 131c6.1-6.1 16-6 22.1.1L790 316.7c6.1 6.1 6 16-.1 22.1l-44.4 44.1c-6.1 6.1-16 6-22.1-.1l-71.7-72.3c4.3 89.1-28.1 179.5-97 246.9C427 682 224 680.1 98.6 553.1-26.6 426.2-25.5 221.9 101.2 96.4 160.5 37.6 236.9 6.5 314.3 3c9-.4 16.5 6.7 16.4 15.7l-.3 62.6c0 8.2-6.5 15.1-14.7 15.5-53.9 3.1-107 25.1-148.5 66.2-90.1 89.3-90.7 234.4-1.4 324.5s234.4 90.7 324.5 1.4c54.2-53.7 76-127.6 65.4-197.5l-91 90.2c-6.1 6.1-16 6-22.1-.1l-44.1-44.4c-6.1-6.2-6.1-16.1.1-22.2h0z"/></svg>Remove</a><br /><br /><strong>Replace Floor Plan Image</strong>:<br /><input name="replace_ufp<?php echo $list_fp['id']; ?>" type="file" id="replace_ufp<?php echo $list_fp['id']; ?>" size="18" /></div>
								<div class="clear"></div>
							<?php } ?>
							<div class="section_form_25">&nbsp;</div>
							<div class="section_form_25">&nbsp;</div>
							<div class="section_form_25">&nbsp;</div>
							<div class="section_form_25"><input name="fp_rep" type="submit" id="fp_rep" value="Replace" class="section_submit_button" /><input name="order_id" type="hidden" id="order_id" value="<?php echo $order_id; ?>" /></div>
							<div class="clear"></div>
						</div>
					</form>
				</div>
