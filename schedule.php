...
if ($arc!='y'){
	...
	$link = mysqli_connect($connection2, $sqluser2, $sqlpw2, $dbname2);
		if(mysqli_connect_errno()) {
    	$msg = "Database connection failed: ";
    	$msg .= mysqli_connect_error();
    	$msg .= " : " . mysqli_connect_errno();
	...
	$link2 = mysqli_connect($connection, $sqluser, $sqlpw, $dbname);
	if(mysqli_connect_errno()) {
    	$msg = "Database connection failed: ";
    	$msg .= mysqli_connect_error();
    	$msg .= " : " . mysqli_connect_errno();
...
if($orders['scheduled']!="0000-00-00 00:00:00"){
	$schedule_date = $orders['scheduled'];
} else {
	$schedule_date = date("Y-m-d H:i:s");
}

$appointment = strtotime($schedule_date);

$year = date("Y",$appointment);
$month = date("m",$appointment);
$day = date("d",$appointment);
$hour = date("g",$appointment);
$min = date("i",$appointment);
$ampm = date("A",$appointment);
...
if ($_GET['task'] == "cancel"){
	...
	header("Location: schedule.php?order_id=".$apt_cancel);
	exit();
}

if ($_GET['task'] == "cancelorder"){
	$ord_cancel = $_GET['order_id'];
	if(!($status_change = mysqli_query($link,"UPDATE orders set status = 'canceled' WHERE id = '$ord_cancel'"))){
		printf("%s", sprintf("internal error %d:%s\n", mysqli_errno($link), mysqli_error($link)));
		exit();
	}
	if ($_SESSION['SESS_LEVEL']=="admin") {
		header("Location: admin_index.php");
	}else if ($_SESSION['SESS_LEVEL']=="staff"){
		header("Location: staff_index.php");
	} else if ($_SESSION['SESS_LEVEL']=="dealer"){
...
if ($_GET['task'] == "hold"){
	$ord_hold = $_GET['order_id'];
	if(!($status_change = mysqli_query($link,"UPDATE orders set status = 'hold' WHERE id = '$ord_hold'"))){
		printf("%s", sprintf("internal error %d:%s\n", mysqli_errno($link), mysqli_error($link)));
		exit();
	}
	if ($_SESSION['SESS_LEVEL']=="admin") {
...
if($_POST['Submitnote']){ 
	$order_note = $_POST['order_id'];
	$note = $_POST['note'];
	$type = $_POST['type'];
	if(!($note_update = mysqli_query($link,"INSERT INTO notes SET order_id='$order_note',type='$type',content='$note',user='$id',date=CURDATE()"))){
		printf("%s", sprintf("internal error %d:%s\n", mysqli_errno($link), mysqli_error($link)));
		exit();
	}
	header("Location: schedule.php?order_id=".$order_note);
	exit();
}

if($_POST['Submitint']){
	$order_int = $_POST['order_id'];
	$intnote = $_POST['intnote'];
	$type = $_POST['type'];
	if(!($intnote_update = mysqli_query($link,"INSERT INTO notes SET order_id='$order_int',type='$type',content='$intnote',user='$id',date=CURDATE()"))){
		printf("%s", sprintf("internal error %d:%s\n", mysqli_errno($link), mysqli_error($link)));
		exit();
	}
	header("Location: schedule.php?order_id=".$order_int);
	exit();
}

if($_POST['Submit']){
	$time_id = $_POST['time_id'];
	$schedule = $_POST['year']."-".$_POST['month']."-".$_POST['day']." ".$_POST['hour'].":".$_POST['min'].":00 ".$_POST['ampm'];
	$schedule = date("Y-m-d H:i:00", strtotime($schedule));
	$schedule_display = date("l, M d, h:i A", strtotime($schedule));
	if(!($set_apt = mysqli_query($link,"UPDATE orders SET scheduled='$schedule', status='scheduled' WHERE id = '$time_id'"))){
		printf("%s", sprintf("internal error %d:%s\n", mysqli_errno($link), mysqli_error($link)));
		exit();
	}
...
<html>
	<head>
		...
		<title>Schedule Order | Look2 Home Marketing</title>
				...
				<?php } } else { ?>
					<div class="section_error_row">
						<div class="section_full">
							<?php echo "<strong>This order is archived and can not be modified until unarchived!</strong>"; ?>
						</div>
					</div>
				<?php } ?>
					...
					<div class="section_result_row">
						...
						<div class="section_detail_links">
							<div class="section_review_full">
								<?php if ($arc!='y'){ ?>
									<a href="enter_order1.php?order_id=<?php echo $order_id; ?>"><svg xmlns="http://www.w3.org/2000/svg" class="section_icon_sg" viewBox="0 0 800 800"><path d="M675.7 0L550.2 109.4s41.4 24.6 79.4 59.1c36.8 34.6 63.3 73.7 63.3 73.7l107-123.9S798.8 96 748.1 48C696.4 0 675.7 0 675.7 0zM267 612.6l-5.7 53.6 398.3-401.7s-26.5-35.7-58.7-65.8c-33.4-31.2-71.4-55.8-71.4-55.8L131.2 544.5l62.2-3.4 6.9 64.7 66.7 6.8zM0 783.3l99-196.4 55.3-4.5 4.6 61.4 66.8 8.9-6.9 49.1L18.4 800 0 783.3z"/></svg>Modify Order</a>&nbsp;&nbsp;|&nbsp;&nbsp;<a class="remove_rec" href="<?php echo $_SERVER["PHP_SELF"]; ?>?task=cancelorder&amp;order_id=<?php echo $order_id; ?>"><svg xmlns="http://www.w3.org/2000/svg" class="section_icon_sg" viewBox="0 0 800 800"><path d="M400 0C178.8 0 0 178.8 0 400s178.8 400 400 400 400-178.8 400-400S621.2 0 400 0zm200 543.6L543.6 600 400 456.4 256.4 600 200 543.6 343.6 400 200 256.4l56.4-56.4L400 343.6 543.6 200l56.4 56.4L456.4 400 600 543.6z"/></svg>Cancel Order</a>&nbsp;&nbsp;|&nbsp;&nbsp;<a class="remove_rec" href="<?php echo $_SERVER["PHP_SELF"]; ?>?task=hold&amp;order_id=<?php echo $order_id; ?>"><svg xmlns="http://www.w3.org/2000/svg" class="section_icon_sg" viewBox="0 0 800 800"><path d="M400 0C178.8 0 0 178.8 0 400s178.8 400 400 400 400-178.8 400-400S621.2 0 400 0zm-65.5 619.5h-89.7v-439h89.7v439zm220.8 0h-89.7v-439h89.7v439z"/></svg>Place on Hold</a>
								<?php } ?>
							</div>
							<div class="section_review_full">
							...
							<div class="section_review_full">
								<strong>Dealer:</strong>&nbsp;<?php $assigned = $orders['assigned'];
									if ($arc!='y'){
										if(!($d_order = mysqli_query($link,"SELECT firstname, lastname FROM users where id = '$assigned'"))){
											printf("%s", sprintf("internal error %d:%s\n", mysqli_errno($link), mysqli_error($link)));
											exit();
										}
									} else {
										if(!($d_order = mysqli_query($link2,"SELECT firstname, lastname FROM users where id = '$assigned'"))){
											printf("%s", sprintf("internal error %d:%s\n", mysqli_errno($link), mysqli_error($link)));
											exit();
										}
									}
								$dealer = mysqli_fetch_array($d_order);  echo $dealer['lastname'].", ".$dealer['firstname'];?>
							</div>
							<?php if($orders['scheduled']!="0000-00-00 00:00:00"){ ?>
								<div class="section_review_full">
								
									<strong>Appointment:</strong> <?php echo date("m/d/y g:i A",strtotime($orders['scheduled'])); ?>&nbsp;&nbsp;<a class="remove_rec" href="?task=cancel&order_id=<?Php echo $order_id; ?>"><svg xmlns="http://www.w3.org/2000/svg" class="section_icon_sg" viewBox="0 0 800 800"><path d="M400 0C178.8 0 0 178.8 0 400s178.8 400 400 400 400-178.8 400-400S621.2 0 400 0zm200 543.6L543.6 600 400 456.4 256.4 600 200 543.6 343.6 400 200 256.4l56.4-56.4L400 343.6 543.6 200l56.4 56.4L456.4 400 600 543.6z"/></svg>Cancel</a>
								</div>
							<?php } if ($arc!='y'){ if($orders['scheduled']=="0000-00-00 00:00:00"){ ?>
								<div class="section_review_full">
									<strong>Set Appointment</strong><br><br>
										<form action="schedule.php" method="post" name="sched" id="sched" >
											Date:&nbsp;&nbsp;&nbsp;&nbsp;
											<select name="month" class="drop_small" id="month">
                							<?php for($i=1; $i<13; $i++){ ?>
                								<option value="<?php printf("%02d", $i); ?>"  <?php if($i==$month) echo 'selected="selected"'; ?>><?php printf("%02d", $i); ?></option>
                							<?php } ?>
              								</select>
                							<select name="day" class="drop_small" id="day">
                  							<?php for($i=1; $i<32; $i++){ ?>
                  								<option value="<?php printf("%02d", $i); ?>"  <?php if($i==$day) echo 'selected="selected"'; ?>><?php printf("%02d", $i); ?></option>
                  							<?php } ?>
                							</select>
                							<select name="year" class="drop_small" id="year">
                								<option value="<?php echo date("Y") - 1; ?>" <?php if(date("Y") - 1 ==$year) echo 'selected="selected"'; ?>><?php echo date("Y") - 1; ?></option>
                  								<option value="<?php echo date("Y"); ?>" <?php if(date("Y")==$year) { echo 'selected="selected"'; }  if($year=="1969") { echo 'selected="selected"'; } ?>><?php echo date("Y"); ?></option>
                  								<option value="<?php echo date("Y") + 1; ?>" <?php if(date("Y") + 1 ==$year) echo 'selected="selected"'; ?>><?php echo date("Y") + 1; ?></option>
            								</select><br><br>
                							Time:&nbsp;&nbsp;&nbsp;&nbsp;
											<select name="hour" class="drop_small" id="hour">
                  							<?php for($i=1; $i<13; $i++){ ?>
                  								<option value="<?php printf("%02d", $i); ?>" <?php if($i==$hour) echo 'selected="selected"'; ?>><?php printf("%02d", $i); ?></option>
                  							<?php } ?>
                							</select>
                							<select name="min" class="drop_small" id="min">
                  							<?php for($i=0; $i<46; $i+=15){ ?>
                  								<option value="<?php printf("%02d", $i); ?>" <?php if($i==$min) echo 'selected="selected"'; ?>><?php printf("%02d", $i); ?></option>
                  							<?php } ?>
                							</select>
                							<select name="ampm" class="drop_small" id="ampm">
											...
											<input name="time_id" type="hidden" id="time_id" value="<?php echo $order_id; ?>" /><br>
											<input name="Submit" type="submit" id="Submit" value="Schedule" class="section_submit_button" />
                						</form>
								</div>							
							<?php } } if($orders['status']=="complete") { ?>
								<div class="section_review_full">
									<svg xmlns="http://www.w3.org/2000/svg" class="section_icon" viewBox="0 0 800 800"><path d="M0 697.8V0h800v697.8l-32-65.3-53-109-38.9-79.7c-2.9-6-7.3-9.7-14.4-9.6-34.1.1-68.2-.2-102.2.3-4.3.1-9.5 2.6-12.7 5.7-8.3 8-15.6 16.8-23.3 25.4l-17.8 20.2h8.5l111.2-.1c9.5-.1 15.5 3.7 19.6 12.2l68.4 140.6 10.6 22.2H76.6l12.1-25.2 66.4-136.6c4.4-9.2 10.6-13.3 20.9-13.2l110.5.1h8.1l-39-43.9c-5.1-5.9-11-7.9-18.5-7.8h-95.8c-8.6-.1-13.9 3.2-17.6 10.8L81 532.1l-47 96.4-34 69.3zm581-436.4c-.2-20.4-6.1-49.1-20.1-75.9-16.3-31.4-39.7-56.2-70.3-73.8-31.8-18.3-66.1-26.1-102.8-23.8-42.1 2.7-78.5 18.5-109 47-30.6 28.6-49.5 64-56.6 105.4-8.4 48.5 1 93.4 27.3 134.9 17.3 27.3 39.7 50.4 61.2 74.2 28.7 31.9 54.9 65.5 72.5 105 2 4.5 3.7 9.1 6.5 13.1 5.2 7.7 15.8 7.3 20.9-.3 1.4-2.1 2.5-4.4 3.5-6.8 11.7-27.8 27-53.6 46.2-76.8 21.2-25.5 43.4-50.1 65.1-75.2 14.5-16.8 27.3-34.8 37-54.8 12.5-26 18-53.7 18.6-92.2z" fill="#fff"/><path d="M0 697.8l34-69.3 47-96.4 42.3-87.3c3.7-7.7 9-10.9 17.6-10.8h95.8c7.5-.1 13.4 1.9 18.5 7.8 12.4 14.5 25.3 28.5 39 43.9h-8.1c-36.8 0-73.7.2-110.5-.1-10.3-.1-16.5 3.9-20.9 13.2L88.3 635.4l-12.1 25.2h647.4L713 638.4l-68.4-140.6c-4.1-8.5-10-12.3-19.6-12.2l-111.2.1h-8.5l17.8-20.2c7.7-8.6 15.1-17.4 23.3-25.4 3.2-3.1 8.4-5.6 12.7-5.7l102.2-.3c7.1 0 11.5 3.6 14.4 9.6l38.9 79.7 53 109 32 65.3v7.7c-4.4 6.8-11.1 7.5-18.5 7.5-157.3-.1-314.6 0-471.9 0H17.4c-7.2 0-13.6-1-17.8-7.5.4-2.5.4-5.1.4-7.6z"/><path d="M0 705.4c4.3 6.5 10.7 7.5 17.8 7.5h291.8 471.9c7.3 0 14-.7 18.5-7.5V800H0v-94.6z" fill="#fff"/><path d="M581 261.4c-.6 38.5-6 66.3-18.6 92.4-9.7 20-22.5 38-37 54.8l-65.1 75.2c-19.2 23.2-34.5 49-46.2 76.8-1 2.3-2.1 4.7-3.5 6.8-5.1 7.7-15.7 8-20.9.3-2.7-4-4.5-8.7-6.5-13.1-17.6-39.5-43.8-73.1-72.5-105-21.5-23.8-43.9-46.9-61.2-74.2-26.3-41.5-35.7-86.4-27.3-134.9 7.2-41.5 26-76.8 56.6-105.4 30.5-28.6 66.9-44.3 109-47 36.8-2.4 71.1 5.4 102.8 23.8 30.6 17.7 53.9 42.5 70.3 73.8 14 26.6 19.9 55.3 20.1 75.7zm-181.4-88.9c-52.7.2-95.7 43.5-95.5 96.2.2 52.3 43.5 95.4 95.8 95.3 52.8-.1 96.4-43.7 96-95.8-.5-53-43.7-95.9-96.3-95.7z"/><path d="M399.6 172.5c52.7-.2 95.8 42.7 96.2 95.7.4 52.2-43.2 95.7-96 95.8-52.2.1-95.6-43-95.8-95.3-.2-52.7 42.8-96 95.6-96.2z" fill="#fff"/></svg><strong>Tour Links</strong>
								</div>
								<div class="section_review_full">
								 	<a href="http://www.look2homes.com/l2fp/floorplan/index/<?php echo $order_id;?>" target="_blank">Branded Tour</a>&nbsp;&nbsp;&nbsp;|&nbsp;&nbsp;&nbsp;<a href="http://www.look2homes.com/l2fp/floorplan/unbranded/index/<?php echo $order_id;?>" target="_blank">Unbranded Tour</a>
								</div>
							<?php } ?>
						</div>
						<div class="section_half">
							...
							<?php 
								if ($arc!='y'){
									if(!($notes = mysqli_query($link,"SELECT * from notes where order_id = '$order_id' AND type = 'any' ORDER BY date DESC"))){
										printf("%s", sprintf("internal error %d:%s\n", mysqli_errno($link), mysqli_error($link)));
										exit();
	  								} 
								} else {
									if(!($notes = mysqli_query($link2,"SELECT * from notes where order_id = '$order_id' AND type = 'any' ORDER BY date DESC"))){
										printf("%s", sprintf("internal error %d:%s\n", mysqli_errno($link), mysqli_error($link)));
										exit();
									}
								}
								while($disp_notes=mysqli_fetch_array($notes)){ $entered_id = $disp_notes['user']; ?><?php echo date("m/d/Y",strtotime($disp_notes['date'])); ?> - 
							<?php 
								if ($arc!='y'){
									if(!($e_notes = mysqli_query($link,"SELECT firstname, lastname FROM users where id = '$entered_id'"))){
										printf("%s", sprintf("internal error %d:%s\n", mysqli_errno($link), mysqli_error($link)));
										exit();
									} 
								} else {
									if(!($e_notes = mysqli_query($link2,"SELECT firstname, lastname FROM users where id = '$entered_id'"))){
										printf("%s", sprintf("internal error %d:%s\n", mysqli_errno($link), mysqli_error($link)));
										exit();
									}
								}
								$entered = mysqli_fetch_array($e_notes);  echo $entered['lastname'].", ".$entered['firstname']; ?><br /><?php echo $disp_notes['content']; ?>
							<?php } ?><br><br>
							<form action="schedule.php" method="post" name="notes" id="notes" >
								<strong>Enter Note(s)</strong><br />
                 				<textarea name="note" cols="30" rows="3" id="note"></textarea>
								<input name="order_id" type="hidden" id="order_id" value="<?php echo $order_id; ?>" />
								<input name="type" type="hidden" id="type" value="any" /><br>
                  				<input name="Submitnote" type="submit" id="Submitnote" value="Save" class="section_submit_button" />
							</form>
						</div>
						<div class="section_half">
							<?php if ($arc!='y'){
								if(!($a_order = mysqli_query($link,"SELECT * FROM users where id = '$agent_id'"))){
									printf("%s", sprintf("internal error %d:%s\n", mysqli_errno($link), mysqli_error($link)));
									exit();
								} 
							} else {
								if(!($a_order = mysqli_query($link2,"SELECT * FROM users where id = '$agent_id'"))){
									printf("%s", sprintf("internal error %d:%s\n", mysqli_errno($link), mysqli_error($link)));
									exit();
								}
							}
							$agents = mysqli_fetch_array($a_order);
	
							$company_id = $agents['company_id'];
	
							if ($arc!='y'){
								if(!($c_order = mysqli_query($link,"SELECT name, city, state FROM companies where id = '$company_id'"))){
									printf("%s", sprintf("internal error %d:%s\n", mysqli_errno($link), mysqli_error($link)));
									exit();
								} 
							} else {
								if(!($c_order = mysqli_query($link2,"SELECT name, city, state FROM companies where id = '$company_id'"))){
									printf("%s", sprintf("internal error %d:%s\n", mysqli_errno($link), mysqli_error($link)));
									exit();
								}
							}
							$company = mysqli_fetch_array($c_order);
							?>
							<div class="section_review_half">
								<span class="agent_pic_order"><?php if(file_exists("floorplan/profile/".$agent_id.".jpg")){ ?><img src="floorplan/profile/<?php echo $agent_id; ?>.jpg" width="40px" alt="<?php echo $agents['firstname']." ". $agents['lastname']; ?>"><?php } ?></span><strong>Order For:</strong><br><?php echo $agents['lastname'].", ".$agents['firstname']; ?><br><a href="mailto:<?php echo $agents['email']; ?>"><?php echo $agents['email']; ?></a>
							</div>
							<div class="section_review_half">
								<strong>Company:</strong><br><?php echo $company['name']; ?>
							</div>
							<div class="clear"></div>
							<div class="section_review_half">
								<strong>Home Owner:</strong><br>
                 				Name: <?php echo $orders['owner']; ?><br>
                  				Email: <?php echo $orders['owner_email']; ?><br>
                  				Phone: <?php echo $orders['cell_phone1']; ?><br>
							</div>
							<div class="section_review_half">
								<strong>Access Info:</strong><br>
								Vacant: <?php echo $orders['vacant']; ?><br>
								Pet: <?php echo $orders['pets']; ?><br>
								Lock Box: <?php echo $orders['lockbox']; ?><br>
							</div>
							<div class="clear"></div>
							<div class="section_review_full">
								<strong>Special Instructions: </strong><br>
								<?php echo $orders['instruct']; ?>
							</div>
						</div>
						<div class="clear"></div>
					</div>
