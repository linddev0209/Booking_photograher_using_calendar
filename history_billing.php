...
$totpage = 30; // total records per page

if($a_id!="") {
	if(!($agent_info = mysqli_query($link,"SELECT firstname, lastname FROM users where id = '$a_id'"))){
		printf("%s", sprintf("internal error %d:%s\n", mysqli_errno($link), mysqli_error($link)));
		exit();
	}
	$agent_profile = mysqli_fetch_array($agent_info);

	if(!($bh_db = mysqli_query($link,"SELECT orders.id, orders.street_num, orders.street_dir, orders.street_name, orders.unit, orders.city, orders.state, transactions.* FROM orders,transactions WHERE orders.agent_id = '$a_id' AND orders.id=transactions.order_id AND transactions.res_ad='A' ORDER BY transactions.date_trans DESC LIMIT $offset, $totpage"))){
		printf("%s", sprintf("internal error %d:%s\n", mysqli_errno($link), mysqli_error($link)));
		exit();
	}
	if(!($bh_count = mysqli_query($link,"SELECT orders.id, orders.street_num, orders.street_dir, orders.street_name, orders.unit, orders.city, orders.state, transactions.* FROM orders,transactions WHERE orders.agent_id='$a_id' AND orders.id=transactions.order_id AND transactions.res_ad='A'"))){
		printf("%s", sprintf("internal error %d:%s\n", mysqli_errno($link), mysqli_error($link)));
		exit();
	}
	$bh_records = mysqli_num_rows($bh_count);
}
...
if ($c_id!="") {
	...
	if(!($bh_db = mysqli_query($link,"SELECT orders.id, orders.street_num, orders.street_dir, orders.street_name, orders.unit, orders.city, orders.state, transactions.* FROM orders,transactions WHERE orders.company_id='$c_id' AND orders.id=transactions.order_id AND transactions.res_ad='A' ORDER BY transactions.date_trans DESC LIMIT $offset, $totpage"))){
		printf("%s", sprintf("internal error %d:%s\n", mysqli_errno($link), mysqli_error($link)));
		exit();
	}
	if(!($bh_count = mysqli_query($link,"SELECT orders.id, orders.street_num, orders.street_dir, orders.street_name, orders.unit, orders.city, orders.state, transactions.* FROM orders,transactions WHERE orders.company_id='$c_id' AND orders.id=transactions.order_id AND transactions.res_ad='A'"))){
		printf("%s", sprintf("internal error %d:%s\n", mysqli_errno($link), mysqli_error($link)));
		exit();
	}
	$bh_records = mysqli_num_rows($bh_count);
...
<html>
	<head>
		...
		<title>Billing History | Look2 Home Marketing</title>
	...
	<div id="container">
		...
		<div id="right_column">
			...
			<div id="rc_content_container">
				<?php 
					if ($_SESSION['SESS_LEVEL']=="admin" || $_SESSION['SESS_LEVEL']=="staff" || $_SESSION['SESS_LEVEL']=="dealer") {
						if($c_id!="") { ?>
				...
				<div class="section_container">
					<div class="section_header">
						<svg xmlns="http://www.w3.org/2000/svg" class="section_icon" xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="0 0 512.001 512.001"><path d="M448.875 1.78l-59.314 29.657L330.247 1.78c-4.7-2.35-10.232-2.35-14.932 0L256 31.437 196.686 1.78c-4.7-2.35-10.232-2.35-14.932 0L122.44 31.437 63.125 1.78C52.05-3.758 38.964 4.304 38.964 16.713v445.202c0 6.324 3.573 12.104 9.229 14.933l66.78 33.39c4.7 2.35 10.232 2.35 14.932 0l59.314-29.657 59.314 29.657c4.7 2.35 10.232 2.35 14.932 0l59.314-29.657 59.314 29.657c4.7 2.35 10.232 2.35 14.932 0l66.78-33.39c5.656-2.828 9.229-8.609 9.229-14.933V16.713c.002-12.383-13.06-20.481-24.159-14.933zm-9.229 393.355v56.462l-50.085 25.043-59.314-29.657c-2.35-1.175-4.908-1.762-7.466-1.762a16.7 16.7 0 0 0-7.466 1.762L256 476.639l-59.314-29.657c-4.7-2.35-10.232-2.35-14.932 0l-59.314 29.657-50.085-25.043v-56.462V43.726l42.619 21.31c4.7 2.35 10.232 2.35 14.932 0l59.314-29.657 59.314 29.657c4.7 2.35 10.232 2.35 14.932 0l59.314-29.657 59.314 29.657c4.7 2.35 10.232 2.35 14.932 0l42.619-21.31v351.409z"/><use xlink:href="#B"/><use xlink:href="#B" y="66.78"/><use xlink:href="#B" y="133.56"/><path d="M211.48 300.53h-89.04a16.7 16.7 0 0 0-16.695 16.695 16.7 16.7 0 0 0 16.695 16.695h89.04a16.7 16.7 0 0 0 16.695-16.695c0-9.221-7.475-16.695-16.695-16.695zm-89.04 77.91H89.05a16.7 16.7 0 0 0-16.695 16.695A16.7 16.7 0 0 0 89.05 411.83h33.39a16.7 16.7 0 0 0 16.695-16.695 16.7 16.7 0 0 0-16.695-16.695z"/><use xlink:href="#C"/><use xlink:href="#C" x="100.171"/><use xlink:href="#C" x="200.341"/><path d="M352.843 191.165c-17.515-9.258-35.627-18.832-35.627-29.761 0-15.343 12.482-27.825 27.825-27.825s27.825 12.482 27.825 27.825a16.7 16.7 0 0 0 16.695 16.695 16.7 16.7 0 0 0 16.695-16.695c0-27.966-18.858-51.594-44.52-58.882v-7.898a16.7 16.7 0 0 0-16.695-16.695 16.7 16.7 0 0 0-16.695 16.695v7.898c-25.663 7.287-44.52 30.916-44.52 58.882 0 31.046 29.616 46.701 53.413 59.28 17.515 9.258 35.627 18.832 35.627 29.761 0 15.343-12.482 27.825-27.825 27.825s-27.825-12.482-27.825-27.825a16.7 16.7 0 0 0-16.695-16.695 16.7 16.7 0 0 0-16.695 16.695c0 27.966 18.858 51.594 44.52 58.882v7.898a16.7 16.7 0 0 0 16.695 16.695 16.7 16.7 0 0 0 16.695-16.695v-7.898c25.663-7.287 44.52-30.916 44.52-58.882 0-31.047-29.616-46.701-53.413-59.28z"/><defs ><path id="B" d="M211.48 100.189h-89.04a16.7 16.7 0 0 0-16.695 16.695 16.7 16.7 0 0 0 16.695 16.695h89.04a16.7 16.7 0 0 0 16.695-16.695 16.7 16.7 0 0 0-16.695-16.695z"/><path id="C" d="M222.61 378.44h-33.39a16.7 16.7 0 0 0-16.695 16.695 16.7 16.7 0 0 0 16.695 16.695h33.39a16.7 16.7 0 0 0 16.695-16.695 16.7 16.7 0 0 0-16.695-16.695z"/></defs></svg><h1>Billing History - <?php if($c_id!="") { echo $company_profile['name']; } if($a_id!="") { echo $agent_profile['firstname'].' '.$agent_profile['lastname']; }?></h1>
					</div>
					...
					<?php while($bh_detail=mysqli_fetch_array($bh_db)) { ?>
						<div class="section_result_row">
							<div class="section_form_17">
								<?php echo date("m/d/Y",strtotime($bh_detail['date_trans'])); ?>
							</div>
							<div class="section_form_25">
								<?php if($bh_detail['street_num']!="0") { echo $bh_detail['street_num']; } ?> <?php echo $bh_detail['street_dir']." ".$bh_detail['street_name']." ".$bh_detail['unit']; ?><br /><?php echo $bh_detail['city'].", ".$bh_detail['state']; ?>
							</div>
							<div class="section_form_14">
								<?php echo $bh_detail['order_id']; ?>
							</div>
							<div class="section_form_24">
								<strong>Total:</strong> <?php echo $bh_detail['total_trans']; ?><br>
								<?php if($bh_detail['ifp']!="0.00") { ?>IFP: <?php echo $bh_detail['ifp']; ?><br><?php } if($bh_detail['hdr_up']!="0.00") { ?>HDR Photo Upgrade: <?php echo $bh_detail['hdr_up']; ?><br><?php } if($bh_detail['hdr_only']!="0.00") { ?>HDR Photos Only: <?php echo $bh_detail['hdr_only']; ?><br><?php } if($bh_detail['spg_only']!="0.00") { ?>Standard Photos Only: <?php echo $bh_detail['spg_only']; ?><br><?php } if($bh_detail['tlight_up']!="0.00") { ?>Twilight Photos: <?php echo $bh_detail['tlight_up']; ?><br><?php } if($bh_detail['aphotos']!="0.00") { ?>Aerial Photos: <?php echo $bh_detail['aphotos']; ?><br> <?php } if($bh_detail['real_com']!="0.00") { ?>Add to Realtor.com: <?php echo $bh_detail['real_com']; ?><br><?php } if($bh_detail['addy_url']!="0.00") { ?>Address URL: <?php echo $bh_detail['addy_url']; ?><br><?php } if($bh_detail['vid']!="0.00") { ?>Walk Through Video: <?php echo $bh_detail['vid']; ?><br><?php } if($bh_detail['cvid']!="0.00") { ?>Cinemaic Video: <?php echo $bh_detail['cvid']; ?><br><?php } if($bh_detail['3d']!="0.00") { ?>Zillow 3D Tour: <?php echo $bh_detail['3d']; ?><br><?php } if($bh_detail['avid']!="0.00") { ?>Aerial Video: <?php echo $bh_detail['avid']; ?><br><?php } if($bh_detail['vid_o']!="0.00") { ?>Video Only: <?php echo $bh_detail['vid_o']; ?><br><?php } if($bh_detail['draw_only']!="0.00") { ?>Drawing Only: <?php echo $bh_detail['draw_only']; ?><br><?php } if($bh_detail['rebuild']!="0.00") { ?>Rebuild Fee: <?php echo $bh_detail['rebuild']; ?><br><?php } if($bh_detail['retake']!="0.00") { ?>Photo Retake Fee: <?php echo $bh_detail['retake']; ?><br><?php } if($bh_detail['canc_fee']!="0.00") { ?>Cancellation Fee: <?php echo $bh_detail['canc_fee']; ?><br><?php } if($bh_detail['trip_fee']!="0.00") { ?>Trip Fee: <?php echo $bh_detail['trip_fee']; ?><br><?php } if($bh_detail['ae']!="0.00") { ?>Advanced Editing: <?php echo $bh_detail['ae']; ?><br><?php } if($bh_detail['misc']!="0.00") { ?>Miscellaneous: <?php echo $bh_detail['misc']; ?><br><?php } if($bh_detail['rush_fee']!="0.00") { ?>Rush Fee: <?php echo $bh_detail['rush_fee']; ?><br><?php } if($bh_detail['sign']!="0.00") { ?>Sign: <?php echo $bh_detail['sign']; ?><br><?php }if($bh_detail['post_only']!="0.00") { ?>Post Only: <?php echo $bh_detail['post_only']; ?><br><?php }if($bh_detail['lighted']!="0.00") { ?>Lighted Sign Post: <?php echo $bh_detail['lighted']; ?><br><?php }if($bh_detail['sr']!="0.00") { ?>Sign Rider: <?php echo $bh_detail['sr']; ?><br><?php } if($bh_detail['qr']!="0.00") { ?>QR Sticker: <?php echo $bh_detail['qr']; ?><br><?php } if($bh_detail['qrgen']!="0.00") { ?>QR Code Generation: <?php echo $bh_detail['qrgen']; ?><br><?php } ?>
							</div>
							<div class="section_form_20">
								<?php echo $bh_detail['trans_id']; ?>
							</div>
							<div class="clear"></div>						
						</div>
					<?php } ?>
					<div class="section_footer">
						<div class="section_navigate">
							<?php 
								if($c_id!="") { 
									$crumbext="company=".$c_id; 
								} if($a_id!="") { 
									$crumbext="agent=".$a_id;
								}
								$prev = $offset-$totpage;
								if ($prev >= 0) $nav.="<a href='".$PHP_SELF."?offset=".$prev."&".$crumbext."'><strong>&lt;&lt;&nbsp;Prev</strong></a> | "; 
								if($bh_records!="0") {
									$nav.="Page";
								}
								if($bh_records/$totpage < 10) {
									$crumbmax = $bh_records/$totpage;
								} else {
									$crumbmax = 10; 
								}
								$bottom = 0;
								...
								$top = $bottom+$crumbmax;
								if(ceil($bh_records/$totpage) < ($offset/$totpage) + $crumbmax/2)	$top = ceil($bh_records/$totpage);
								for($i=$bottom; $i<$top; $i++) {
									if($offset/$totpage==$i) {
										$nav.=" <strong>".($i+1)."</strong>"; 
									} else {  
										$nav.=" <a href='".$PHP_SELF."?offset=".($i)*$totpage."&".$crumbext."'>".($i+1)."</a>"; 
	  								} 
								} 
    							$next = $offset+$totpage;
								if ($next < ($bh_records))	$nav.=" | <a href='".$PHP_SELF."?offset=".$next."&".$crumbext."'><b>Next&nbsp;&gt;&gt;</b></a>";
								echo $nav; 
							?>&nbsp;
						</div>
						<div class="clear"></div>
