...
$host = "beta.look2homes.com"; // Sub domain
...
if($_POST['Submit']){
	...
	$login = filter_var($_POST['e_addy'], FILTER_VALIDATE_EMAIL);
	$password = filter_var($_POST['password'], FILTER_SANITIZE_STRING);
	
	if (filter_var($login, FILTER_VALIDATE_EMAIL)) {
		//Input Validations
		if($login == '') {
			header("location: index.php?flag=nouser");
			exit();
		}
		if ($login === false ) {
			header("location: index.php?flag=nouser");
		...
		if($password == '') {
			header("location: index.php?flag=nopass");
			exit();
		}
	
		$passencrypt = SHA1($password);

		if(!($result = mysqli_query($link,"SELECT * FROM users WHERE email = '$login' AND pass = '$passencrypt'"))){
		...
		if($result) {
			if(mysqli_num_rows($result) == 1) {
				...
				if($_POST['rememberme']) {
					...
					setcookie('MEMBER_ID', site_crypt($member['id'], 'e' ), time() + 30*24*60*60);
				setcookie('FIRST_NAME', site_crypt($member['firstname'], 'e' ), time() + 30*24*60*60);
				setcookie('LAST_NAME', site_crypt($member['lastname'], 'e' ), time() + 30*24*60*60);
				setcookie('LEVEL', site_crypt($member['level'], 'e' ), time() + 30*24*60*60);
        	} else {
				...
				//Login failed
				header("location: index.php?flag=badlogin");
				exit();
			}
		} else {
			header("location: index.php?flag=noconnect");
			exit();
		} 
	} else {
		header("location: index.php?flag=invalidemail");
		exit();
	}
}
...
<html>
	<head>
		...
		<title>Log In | Look2 Home Marketing</title>
...
<body>
	<div id="login_container">
		...
		<div class="section_container">
			...
			<?php if($_GET['flag'] == "nouser" || $_GET['flag'] == "invalidemail"){ ?>
            ...
            <?php } if($_GET['flag'] == "nopass"){ ?>
				<div class="section_error_row">
					<div class="section_full">
						<?php echo "<strong>A valid password is required to sign in. Please enter your password below.</strong>"; ?>
					</div>
				</div>
            <?php } if($_GET['flag'] == "badlogin"){ ?>
				<div class="section_error_row">
					<div class="section_full">
						<?php echo "<strong>Invalid Username and/or Password. If you continue to have trouble logging in please Look2 Home Marketing.</strong>"; ?>
					</div>
				</div>
            <?php } if($_GET['flag'] == "failedauth"){ ?>
				<div class="section_error_row">
					<div class="section_full">
						<?php echo "<strong>Something went wrong, please sign in again.</strong>"; ?>
					</div>
				</div>
            <?php } if($_GET['flag'] == "timed"){ ?>
				<div class="section_error_row">
					<div class="section_full">
						<?php echo "<strong>Your session timed out. Please log in again.</strong>"; ?>
					</div>
				</div>
            <?php } if($_GET['flag'] == "signout"){ ?>
				<div class="section_flag_row">
					<div class="section_full">
						<?php echo "<strong>You have been successfully signed out!</strong>"; ?>
					</div>
				</div>
             <?php } if($_GET['flag'] == "reset_complete"){ ?>
				<div class="section_flag_row">
					<div class="section_full">
						<?php echo "<strong>Your password has been successfully updated. You can log in below.</strong>"; ?>
					</div>
				</div>
            <?php } ?>
			...
			<form id="form1" name="form1" method="post" action="">
				<div class="section_result_row">
					...
					<div class="section_form_50"><strong>EMAIL</strong></div>
					<div class="clear"></div>
					<div class="section_form_25">&nbsp;</div>
					<div class="section_form_50"><input name="e_addy" id="e_addy" type="text" size="32" /></div>
					<div class="clear"></div>
					<div class="section_form_25">&nbsp;</div>
					<div class="section_form_50"><strong>PASSWORD</strong></div>
					<div class="clear"></div>
					<div class="section_form_25">&nbsp;</div>
					<div class="section_form_50"><input name="password" id="password" type="password" size="32"/></div>
					<div class="clear"></div>
					<div class="section_form_25">&nbsp;</div>
					<div class="section_form_50">Remember Me: <input type="checkbox" name="rememberme" value="1"></div>
					<div class="clear"></div>
					<div class="section_form_25">&nbsp;</div>
					<div class="section_form_50"><input name="Submit" type="submit" value="Sign In" class="section_submit_button_md" /></div>
					<div class="clear"></div>
					<div class="section_form_25">&nbsp;</div>
					<div class="section_form_50">&nbsp;</div>
					<div class="clear"></div>
					<div class="section_form_25">&nbsp;</div>
					<div class="section_form_50"><a href="reset_pwd.php" style="text-decoration: underline;">Forgot Password</a></div>
					<div class="clear"></div>
				</div>
			</form>
		</div>
