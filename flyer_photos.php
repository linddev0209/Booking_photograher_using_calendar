...
if($_POST['Submit2']){
	...
	while($photo_sort=mysqli_fetch_array($up_sort)){
		...
		$sort = $_POST['sort'.$pid];
		if($sort!=""){
			if(!($rec_record = mysqli_query($link,"UPDATE photos set flyer_sort = '$sort' WHERE id = '$pid'"))){
				printf("%s", sprintf("internal error %d:%s\n", mysqli_errno($link), mysqli_error($link)));
				exit();
			}
		}
	}
	header("Location: flyer_photos.php?order_id=".$order_sort."&flag=complete");
	exit();
}
?>
<!doctype html>
<html>
	<head>
		...
		<title>Update Flyer Photos | Look2 Home Marketing</title>
		...
		<div id="right_column">
			...
			<div id="rc_content_container">
				<div class="section_full_ra">
					<a href="order_detail.php?order_id=<?php echo $order_id; ?>">Back to Order Details</a>
				</div>
				<div class="section_container">
					...
					<div class="section_full">
						<div class="section_result_row">
							<div class="section_review_half">
								<?php if($flag=="complete") { ?>
                					<div class="section_full"><strong>Fyler Updated!</strong></div>
              					<?php } ?>
								<div class="section_full">
									<strong><?php if($orders['street_num']!="0") { echo $orders['street_num']; } ?><?php if($orders['street_dir']!="") { echo $orders['street_dir']; } ?> <?php echo $orders['street_name']; ?> <?php if($orders['unit']!="") { echo $orders['unit']; } ?> - <?php echo $orders['city'].", ".$orders['state']." ".$orders['zip']; ?></strong>
								</div>
								<div class="section_full">
									<strong>Instructions:</strong>
								</div>
								<div class="section_full">
									Put the number in the box next to the photo for the corresponding area that you would like to have the photo show on the flyer. After putting the numbers in the boxes next to the photos, click the button at the bottom &quot;Update Flyer Photo(s)&quot;. Once submitted the changes are applied immediately.
								</div>
								<div class="section_full">
									If you wish to change where photos are at anytime simply change the sort numbers below according to the instructions above. There is no limit to the amount of times you rearrange the photos.
								</div>
								<div class="section_full">
									You can view the flyers for these properties by clicking on the link below. Please note that the Flyer will only render correctly if your order is completed.
								</div>
								<div class="section_full">
									<?php if($orders['status']=="complete") { ?>
										<a href="https://tours.look2homes.com/flyer/<?php echo $order_id;?>" target="_blank">Download Flyer</a
									<?php  } ?>
								</div>
							</div>
						</div>
						<div class="section_review_half_ca">
							<img src="images/flyer_diagram.jpg" width="256" height="300" alt="Flyer Picture Diagram" />
						</div>
						<div class="clear"></div>
					</div>
					...
					<div class="section_result_row">
						<form name="form1" id="form1" method="post" action="flyer_photos.php" >
							<?php if(!($e_photo = mysqli_query($link,"SELECT * FROM photos where order_id = '$order_id' ORDER BY fp_id, sort ASC"))){
								printf("%s", sprintf("internal error %d:%s\n", mysqli_errno($link), mysqli_error($link)));
								exit();
							}
							for($i=0; $i<mysqli_num_rows($e_photo)/2; $i++){ ?>
								<div class="section_form_36">
									<?php $list_photo = mysqli_fetch_array($e_photo); $fp_assigned = $list_photo['fp_id'];?><img src="includes/rezor.php?src=floorplan/<?php echo $order_id; ?>/photos/<?php echo $order_id; ?>_<?php echo $list_photo['fp_id']; ?>_<?php echo $list_photo['id']; ?>.jpg&amp;w=280&amp;zc=1&amp;q=90" alt="Order#<?php echo $order_id; ?>" />
								</div>
								<div class="section_form_14">
									Sort: <input name="sort<?php echo $list_photo['id']; ?>" type="text" id="sort<?php echo $list_photo['id']; ?>" value="<?php echo $list_photo['flyer_sort']; ?>" size="4" />
								</div>
								<?php $list_photo = mysqli_fetch_array($e_photo); if($list_photo!="") { $fp_assigned = $list_photo['fp_id']; ?>
									<div class="section_form_36">
										<img src="includes/rezor.php?src=floorplan/<?php echo $order_id; ?>/photos/<?php echo $order_id; ?>_<?php echo $list_photo['fp_id']; ?>_<?php echo $list_photo['id']; ?>.jpg&amp;w=280&amp;zc=1&amp;q=90" alt="Order#<?php echo $order_id; ?>" />
									</div>
									<div class="section_form_14">
										Sort: <input name="sort<?php echo $list_photo['id']; ?>" type="text" id="sort<?php echo $list_photo['id']; ?>" value="<?php echo $list_photo['flyer_sort']; ?>" size="4" />
									</div>
							...
							<div class="section_footer">
								<div class="section_full_ra">
									<input name="order_id" type="hidden" id="order_id" value="<?php echo $order_id; ?>" /><input class="section_submit_button_md" name="Submit2" type="submit" id="Submit2" value="Update Flyer Photo(s)" />
								</div>
							</div>
						</form>
					</div>
