...
if($_POST['Submit']) {
	if(isset($_POST["email"]) && (!empty($_POST["email"]))){
		...
		if (!$email) {
   			header("location: reset_pwd.php?flag=invalidemail");
			exit();
   		}else{
			if(!($reset_result = mysqli_query($link,"SELECT * FROM users WHERE email = '".$email."'"))){
			printf("%s", sprintf("internal error %d:%s\n", mysqli_errno($link), mysqli_error($link)));
			exit();
		}
   			$loginrows = mysqli_num_rows($reset_result);
   			if ($loginrows==""){
   				header("location: reset_pwd.php?flag=nouser");
			exit();
   			}
  		}
   		$exp_format = mktime(
   		...
   		$hashed_key = $key . $addKey;
		$loginresults = mysqli_fetch_array($reset_result);
		$u_id = $loginresults['id'];
		// Insert into Temp Table
		if(!($dba = mysqli_query($link,"INSERT INTO password_reset_temp SET email = '$email', reset_key='$hashed_key', exp_date='$exp_date', user_id='$u_id'"))){
			printf("%s", sprintf("internal error %d:%s\n", mysqli_errno($link), mysqli_error($link)));
			exit();
		}
	
		$message_body = $loginresults['firstname']." ".$loginresults['lastname'].",";
		...
		To reset your password please click the link below  or copy and paste the URL into your browser.
		<a href="https://manager.look2homes.com/reset_password.php?key='.$hashed_key.'&email='.$email.'&action=reset">https://manager.look2homes.com/reset_password.php?key='.$hashed_key.'&email='.$email.'&action=reset</a>

		';
	
		$message_body.='The link will expire after 1 day for security.

		';
	
		$message_body.= "

		For questions or support please contact us at (803) 847-0006
		...
		";
	 	...
	 	$html_data = '<html><head><title>Reset Password - Look2 Home Marketing</title><style>.table td, .table th {font-size: 12px;}</style></head><body bgcolor="#FFF" style="background: #FFF top center repeat-x; margin: 0px; padding: 0px 12px 6px 12px; font-size:12px; FONT-FAMILY: Arial, Helvetica, sans-serif; "><table width="100%" cellspacing="0" cellpadding="0"><tbody><tr><td><table width="700" align="center" cellspacing="0" cellpadding="0"><tr><td height="118" valign="top" cellpadding="0" align="center"><img src="http://manager.look2homes.com/html_email/images/logo_notrans.jpg" width="197" height="118" alt="Look2 Home Marketing"></td></tr><tr><td ><div style="width: 100%; font-size:12px; padding: 0px; margin: 0px;border-radius: 6px; background: #f2f2f2;overflow: hidden;"><table width="700" align="center" cellpadding="0" cellspacing="0" bgcolor="#F2F2F2"  ><tr><td style="background-color: #666; padding: 6px; color:#fff;"><strong style="font-size:16px;"">Reset Password</strong></td></tr></table><table align="center" cellpadding="0" cellspacing="0" bgcolor="#F2F2F2"  ><tr><td align="left"><div style="background-color: #fff; border-radius: 6px; margin:6px; padding: 6px;">'.$loginresults['firstname'].' '.$loginresults['lastname'].',<br><br>To reset your password please click the link below <br><table><tr><td width="113"><a style="color: #FFF; width: 113px;" href="https://manager.look2homes.com/reset_password.php?key='.$hashed_key.'&email='.$email.'&action=reset"><div style="width: 113px; text-align:center; background-color: #0085b2; border-radius: 6px; margin:6px 6px 6px 0px; padding: 6px; text-decoration: none; font-size:12px; color:#FFF;">Reset Password</div></a></td></tr></table><br>Or copy and paste the URL into your browser.<br><br><a style="COLOR: #0085b2;" href="https://manager.look2homes.com/reset_password.php?key='.$hashed_key.'&email='.$email.'&action=reset">https://manager.look2homes.com/reset_password.php?key='.$hashed_key.'&email='.$email.'&action=reset</a><br><br>The link will expire after 1 day for security.<br><br>For questions or support please contact us at <strong>(803) 847-0006</strong></div></td></tr></table></div></td></tr><tr><td style="FONT-SIZE: 12px;" ><br><strong>Look2 Home Marketing</strong><br><br><a style="COLOR: #333" href="http://www.look2homes.com">http://www.look2homes.com</a><br><a style="COLOR: #333" href="mailto: orders@look2homes.com">orders@look2homes.com</a></td></tr></table></td></tr></tbody></table></body></html>';
		
		$mailsubject = "Reset your Look2 Home Marketing Password";
		
		SendEmail("orders@look2homes.com", "Look2 Home Marketing", $email, $loginresults['firstname']." ".$loginresults['lastname'], $mailsubject, nl2br($message_body), $html_data);
			
		header("Location: reset_pwd.php?flag=complete");
		exit();	
	}
}
...
<body>
	<div id="login_container">
		...
		<div class="section_container">
			...
			<?php if($_GET['flag'] == "invalidemail"){ ?>
           		<div class="section_error_row">
					<div class="section_full">
						<?php echo "<strong>A valid email address is required to sign in. Please enter your email address below.</strong>"; ?>
					</div>
				</div>
            <?php } if($_GET['flag'] == "nouser"){ ?>
				<div class="section_error_row">
					<div class="section_full">
						<?php echo "<strong>An account associated to that email address was not found.</strong>"; ?>
					</div>
				</div>
			 <?php } if($_GET['flag'] == "norecord"){ ?>
				<div class="section_error_row">
					<div class="section_full">
						<?php echo "<strong>Could not find reset record. Please try again below.</strong>"; ?>
					</div>
				</div>
			<?php } if($_GET['flag'] == "expired"){ ?>
				<div class="section_error_row">
					<div class="section_full">
						<?php echo "<strong>Reset password link expired. Reset links expire in 1 hour</strong>"; ?>
					</div>
				</div>
            <?php } if($_GET['flag'] == "complete"){ ?>
				<div class="section_flag_row">
					<div class="section_full">
						<?php echo "<strong>Email Sent!</strong>"; ?>
					</div>
				</div>
            <?php } ?>
			...
			<form id="reset" name="reset" method="post" action="">
				<div class="section_result_row">
					...
					<div class="section_form_50">Enter the email address associated with your account and we'll send you a link to reset your password.</div>
					<div class="clear"></div>
					<div class="section_form_25">&nbsp;</div>
					<div class="section_form_50"><input name="email" id="email" type="text" size="32" placeholder="username@email.com"/></div>
					<div class="clear"></div>
					<div class="section_form_25">&nbsp;</div>
