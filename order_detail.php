...
if(!($agent_db = mysqli_query($link,"select users.*, companies.name as c_name, companies.id as c_id, companies.city as c_city, companies.state as c_state from users, companies where users.id = '$agent_id' AND companies.id = users.company_id"))){
	printf("%s", sprintf("internal error %d:%s\n", mysqli_errno($link), mysqli_error($link)));
	exit();
}
$agents = mysqli_fetch_array($agent_db);

$company_id = $agents['c_id'];

if($_POST['SubmitPU']){
	$order_id = $_POST['order_id'];
	$price = $_POST['list_price'];
	
	if(!($update_order = mysqli_query($link,"UPDATE orders SET price = '$price' WHERE id = '$order_id'"))){
				printf("%s", sprintf("internal error %d:%s\n", mysqli_errno($link), mysqli_error($link)));
				exit();
		}
		
	header("Location: order_detail.php?order_id=".$order_id.'&flag=pu');
	exit();
}
...
if($_POST['SubmitBC']){
	...
	if(!($bc_update = mysqli_query($link,"UPDATE order_data SET beds='$beds', offices='$offices', baths='$baths', h_bath='$h_baths' WHERE order_id = '$order_id'"))){
			printf("%s", sprintf("internal error %d:%s\n", mysqli_errno($link), mysqli_error($link)));
			exit();
	}
	
	header("Location: order_detail.php?order_id=".$order_id.'&flag=bc');
	exit();
}
...
if($_POST['SubmitMO']){
	...
	$move_agent = $_POST['move_agent'];
	
	if ($move_agent=="") {
		header("Location: order_detail.php?order_id=".$order_id.'&flag=no_agent');
		exit();
	} else {
	
		if(!($mo_update = mysqli_query($link,"UPDATE orders SET company_id='$comp_id', agent_id='$move_agent' WHERE id = '$order_id'"))){
			printf("%s", sprintf("internal error %d:%s\n", mysqli_errno($link), mysqli_error($link)));
			exit();
		}
		
		header("Location: order_detail.php?order_id=".$order_id.'&flag=mo');
		exit();
	}
}
...
<html>
	<head>
		...
		<title>Order Details | Look2 Home Marketing</title>
		...
		<script>
      		...
      		function initMap() {
        		...
        		var maps = document.getElementsByClassName("maps_detail");
		...
		<script type="text/javascript">
			function loadSubCats(value) {
  				$.post('load_agents.php',{comp_id : value},function(data) {
...
<body>
			...
			<div id="rc_content_container">
				<div class="section_container">
					...
					<div class="section_result_row">
						<?php 
							if(!($e_photo = mysqli_query($link,"SELECT * FROM photos where order_id = '$order_id' ORDER BY fp_id, sort ASC LIMIT 1"))){
								printf("%s", sprintf("internal error %d:%s\n", mysqli_errno($link), mysqli_error($link)));
								exit();
							}
							$list_photo = mysqli_fetch_array($e_photo); 
							$fp_assigned = $list_photo['fp_id']; 
						
							if($orders['street_num']=="0") {
    							$map_num = ""; 
							} else { 
    							$map_num = $orders['street_num']." "; 
							}

							if($orders['city']=="") {
								$order_city = "";
							} else {
								$order_city = $orders['city'];
								$order_city_display = $orders['city'].",";
							}
							...
							if($orders['zip']=="0") {
								...
								$order_zip = $orders['zip'];
							}
							
							$map_address = "<b>".$map_num."".$orders['street_name']."</b><br>".$order_city.", ".$order_state." ".$order_zip;

							if($orders['longitude']!="0") {
    							$lat= $orders['latitude']; 
    							$long = $orders['longitude'];
    						} else { 
       							// google map geocode api url
        						$url = "https://maps.googleapis.com/maps/api/geocode/json?sensor=false&address=".urlencode($map_address)."&key=AIzaSyB4VVnzzUmNik_SQwj80agUr6r1f6yNycY"; 
						...
						?>
						<div class="section_detail_pic">
							<div class="section_order_header_address">
								<h1>
									<?php if($orders['street_num']!="0") { echo $orders['street_num']; } ?> <?php if($orders['street_dir']!="") { echo $orders['street_dir']; } ?> <?php echo $orders['street_name']; ?>&nbsp;<?php if($orders['unit']!="") { echo $orders['unit']; } ?>
								</h1><?php echo $orders['city']; ?>, <?php echo $orders['state']; ?> <?php echo $orders['zip']; ?>
							</div>
							<?php if(file_exists("floorplan/".$order_id."/photos/".$order_id."_".$fp_assigned."_".$list_photo['id'].".jpg")) { ?><img class="history_order" src="includes/rezor.php?src=floorplan/<?php echo $order_id; ?>/photos/<?php echo $order_id; ?>_<?php echo $fp_assigned; ?>_<?php echo $list_photo['id']; ?>.jpg&amp;w=800&amp;zc=1&amp;q=90" alt="Order#<?php echo $order_id; ?>" /><?php } else { ?><div id="maps<?php echo $order_id; ?>" class="maps_detail" data-gps="<?php echo $lat; ?>,<?php echo $long; ?>"></div><?php } ?>
							<div class="section_order_header_status" <?php if($orders['status']=="complete") { echo'style="background: #0085B2;"'; } if($orders['status']=="received") { echo'style="background: #7DB366;"'; } if($orders['status']=="scheduled") { echo'style="background: #7d0052;"'; } if($orders['status']=="pending") { echo'style="background: #b29300;"'; } if($orders['status']=="archived") { echo'style="background: #333;"'; } if($orders['status']=="canceled") { echo'style="background: #cd232b;"'; } if($orders['status']=="expired") { echo'style="background: #666633;"'; } ?>>
								<?php echo ucfirst($orders['status']); ?>
							</div>
						</div>
						<div class="section_detail_links">
							<div class="section_review_full">
								 <strong>Ordered:</strong>&nbsp;<?php if($orders['fp']=="y") echo "Interactive FloorPlan"; if($orders['draw_only']=="y") echo ", Drawing Only Floor Plan"; if($orders['hdr_photos']=="y") echo " - HDR Photos"; if($orders['hdr_only']=="y") echo " - HDR Photos Only"; if($orders['twilight']=="y") echo " - Twlight Photos Upgrade"; if($orders['aerial_pics']=="y") echo " - Aerial Photos Upgrade"; if($orders['cine_vid']=="y") echo " - Cinematic Video"; if($orders['3d']=="y") echo " - Zillow 3D Tour"; if($orders['atc']=="y") echo " - Aerial/Twilight Photo Package"; if($orders['avid_o']=="y") echo " - Aerial Video Only"; if($orders['avid']=="y") echo " - Aerial Video"; if($orders['cvid_o']=="y") echo " - Cinematic Video Only"; if($orders['photos']=="y") echo " - Photos"; if($orders['realtor_com']=="y") echo " - Add to Realtor.com"; if($orders['addy_url']=="y") echo " - Add Address URL"; if($orders['vid']=="y") echo " - Video"; if($orders['areacalc']=="y") echo " - Display Area Calculations"; if($orders['fact_sketch']=="y") echo " - Fact Sketch Only"; if($orders['vid_only']=="y") echo " - Video Only"; if($orders['rush']=="y") echo " - Rush Order";  ?>
							</div>
							<?php if($orders['status']=="complete") { ?>
								<div class="section_review_full">
									<a href="order_stats.php?order_id=<?php echo $order_id;?>"><svg xmlns="http://www.w3.org/2000/svg" class="section_icon" viewBox="0 0 800 800" ><path d="M75,725V0H0v800h800v-75H75z" fill-rule="evenodd"/><path d="M325 650V200.1h75V650h-75zm-175 0V400h75v250h-75zm525 0V300h75v350h-75zm-175 0V50h75v600h-75z"/></svg>View Stats</a>
								</div>
								<div class="section_review_full">
									<svg xmlns="http://www.w3.org/2000/svg" class="section_icon" viewBox="0 0 800 800"><path d="M0 697.8l34-69.3 47-96.4 42.3-87.3c3.7-7.7 9-10.9 17.6-10.8h95.8c7.5-.1 13.4 1.9 18.5 7.8 12.4 14.5 25.3 28.5 39 43.9h-8.1c-36.8 0-73.7.2-110.5-.1-10.3-.1-16.5 3.9-20.9 13.2L88.3 635.4l-12.1 25.2h647.4L713 638.4l-68.4-140.6c-4.1-8.5-10-12.3-19.6-12.2l-111.2.1h-8.5l17.8-20.2c7.7-8.6 15.1-17.4 23.3-25.4 3.2-3.1 8.4-5.6 12.7-5.7l102.2-.3c7.1 0 11.5 3.6 14.4 9.6l38.9 79.7 53 109 32 65.3v7.7c-4.4 6.8-11.1 7.5-18.5 7.5-157.3-.1-314.6 0-471.9 0H17.4c-7.2 0-13.6-1-17.8-7.5.4-2.5.4-5.1.4-7.6h0zm581-436.4c-.6 38.5-6 66.3-18.6 92.4-9.7 20-22.5 38-37 54.8l-65.1 75.2c-19.2 23.2-34.5 49-46.2 76.8-1 2.3-2.1 4.7-3.5 6.8-5.1 7.7-15.7 8-20.9.3-2.7-4-4.5-8.7-6.5-13.1-17.6-39.5-43.8-73.1-72.5-105-21.5-23.8-43.9-46.9-61.2-74.2-26.3-41.5-35.7-86.4-27.3-134.9 7.2-41.5 26-76.8 56.6-105.4 30.5-28.6 66.9-44.3 109-47 36.8-2.4 71.1 5.4 102.8 23.8 30.6 17.7 53.9 42.5 70.3 73.8 14 26.6 19.9 55.3 20.1 75.7h0zm-181.4-88.9c-52.7.2-95.7 43.5-95.5 96.2.2 52.3 43.5 95.4 95.8 95.3 52.8-.1 96.4-43.7 96-95.8-.5-53-43.7-95.9-96.3-95.7h0z"/></svg><strong>Tour Links</strong>
								</div>
								<div class="section_review_full">
								 	<a href="http://www.look2homes.com/l2fp/floorplan/index/<?php echo $order_id;?>" target="_blank">Branded Tour</a>
								</div>
								<div class="section_review_full">
								 	<a href="http://www.look2homes.com/l2fp/floorplan/unbranded/index/<?php echo $order_id;?>" target="_blank">Unbranded Tour</a>
								</div>
								<div class="section_review_full">
									<svg xmlns="http://www.w3.org/2000/svg" class="section_icon" viewBox="0 0 800 800"><path d="M400 0c24.5 0 44.4 19.9 44.4 44.4v426l101.9-101.9c17.7-17.1 45.8-16.6 62.8 1.1 16.6 17.2 16.6 44.5 0 61.8L431.4 609.2c-17.4 17.4-45.5 17.4-62.8 0L190.8 431.4c-17.7-17.1-18.1-45.2-1.1-62.8s45.2-18.1 62.8-1.1l1.1 1.1 101.9 101.9v-426C355.6 19.9 375.5 0 400 0zM133.3 622.2c0-24.5-19.9-44.4-44.4-44.4s-44.4 19.9-44.4 44.4v26.7C44.5 732.5 112 800 195.6 800h408.9c83.6 0 151.1-67.5 151.1-151.1v-26.7c0-24.5-19.9-44.4-44.4-44.4s-44.4 19.9-44.4 44.4v26.7c0 34.6-27.6 62.2-62.2 62.2h-409c-34.6 0-62.2-27.6-62.2-62.2v-26.7z"/></svg><strong>Download Files:</strong>
								</div>
								<div class="section_review_full">
									<a href="zipfiles.php?order_id=<?php echo $order_id; ?>">High Res - *For Print*</a>
								</div>
								<div class="section_review_full">
									<a href="lr_zip.php?order_id=<?php echo $order_id; ?>">Low Res - *For MLS*</a> - <em>Slower as this option will be resizing the photos</em>
								</div>
								<div class="section_review_full">
									<svg xmlns="http://www.w3.org/2000/svg" class="section_icon" viewBox="0 0 800 800"><path d="M658.1 130.8H613V65.5 0l-2.4.1c-1.5.1-117.6.1-258.1.3L96.8.6v334.1 334.1l44.9.2 44.8.2.2 65.3.2 65.4h258.2 258.1V465.4 130.8h-45.1zM187 390.4v259.4h-71v-315V19.7l424.6-.8L187 131.1v259.3zm125.7-280l239-78.2 41.2-13.3c.8-.1 1.1 11.7 1.1 55.9v56H422.6c-94.2 0-171-.2-170.9-.3.2-.2 27.6-9.2 61-20.1zm294.4 595.4h-324c-5.4 0-9.7-4.3-9.7-9.7s4.3-9.7 9.7-9.7h324c5.4 0 9.7 4.3 9.7 9.7-.1 5.4-4.4 9.7-9.7 9.7zm0-59h-324c-5.4 0-9.7-4.3-9.7-9.7s4.3-9.7 9.7-9.7h324c5.4 0 9.7 4.3 9.7 9.7-.1 5.4-4.4 9.7-9.7 9.7zm0-59.1h-324c-5.4 0-9.7-4.3-9.7-9.7s4.3-9.7 9.7-9.7h324c5.4 0 9.7 4.3 9.7 9.7-.1 5.4-4.4 9.7-9.7 9.7zm0-58.9h-324c-5.4 0-9.7-4.3-9.7-9.7s4.3-9.7 9.7-9.7h324c5.4 0 9.7 4.3 9.7 9.7s-4.4 9.7-9.7 9.7zm9.6-68.7c0 5.4-4.3 9.7-9.7 9.7H283.1c-5.4 0-9.7-4.3-9.7-9.7h0c0-5.4 4.3-9.7 9.7-9.7h324c5.3.1 9.6 4.4 9.6 9.7h0zm0-141.2V374H445.1 273.5v-55.1-55.1h171.6 171.6v55.1z"/></svg><strong>Poperty Brochures</strong>
								</div>
								<div class="section_review_full">
									<a href="flyer_photos.php?order_id=<?php echo $order_id; ?>">Select Poperty Brochures Photos</a>
								</div>
								<div class="section_review_full">
									<a href="https://tours.look2homes.com/flyer/<?php echo $order_id;?>" target="_blank">Download Property Brochure PDF</a>
								</div>
							<?php } ?>
						</div>
						<div class="section_half">
							<strong>QR Code for Order:</strong><br>To save qr code, right click and select "Save image as"<br><img src="floorplan/scripts/qrcode/php/qr_img.php?d=https://tours.look2homes.com/index/<?php echo $order_id; ?>&s=6x6&t=P&e=q" alt="<?php echo $orders['street_num']." ".$orders['street_dir']." ".$orders['street_name']." ".$orders['unit']; ?>" />
						</div>
						<div class="section_half">
							...
							<div class="clear"></div>
							<div class="section_review_full">
								<form name="form1" id="form1" method="post" action="order_detail.php" >
                					<strong>Update List Price:</strong><br>
                						$<input name="list_price" type="text" id="list_price" value="<?php echo $orders['price']; ?>" />
										<br>ex. 159000 (not 159,000)<br>
                  						<input name="SubmitPU" type="submit" value="Update" class="section_submit_button" /><input name="order_id" type="hidden" id="order_id" value="<?php echo $order_id; ?>" />
								</form>
							</div>
							<div class="section_review_full">
								<form name="form1" id="form1" method="post" action="order_detail.php" >
                  					<strong>Update Property Information:</strong><br>
									<?php if(!($pd_sel = mysqli_query($link,"SELECT * FROM order_data WHERE order_id = '$order_id'"))){
										printf("%s", sprintf("internal error %d:%s\n", mysqli_errno($link), mysqli_error($link)));
										exit();
									} 
									$show_pd = mysqli_fetch_array($pd_sel); if($orders['prop_type']=="r") { ?>
										<input name="offices" type="hidden" id="offices" value="<?php echo $show_pd['offices']; ?>" /># of Bedrooms:&nbsp;
                						<input name="beds" type="text" id="beds" size="4" value="<?php echo $show_pd['beds']; ?>" /><?php } if($orders['prop_type']=="c") { ?><input name="beds" type="hidden" id="beds" value="<?php echo $show_pd['beds']; ?>" /># of Offices:&nbsp;<input name="offices" type="text" id="offices" size="4" value="<?php echo $show_pd['offices']; ?>" /><?php } ?><br><br>
                						# of Bathrooms: <input name="baths" type="text" id="baths" size="4" value="<?php echo $show_pd['baths']; ?>" /><?php if($orders['prop_type']=="r") { ?>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;# of Half Baths:&nbsp;<input name="h_baths" type="text" id="h_baths" size="4" value="<?php echo $show_pd['h_bath']; ?>" /><?php } ?>
                						<br><input name="SubmitBC" type="submit" value="Update" class="section_submit_button" /><input name="order_id" type="hidden" id="order_id" value="<?php echo $order_id; ?>" />
								</form>
							</div>
						</div>
						<div class="clear"></div>
					...
					<?php if ($_SESSION['SESS_LEVEL']=="admin" || $_SESSION['SESS_LEVEL']=="staff") { ?>
						...
						<form name="move_order" id="move_order" method="post" action="order_detail.php" >
							<div class="section_result_row">
								<div class="section_half">
									Company:&nbsp; <?php if(!($comp_db = mysqli_query($link,"SELECT * FROM companies ORDER BY name ASC"))){
											printf("%s", sprintf("internal error %d:%s\n", mysqli_errno($link), mysqli_error($link)));
											exit();
									} ?>
                  					<select name="comp_id" id="comp_id" onChange='loadSubCats(this.value)'>
									<option value="">Select Company</option>
                   					<?php while($sel_comp=mysqli_fetch_array($comp_db)){	?>
                						<option value="<?php echo $sel_comp['id']; ?>"><?php echo $sel_comp['name']; ?></option> 
               					 	<?php } ?>
                 					</select>									
								</div>
								...
								<div class="section_form_14">
									<input name="SubmitMO" type="submit" value="Update" class="section_submit_button" /><input name="order_id" type="hidden" id="order_id" value="<?php echo $order_id; ?>" />
								</div>
							</div>
						</form>
					<?php } ?>
				...
				<div class="section_container">
					<div class="section_header">
						<svg xmlns="http://www.w3.org/2000/svg" class="section_icon" xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="0 0 512.001 512.001"><path d="M448.875 1.78l-59.314 29.657L330.247 1.78c-4.7-2.35-10.232-2.35-14.932 0L256 31.437 196.686 1.78c-4.7-2.35-10.232-2.35-14.932 0L122.44 31.437 63.125 1.78C52.05-3.758 38.964 4.304 38.964 16.713v445.202c0 6.324 3.573 12.104 9.229 14.933l66.78 33.39c4.7 2.35 10.232 2.35 14.932 0l59.314-29.657 59.314 29.657c4.7 2.35 10.232 2.35 14.932 0l59.314-29.657 59.314 29.657c4.7 2.35 10.232 2.35 14.932 0l66.78-33.39c5.656-2.828 9.229-8.609 9.229-14.933V16.713c.002-12.383-13.06-20.481-24.159-14.933zm-9.229 393.355v56.462l-50.085 25.043-59.314-29.657c-2.35-1.175-4.908-1.762-7.466-1.762a16.7 16.7 0 0 0-7.466 1.762L256 476.639l-59.314-29.657c-4.7-2.35-10.232-2.35-14.932 0l-59.314 29.657-50.085-25.043v-56.462V43.726l42.619 21.31c4.7 2.35 10.232 2.35 14.932 0l59.314-29.657 59.314 29.657c4.7 2.35 10.232 2.35 14.932 0l59.314-29.657 59.314 29.657c4.7 2.35 10.232 2.35 14.932 0l42.619-21.31v351.409z"/><use xlink:href="#B"/><use xlink:href="#B" y="66.78"/><use xlink:href="#B" y="133.56"/><path d="M211.48 300.53h-89.04a16.7 16.7 0 0 0-16.695 16.695 16.7 16.7 0 0 0 16.695 16.695h89.04a16.7 16.7 0 0 0 16.695-16.695c0-9.221-7.475-16.695-16.695-16.695zm-89.04 77.91H89.05a16.7 16.7 0 0 0-16.695 16.695A16.7 16.7 0 0 0 89.05 411.83h33.39a16.7 16.7 0 0 0 16.695-16.695 16.7 16.7 0 0 0-16.695-16.695z"/><use xlink:href="#C"/><use xlink:href="#C" x="100.171"/><use xlink:href="#C" x="200.341"/><path d="M352.843 191.165c-17.515-9.258-35.627-18.832-35.627-29.761 0-15.343 12.482-27.825 27.825-27.825s27.825 12.482 27.825 27.825a16.7 16.7 0 0 0 16.695 16.695 16.7 16.7 0 0 0 16.695-16.695c0-27.966-18.858-51.594-44.52-58.882v-7.898a16.7 16.7 0 0 0-16.695-16.695 16.7 16.7 0 0 0-16.695 16.695v7.898c-25.663 7.287-44.52 30.916-44.52 58.882 0 31.046 29.616 46.701 53.413 59.28 17.515 9.258 35.627 18.832 35.627 29.761 0 15.343-12.482 27.825-27.825 27.825s-27.825-12.482-27.825-27.825a16.7 16.7 0 0 0-16.695-16.695 16.7 16.7 0 0 0-16.695 16.695c0 27.966 18.858 51.594 44.52 58.882v7.898a16.7 16.7 0 0 0 16.695 16.695 16.7 16.7 0 0 0 16.695-16.695v-7.898c25.663-7.287 44.52-30.916 44.52-58.882 0-31.047-29.616-46.701-53.413-59.28z"/><defs ><path id="B" d="M211.48 100.189h-89.04a16.7 16.7 0 0 0-16.695 16.695 16.7 16.7 0 0 0 16.695 16.695h89.04a16.7 16.7 0 0 0 16.695-16.695 16.7 16.7 0 0 0-16.695-16.695z"/><path id="C" d="M222.61 378.44h-33.39a16.7 16.7 0 0 0-16.695 16.695 16.7 16.7 0 0 0 16.695 16.695h33.39a16.7 16.7 0 0 0 16.695-16.695 16.7 16.7 0 0 0-16.695-16.695z"/></defs></svg><h1>Billing Details</h1>
					</div>
					<div class="section_titles">
						<div class="section_form_20">
							<strong>Transaction #</strong>
						</div>
						<div class="section_form_17">
							<strong>Date</strong>
						</div>
						<div class="section_form_14">
					...
					<div class="section_result_row">
						<?php 
							if(!($trans_db = mysqli_query($link,"SELECT * FROM transactions WHERE order_id = '$order_id'"))){
								printf("%s", sprintf("internal error %d:%s\n", mysqli_errno($link), mysqli_error($link)));
								exit();
							}
							if(mysqli_num_rows($trans_db)!=""){ 
								$i = 1; while($trans_detail=mysqli_fetch_array($trans_db)){ ?>
									<div class="section_form_20"><?php echo $trans_detail['trans_id']; ?></div>
									<div class="section_form_17"><?php echo date("m/d/Y h:i A", strtotime($trans_detail['date_trans'])); ?></div>
									<div class="section_form_14">$<?php echo $trans_detail['total_trans']; ?></div>
									<div class="section_form_14"><?php if($trans_detail['res_ad']=="A"){ echo "Approved"; }else{ echo "Not Approved"; } ?></div>
									<div class="section_form_10"><?php echo $trans_detail['c4']; ?></div>
									<div class="section_form_24"><?php if($trans_detail['ifp']!="0.00") { ?>IFP: <?php echo $trans_detail['ifp']; ?><br /><?php } if($trans_detail['hdr_up']!="0.00") { ?>HDR Photo Upgrade: <?php echo $trans_detail['hdr_up']; ?><br /><?php } if($trans_detail['hdr_only']!="0.00") { ?>HDR Photos Only: <?php echo $trans_detail['hdr_only']; ?><br /><?php } if($trans_detail['spg_only']!="0.00") { ?>Standard Photos Only: <?php echo $trans_detail['spg_only']; ?><br /><?php } if($trans_detail['tlight_up']!="0.00") { ?>Twilight Photos: <?php echo $trans_detail['tlight_up']; ?><br /><?php } if($trans_detail['aphotos']!="0.00") { ?>Aerial Photos: <?php echo $trans_detail['aphotos']; ?><br /> <?php } if($trans_detail['real_com']!="0.00") { ?>Add to Realtor.com: <?php echo $trans_detail['real_com']; ?><br /><?php } if($trans_detail['addy_url']!="0.00") { ?>Address URL: <?php echo $trans_detail['addy_url']; ?><br /><?php } if($trans_detail['vid']!="0.00") { ?>Walk Through Video: <?php echo $trans_detail['vid']; ?><br /><?php } if($trans_detail['cvid']!="0.00") { ?>Cinemaic Video: <?php echo $trans_detail['cvid']; ?><br /><?php } if($trans_detail['3d']!="0.00") { ?>Zillow 3D Tour: <?php echo $trans_detail['3d']; ?><br /><?php } if($trans_detail['avid']!="0.00") { ?>Aerial Video: <?php echo $trans_detail['avid']; ?><br /><?php } if($trans_detail['vid_o']!="0.00") { ?>Video Only: <?php echo $trans_detail['vid_o']; ?><br /><?php } if($trans_detail['draw_only']!="0.00") { ?>Drawing Only: <?php echo $trans_detail['draw_only']; ?><br /><?php } if($trans_detail['rebuild']!="0.00") { ?>Rebuild Fee: <?php echo $trans_detail['rebuild']; ?><br /><?php } if($trans_detail['retake']!="0.00") { ?>Photo Retake Fee: <?php echo $trans_detail['retake']; ?><br /><?php } if($trans_detail['canc_fee']!="0.00") { ?>Cancellation Fee: <?php echo $trans_detail['canc_fee']; ?><br /><?php } if($trans_detail['trip_fee']!="0.00") { ?>Trip Fee: <?php echo $trans_detail['trip_fee']; ?><br /><?php } if($trans_detail['ae']!="0.00") { ?>Advanced Editing: <?php echo $trans_detail['ae']; ?><br /><?php } if($trans_detail['misc']!="0.00") { ?>Miscellaneous: <?php echo $trans_detail['misc']; ?><br /><?php } if($trans_detail['rush_fee']!="0.00") { ?>Rush Fee: <?php echo $trans_detail['rush_fee']; ?><br /><?php } if($trans_detail['sign']!="0.00") { ?>Sign: <?php echo $trans_detail['sign']; ?><br /><?php }if($trans_detail['post_only']!="0.00") { ?>Post Only: <?php echo $trans_detail['post_only']; ?><br /><?php }if($trans_detail['lighted']!="0.00") { ?>Lighted Sign Post: <?php echo $trans_detail['lighted']; ?><br /><?php }if($trans_detail['sr']!="0.00") { ?>Sign Rider: <?php echo $trans_detail['sr']; ?><br /><?php } if($trans_detail['qr']!="0.00") { ?>QR Sticker: <?php echo $trans_detail['qr']; ?><br /><?php } if($trans_detail['qrgen']!="0.00") { ?>QR Code Generation: <?php echo $trans_detail['qrgen']; ?><br /><?php } ?></div>
									<div class="clear"></div>
						<?php $i++; } } ?>
					</div>
				</div>
	...
	<script async defer
        src="https://maps.googleapis.com/maps/api/js?key=AIzaSyByk08lPz3tTnvnxW-pLZJcUWcXw-zK7KI&callback=initMap">
	</script>
</body>
</html>