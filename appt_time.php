...
if($_GET['st']!="") { 
	$appt_st = $_GET['st'];	
	$letter_menu = '<a href="appt_time.php?st='.$dealer_st.'">All</a>';
	if(!($letter_num = mysqli_query($link,"SELECT * FROM users WHERE lastname REGEXP '^[0-9]' AND state = '$appt_st' AND level = 'dealer'"))){
	...
	if($num_count!="0") { 
		$letter_menu .= '<a href="appt_time.php?st='.$appt_st.'&letter=num">#</a>'; 
	} else { 
...
foreach($letter_sort as $letter){
	if($_GET['st']!="") { 
		if(!($letter_result = mysqli_query($link,"Select * FROM users WHERE lastname LIKE '".$letter."%' AND state = '$appt_st' AND level = 'dealer'"))){
			printf("%s", sprintf("internal error %d:%s\n", mysqli_errno($link), mysqli_error($link)));
			exit();
		}
		...
		if($letter_count!="0") { 
			$letter_menu .= '<a href="appt_time.php?st='.$appt_st.'&letter='.$letter.'">'.$letter.'</a>'; 
		} else { 
...
<html>
	<head>
		...
		<title>Appointment Duration | Look2 Home Marketing</title>
			...
			<div id="rc_content_container">
				...
				<div class="section_container">
					...
					<div class="section_result_row">
						<a href="appt_time.php?st=<?php echo $dealer_states['state']; ?>">
					...
					<div class="section_header">
						<svg xmlns="http://www.w3.org/2000/svg" class="section_icon" viewBox="0 0 800 800"><path d="M336 0c-8.8 0-16 7.2-16 16v32c0 8.8 7.2 16 16 16h24v42.4C186.9 126.3 52 273.7 52 452c0 191.9 156.1 348 348 348s348-156.1 348-348c.1-65.7-18.6-130-53.8-185.5l33.9-27.4 15.1 18.7c5.6 6.9 15.6 7.9 22.5 2.4l24.9-20.1c6.9-5.6 7.9-15.6 2.4-22.5L712.4 118c-5.6-6.9-15.6-7.9-22.5-2.4L665 135.7c-6.9 5.6-7.9 15.6-2.4 22.5l15.1 18.7-33.8 27.3c-53.9-53.1-125-88.8-204.1-97.9V64h24c8.8 0 16-7.2 16-16V16c0-8.8-7.2-16-16-16H336zm64 160c161.6 0 292 130.4 292 292S561.6 744 400 744 108 613.6 108 452s130.4-292 292-292zm0 59.5V452l185.9 139.6c52.9-70.4 61.4-164.7 22.1-243.5-39.4-78.8-119.9-128.5-208-128.6h0z"/></svg><h1>Appointment Duration - Select Dealer</h1>
					</div>
					...
					?>
					<div class="section_result_row">
						<a href="appt_edit.php?uid=<?php echo $select_dealer['id']; ?>">
