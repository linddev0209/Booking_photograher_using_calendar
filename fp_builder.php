...
<html>
	<head>
		...
		<title>Build IFP | Look2 Home Marketing</title>
		...
		<div id="right_column">
			...
			<div id="rc_content_container">
				...
				<div class="section_container">
					...
					<div class="section_titles">
						<div class="section_form_50"><strong>Floor Plans:</strong>&nbsp;
							<?php
								if(!($fp_levels = mysqli_query($link,"SELECT * FROM floorplans where order_id = '$order_id' ORDER BY sort ASC"))){
									printf("%s", sprintf("internal error %d:%s\n", mysqli_errno($link), mysqli_error($link)));
									exit();
								}
								while($levels=mysqli_fetch_array($fp_levels)){	
							?>
							<a href="fp_builder.php?order_id=<?php echo $levels['order_id']; ?>&fp_id=<?php echo $levels['id']; ?>"><?php echo $levels['label']; ?></a>&nbsp;&nbsp;&nbsp;
							<?php } ?>
						</div>
						<div class="clear"></div>
					</div>
					<?php 
						if(!($fp_floors = mysqli_query($link,"SELECT * FROM floorplans where order_id = '$order_id' AND id = '$fp_id'"))){
							printf("%s", sprintf("internal error %d:%s\n", mysqli_errno($link), mysqli_error($link)));
							exit();
						}
					$cF = 0;
					foreach($fp_floors as $floorplans) {	
						$cF++;
						
						$photos = mysqli_fetch_all(mysqli_query($link, "SELECT * FROM photos WHERE order_id = " . $order_id . " AND fp_id = " . $fp_id . ' ORDER BY sort'), MYSQLI_ASSOC);
						
						$icons = mysqli_fetch_all(mysqli_query($link, "SELECT * FROM info_cont  WHERE order_id = " . $order_id . " AND fp_id = " . $fp_id . ''), MYSQLI_ASSOC);
					...
					?>
						<div class="section_result_row">
							<div class="pic_section">
								<?php $cPhotos = 0; foreach($photos as $photo) { $cPhotos++; $sortPhotos[$photo['id']] = ['photo' => $photo, 'count' => $cPhotos]; ?>
									<div class="picButton" data-id='<?php echo $photo['id']; ?>'>
										<img src="https://tours.look2homes.com/includes/rezor.php?src=<?php echo $order_id; ?>/photos/<?php echo $order_id;?>_<?php echo $floorplans['id']; ?>_<?php echo $photo['id'];?>.jpg&amp;w=70&amp;h=70&amp;zc=1&amp;q=90">
										<div class="picButton_dsp">
											<span class="count"><?php echo $cPhotos; ?></span>
										</div>
									</div>
								<?php } 
									$cIcons = 0; foreach($icons as $icon) { 
										$cIcons++; $sortIcons[$icon['id']] = ['icon' => $icon, 'count' => $cIcons]; if(!mysqli_fetch_array(mysqli_query($link, 'SELECT * FROM info_coord WHERE info_id = ' . $icon['id']))) {
								?>
									<div class="iconButton" style="display: flex; padding: 0.8em; align-items:center" data-id='<?php echo $icon['id']; ?>'>
										<span class="count"><?php echo $cIcons; ?></span>
									</div>
								<?php } 
								} ?>
							</div>
						...
						<div class="section_result_row">
							<div class="section_full">
								<strong>Keep all icons inside the area of the Floor Plan! </strong>
							</div>
							<div class="tabWrp  <?php if(($fp_id == '1' && $cF == 1) || $fp_id == $floorplans['id']) echo 'current'; ?>" data-id='<?php echo $floorplans['id']; ?>' id="tabWrp<?php echo $floorplans['id']; ?>">
								<div style="width:100%;height:100%; position:relative; padding: 12px 0px 12px 6px;" class="dropWrp">
									<?php 
										$cds = mysqli_fetch_all(mysqli_query($link, "SELECT * FROM coordinates WHERE fp_id = " . $floorplans['id'] . ' AND order_id = ' . $order_id), MYSQLI_ASSOC);
						
										$icons = mysqli_fetch_all(mysqli_query($link, "SELECT * FROM info_coord WHERE fp_id = " . $floorplans['id'] . ' AND order_id = ' . $order_id), MYSQLI_ASSOC);  
		
										foreach($cds as $photo) { if(isset($sortPhotos[$photo['photo_id']])) { 
									?>
										<div class='picDroppable' data-id='<?php echo $photo['photo_id']; ?>' style="display: flex; justify-content: center; align-items: center; top: <?php echo $photo['ycoord']; ?>px; left: <?php echo $photo['xcoord']; ?>px;position: absolute;" draggable=true>
											<span class='picDropCount'><?php echo $sortPhotos[$photo['photo_id']]['count']; ?> </span> 
											<i class="fas fa-times" ></i>
										</div>
									<?php } } foreach($icons as $photo) { if(isset($sortIcons[$photo['info_id']])) { ?>
						 				<div class='iconDroppable' data-id='<?php echo $photo['info_id']; ?>' style="display: flex; justify-content: center; align-items: center; top: <?php echo $photo['ycoord']; ?>px; left: <?php echo $photo['xcoord']; ?>px;position: absolute;" draggable=true>
											<span class='iconDropCount'><?php echo $sortIcons[$photo['info_id']]['count']; ?> </span> 
											<i class="fas fa-times" ></i>
										</div>
									<?php } } ?>
									<img class="floorplanimg" src="<?php echo 'https://tours.look2homes.com/' . $order_id . '/' . $order_id . '_' . $floorplans['id'] . '.jpg'; ?>">
								</div>
							</div>
						</div>
					<? } ?>			
	...
	<script>
		$(function() {
			$(".saveBtn").click(function() {
				$(".logsWrp").html('')
					$('.tabWrp.current .dropWrp .picDroppable').each(function() {
						var p = $(this).position();
					
						var top = p.top.toFixed(2);
						var left = p.left.toFixed(2);
						var el = $(this);
						$.ajax({
							type: 'GET',
							url: 'https://manager.look2homes.com/savecoord.php?xcoord=' + left + '&fp_id=' + $('.tabWrp.current').data('id') + '&order_id=' + $("#order_id").val() + '&ycoord=' + top + '&photo_id=' + $(this).data('id'),
							success: function(res) {
								$(".logsWrp").append($('<span></span>').html('Icon ' + el.find('.picDropCount').html() + " saved <br>" + left + "," + top + "<br><br>"));
							}
						})
					})
					$('.tabWrp.current .dropWrp .iconDroppable').each(function() {
						var p = $(this).position();
						...
						$.ajax({
							type: 'GET',
							url: 'https://manager.look2homes.com/savecoord_icons.php?xcoord=' + left + '&fp_id=' + $('.tabWrp.current').data('id') + '&order_id=' + $("#order_id").val() + '&ycoord=' + top + '&photo_id=' + $(this).data('id'),
							success: function(res) {
								$(".logsWrp").append($('<span></span>').html('Icon ' + el.find('.iconDropCount').html() + " saved at " + left + "," + top));
							}
						})
					})
			})
			$('.tabToggle').click(function() {
				$('.tabToggle').removeClass('current');
				$(this).addClass('current');
				$('.tabWrp').removeClass('current').hide();
				$("#tabWrp" + $(this).data('link')).addClass('current').show();
			});
			$('.picDroppable').draggable({ containment: "parent"});
			$('.iconDroppable').draggable({ containment: "parent"});

			$('.dropWrp').droppable({ drop: function(event, ui) {  }});

			$(".picButton").click(function() {
				var top = $(".tabWrp.current .floorplanimg").height() / 2;
				var left = $(".tabWrp.current .floorplanimg").width() / 2;

				var d=  $('<div class="picDroppable new" data-id=' + $(this).data('id') + ' style="display:flex;justify-content:center;align-items:center;top:' + top + 'px;left: ' + left + 'px;position:absolute" draggable=true><span class=picDropCount>' + $(this).find('.count').html() + '</span> <i class="fas fa-times"></i></div>');
				d.find('i').click(function() {
					var el = $(this).parents('.picDroppable')
					$.ajax({
						url: 'https://manager.look2homes.com/remove.php?fp_id=' + $('.tabWrp.current').data('id') + "&order_id=" + $("#order_id").val() + "&photo_id=" + el.data('id'),
						success: function(res) {
							el.remove();
						}
					})
				});
				$('.tabWrp.current .dropWrp').append(d);
			...
			$(".iconButton").click(function() {
				...
				var d=  $('<div class="iconDroppable new" data-id=' + $(this).data('id') + ' style="display:flex;justify-content:center;align-items:center;top:' + top + 'px;left: ' + left + 'px;position:absolute" draggable=true><span class=iconDropCount>' + $(this).find('.count').html() + '</span> <i class="fas fa-times"></i></div>');
				d.find('i').click(function() {
					var el = $(this).parents('.iconDroppable')
					$.ajax({
						url: 'https://manager.look2homes.com/remove_icons.php?fp_id=' + $('.tabWrp.current').data('id') + "&order_id=" + $("#order_id").val() + "&photo_id=" + el.data('id'),
						success: function(res) {
							el.remove();
						}
					})
			...
			$('.iconDroppable i').click(function() {
				var el = $(this).parents('.iconDroppable')
				if(el.hasClass('new'))
					el.remove();
				else {
