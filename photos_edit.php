...
if(!($zillow_db = mysqli_query($link,"select * FROM zillow_tours WHERE order_id = '$order_id' LIMIT 1"))){
	printf("%s", sprintf("internal error %d:%s\n", mysqli_errno($link), mysqli_error($link)));
	exit();
}

$ztour = mysqli_fetch_array($zillow_db);

if ($_GET['task'] == "remove"){ 
	$p_id = $_GET['p_id'];
	$fp_id = $_GET['fp_id'];
	if(!($p_delete = mysqli_query($link,"DELETE FROM photos WHERE id='$p_id'"))){
		printf("%s", sprintf("internal error %d:%s\n", mysqli_errno($link), mysqli_error($link)));
		exit();
	}
	$del_path = 'floorplan/'. $order_id .'/photos/'; 
	$del_ext = ".jpg" ;
	$del_file = $del_path . $order_id . "_".$fp_id."_".$p_id . $del_ext ;
	unlink($del_file);
	header("Location: photos_edit.php?order_id=".$order_id);
	exit();
}

if ($_GET['task'] == "remvideo"){ 
	$del_path = 'floorplan/'. $order_id .'/video/'; 
	$del_ext = ".mp4" ;
	$del_ext2 = ".ogv" ;
	$del_file = $del_path . $order_id . $del_ext ;
	$del_file2 = $del_path . $order_id . $del_ext2 ;
	unlink($del_file);
	unlink($del_file2);
	if(file_exists('floorplan/'. $order_id .'/video/'.$order_id.'_hr_mp4')){
		$del_file3 = 'floorplan/'. $order_id .'/video/'.$order_id.'_hr_mp4' ;
		unlink($del_file3);
	}
	header("Location: photos_edit.php?order_id=".$order_id);
	exit();
...
if ($_GET['task'] == "rem3d"){ 
	if(!($zd_delete = mysqli_query($link,"DELETE FROM zillow_tours WHERE order_id='$order_id'"))){
		printf("%s", sprintf("internal error %d:%s\n", mysqli_errno($link), mysqli_error($link)));
		exit();
	}
	header("Location: photos_edit.php?order_id=".$order_id);
...
if($_POST['Submit2']){
	$order_sort = $_POST['order_id'];
	if(!($up_sort = mysqli_query($link,"SELECT * FROM photos where order_id = '$order_sort' ORDER BY sort ASC"))){
		printf("%s", sprintf("internal error %d:%s\n", mysqli_errno($link), mysqli_error($link)));
		exit();
	}
	while($photo_sort=mysqli_fetch_array($up_sort)){
		$pid = $photo_sort['id'];
		$sort = $_POST['sort'.$pid];
		$caption = $_POST['caption'.$pid];
		if($sort!=""){
			if(!($rec_record = mysqli_query($link,"UPDATE photos set sort = '$sort' WHERE id = '$pid'"))){
				printf("%s", sprintf("internal error %d:%s\n", mysqli_errno($link), mysqli_error($link)));
				exit();
			}
		}
		if($caption!=""){
			if(!($rec_record = mysqli_query($link,"UPDATE photos set caption = '$caption' WHERE id = '$pid'"))){
				printf("%s", sprintf("internal error %d:%s\n", mysqli_errno($link), mysqli_error($link)));
				exit();
			}
		}
	}
	header("Location: photos_edit.php?order_id=".$order_sort);
	exit();
}

if($_POST['SubmitZD']){
	$order_zd = $_POST['order_id'];
    $tour_zd = $_POST['tour_url'];
	if(!($zd_update = mysqli_query($link,"INSERT INTO zillow_tours SET order_id='$order_zd',tour_url='$tour_zd'"))){
		printf("%s", sprintf("internal error %d:%s\n", mysqli_errno($link), mysqli_error($link)));
		exit();
	}
	header("Location: photos_edit.php?order_id=".$order_zd);
	exit();
}
?>
...
<html>
	<head>
		...
		<title>Edit Photos/Videos) | Look2 Home Marketing</title>
			...
			<div id="rc_content_container">
				...
				<div class="section_container">
					...
					<div class="section_result_row">
						<div class="section_full">
							<strong>Upload Video</strong>:&nbsp;<?php if($order_id!="") { echo "<em>Before uploading please rename your video to </em><strong>".$order_id.".mp4 &amp; ".$order_id.".ogv</strong>"; } ?>&nbsp;&nbsp;<?php if(file_exists("floorplan/".$order_id."/video/".$order_id.".mp4")){ ?><a href="floorplan/<?php echo $order_id; ?>/video/<?php echo $order_id.".mp4"; ?>" target="_blank"><svg xmlns="http://www.w3.org/2000/svg" class="section_icon" viewBox="0 0 512 512" ><path d="M464 56v40h-40V56H88v40H48V56H0v400h48v-40h40v40h336v-40h40v40h48V56h-48zM88 354.672H48v-64h40v64zm0-133.344H48v-64h40v64zm220.094 44.719L206.36 326.781c-.75.438-1.656.469-2.406.031-.734-.422-1.203-1.219-1.203-2.094V264v-60.719c0-.859.469-1.656 1.203-2.094.75-.406 1.656-.391 2.406.031l101.734 60.75c.719.406 1.156 1.203 1.156 2.031 0 .845-.437 1.626-1.156 2.048zM464 354.672h-40v-64h40v64zm0-133.344h-40v-64h40v64z"/></svg>View Video</a> | <a class="remove_rec" href="?order_id=<?php echo $order_id; ?>&task=remvideo"><svg xmlns="http://www.w3.org/2000/svg" class="section_icon" viewBox="0 0 800 653.3" ><path d="M398.6 314.9L584.1 131c6.1-6.1 16-6 22.1.1L790 316.7c6.1 6.1 6 16-.1 22.1l-44.4 44.1c-6.1 6.1-16 6-22.1-.1l-71.7-72.3c4.3 89.1-28.1 179.5-97 246.9C427 682 224 680.1 98.6 553.1-26.6 426.2-25.5 221.9 101.2 96.4 160.5 37.6 236.9 6.5 314.3 3c9-.4 16.5 6.7 16.4 15.7l-.3 62.6c0 8.2-6.5 15.1-14.7 15.5-53.9 3.1-107 25.1-148.5 66.2-90.1 89.3-90.7 234.4-1.4 324.5s234.4 90.7 324.5 1.4c54.2-53.7 76-127.6 65.4-197.5l-91 90.2c-6.1 6.1-16 6-22.1-.1l-44.1-44.4c-6.1-6.2-6.1-16.1.1-22.2h0z"/></svg>Remove</a><?php } ?>
						</div>
						<div class="section_full">
							<div id="filelist">Your browser doesn't have Flash, Silverlight or HTML5 support.</div>
							<div id="plu_container">
						...
						<script type="text/javascript">
							...
							var uploader = new plupload.Uploader({
								...
								container: document.getElementById('plu_container'), // ... or DOM Element itself
								url : 'upload.php?order_id=<?php echo $order_id; ?>',
								...
								filters : {
									...
									mime_types: [
										...
										{title : "Movie files", extensions : "mp4,ogv"}
									]
								},
					...
					<form name="form2" id="form2" method="post" action="photos_edit.php" >
						<div class="section_titles">
							<div class="section_form_25"><strong>Zillow 3D Tour: </strong></div>
							<div class="clear"></div>
						</div>
						<div class="section_result_row">
							<?php 
								if(!($zd_count = mysqli_query($link,"SELECT DISTINCT * FROM zillow_tours WHERE order_id = '$order_id'"))){
									printf("%s", sprintf("internal error %d:%s\n", mysqli_errno($link), mysqli_error($link)));
									exit();
								}

								$zd_records = mysqli_num_rows($zd_count); if ($zd_records > 0) { ?>
  									<div class="section_full"><a href="<?php echo $ztour['tour_url']; ?>" target="_blank">View 3D Tour</a> | <a href="?order_id=<?php echo $order_id; ?>&task=rem3d">Remove</a></div>
							<?php } else { ?>
							<div class="section_full">
								Tour URL: <input name="tour_url" type="text" id="tour_url" size="100" /><input name="order_id" type="hidden" id="order_id" value="<?php echo $order_id; ?>" />
							</div>
							<div class="section_full">
								<input name="SubmitZD" type="submit" class="section_submit_button_md" id="SubmitZD" value="Add Tour" />
							</div>
						    <?php } ?>
						</div>
					</form>
					<form name="form1" id="form1" method="post" action="photos_edit.php" >
						<div class="section_titles">
							<div class="section_form_25"><strong>Photos:</strong></div>
							<div class="clear"></div>
						</div>
						<div class="section_result_row">
							<?php 
								if(!($e_photo = mysqli_query($link,"SELECT * FROM photos where order_id = '$order_id' ORDER BY fp_id, sort ASC"))){
									printf("%s", sprintf("internal error %d:%s\n", mysqli_errno($link), mysqli_error($link)));
									exit();
								}
								for($i=0; $i<mysqli_num_rows($e_photo)/2; $i++) { 
							?>
							<div class="section_form_25">
								<?php $list_photo = mysqli_fetch_array($e_photo); $fp_assigned = $list_photo['fp_id'];?><img src="includes/rezor.php?src=floorplan/<?php echo $order_id; ?>/photos/<?php echo $order_id; ?>_<?php echo $list_photo['fp_id']; ?>_<?php echo $list_photo['id']; ?>.jpg&amp;w=238&amp;zc=1&amp;q=90" alt="Order#<?php echo $order_id; ?>" />
							</div>
							<div class="section_form_25">
								...
								$fpl_row = mysqli_fetch_array($fp_db);  echo $fpl_row['label'];
								?><br><br>
								<strong>Sort:</strong>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<input name="sort<?php echo $list_photo['id']; ?>" type="text" id="sort<?php echo $list_photo['id']; ?>" value="<?php echo $list_photo['sort']; ?>" size="3" /><br><br>
								<strong>Caption:</strong>&nbsp;<input name="caption<?php echo $list_photo['id']; ?>" type="text" id="caption<?php echo $list_photo['id']; ?>" value="<?php echo $list_photo['caption']; ?>" size="16" /><br><br>
								<a class="remove_rec" href="?order_id=<?php echo $order_id; ?>&amp;fp_id=<?php echo $fp_assigned; ?>&amp;p_id=<?php echo $list_photo['id']; ?>&amp;task=remove"><svg xmlns="http://www.w3.org/2000/svg" class="section_icon" viewBox="0 0 800 653.3" ><path d="M398.6 314.9L584.1 131c6.1-6.1 16-6 22.1.1L790 316.7c6.1 6.1 6 16-.1 22.1l-44.4 44.1c-6.1 6.1-16 6-22.1-.1l-71.7-72.3c4.3 89.1-28.1 179.5-97 246.9C427 682 224 680.1 98.6 553.1-26.6 426.2-25.5 221.9 101.2 96.4 160.5 37.6 236.9 6.5 314.3 3c9-.4 16.5 6.7 16.4 15.7l-.3 62.6c0 8.2-6.5 15.1-14.7 15.5-53.9 3.1-107 25.1-148.5 66.2-90.1 89.3-90.7 234.4-1.4 324.5s234.4 90.7 324.5 1.4c54.2-53.7 76-127.6 65.4-197.5l-91 90.2c-6.1 6.1-16 6-22.1-.1l-44.1-44.4c-6.1-6.2-6.1-16.1.1-22.2h0z"/></svg>Remove</a>
							</div>
							<?php $list_photo = mysqli_fetch_array($e_photo); if($list_photo!="") { $fp_assigned = $list_photo['fp_id']; ?>
							<div class="section_form_25"><img src="includes/rezor.php?src=floorplan/<?php echo $order_id; ?>/photos/<?php echo $order_id; ?>_<?php echo $list_photo['fp_id']; ?>_<?php echo $list_photo['id']; ?>.jpg&amp;w=238&amp;zc=1&amp;q=90" alt="Order#<?php echo $order_id; ?>" /></div>
							<div class="section_form_25">
								<strong>Floor Plan:</strong>
								<?php 
									if(!($fp_db = mysqli_query($link,"SELECT label FROM floorplans where id = '$fp_assigned'"))){
								...
								?><br><br>
                				 <strong>Sort:</strong>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<input name="sort<?php echo $list_photo['id']; ?>" type="text" id="sort<?php echo $list_photo['id']; ?>" value="<?php echo $list_photo['sort']; ?>" size="3" /><br><br>
								<strong>Caption:</strong>&nbsp;<input name="caption<?php echo $list_photo['id']; ?>" type="text" id="caption<?php echo $list_photo['id']; ?>" value="<?php echo $list_photo['caption']; ?>" size="16" />
								<br><br>
								<a class="remove_rec" href="?order_id=<?php echo $order_id; ?>&amp;fp_id=<?php echo $fp_assigned; ?>&amp;p_id=<?php echo $list_photo['id']; ?>&amp;task=remove"><svg xmlns="http://www.w3.org/2000/svg" class="section_icon" viewBox="0 0 800 653.3" ><path d="M398.6 314.9L584.1 131c6.1-6.1 16-6 22.1.1L790 316.7c6.1 6.1 6 16-.1 22.1l-44.4 44.1c-6.1 6.1-16 6-22.1-.1l-71.7-72.3c4.3 89.1-28.1 179.5-97 246.9C427 682 224 680.1 98.6 553.1-26.6 426.2-25.5 221.9 101.2 96.4 160.5 37.6 236.9 6.5 314.3 3c9-.4 16.5 6.7 16.4 15.7l-.3 62.6c0 8.2-6.5 15.1-14.7 15.5-53.9 3.1-107 25.1-148.5 66.2-90.1 89.3-90.7 234.4-1.4 324.5s234.4 90.7 324.5 1.4c54.2-53.7 76-127.6 65.4-197.5l-91 90.2c-6.1 6.1-16 6-22.1-.1l-44.1-44.4c-6.1-6.2-6.1-16.1.1-22.2h0z"/></svg>Remove</a>
							</div>
							<?php }?>
							...
							<div class="section_form_25">
								<input name="order_id" type="hidden" id="order_id" value="<?php echo $order_id; ?>" />
                  				<input name="Submit2" type="submit" class="section_submit_button_md" id="Submit2" value="Update Photo(s)" />
							</div>
						</div>
					</form>
				</div>
