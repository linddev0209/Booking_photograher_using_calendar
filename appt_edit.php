...
if ($_SESSION['SESS_LEVEL']=="dealer"){
	...
	if ($u_id=="") {
		if(!($appt_db = mysqli_query($link,"select * from users where id = '$d_id'"))){
			printf("%s", sprintf("internal error %d:%s\n", mysqli_errno($link), mysqli_error($link)));
			exit();
		}
		$appt_user = mysqli_fetch_array($appt_db);
	} else {
		if(!($appt_db = mysqli_query($link,"select * from users where id = '$u_id' AND primary_deal = '$d_id'"))){
			printf("%s", sprintf("internal error %d:%s\n", mysqli_errno($link), mysqli_error($link)));
			exit();
		}
		$appt_user = mysqli_fetch_array($appt_db);
	...
	if(!($appt_db = mysqli_query($link,"select * from users where id = '$u_id'"))){
		printf("%s", sprintf("internal error %d:%s\n", mysqli_errno($link), mysqli_error($link)));
		exit();
	}
	$appt_user = mysqli_fetch_array($appt_db);
...
$appt_dur_id = $appt_user['id'];
$appt_dur_level = $appt_user['level'];
$appt_dur_name = $appt_user['lastname'].', '.$appt_user['firstname'];

if(!($appt_dur = mysqli_query($link,"select * from apt_time where user_id = '$appt_dur_id'"))){
	printf("%s", sprintf("internal error %d:%s\n", mysqli_errno($link), mysqli_error($link)));
	exit();
}
$appt_times = mysqli_fetch_array($appt_dur);
...
if($_POST['Submittime']){
	...
	$fp4 = $_POST['fp4time'];
	$fp5 = $_POST['fp5time'];
	$photos = $_POST['phototime'];
	$zd = $_POST['zillowtime'];
	$ap = $_POST['aphotostime'];
	$av = $_POST['avideotime'];
	$draw = $_POST['drawtime'];	
	
	if(!($appt_count = mysqli_query($link,"SELECT * FROM apt_time WHERE user_id = '$appt_id'"))){
			printf("%s", sprintf("internal error %d:%s\n", mysqli_errno($link), mysqli_error($link)));
			exit();
		}
	if(mysqli_num_rows($appt_count)==""){
		if(!($appt_update = mysqli_query($link,"INSERT INTO apt_time SET user_id = '$appt_id', fp1 = '$fp1', fp2 = '$fp2', fp3 = '$fp3', fp4 = '$fp4', fp5 = '$fp5', photos = '$photos', zillow_3d = '$zd', aerial_photos = '$ap', aerial_vid = '$av', draw = '$draw'"))){
			printf("%s", sprintf("internal error %d:%s\n", mysqli_errno($link), mysqli_error($link)));
			exit();
		}
	} else {
		if(!($appt_update = mysqli_query($link,"UPDATE apt_time SET fp1 = '$fp1', fp2 = '$fp2', fp3 = '$fp3', fp4 = '$fp4', fp5 = '$fp5', photos = '$photos', zillow_3d = '$zd', aerial_photos = '$ap', aerial_vid = '$av', draw = '$draw' WHERE user_id = '$appt_id'"))){
			printf("%s", sprintf("internal error %d:%s\n", mysqli_errno($link), mysqli_error($link)));
			exit();
		}
	}
	...
	if ($_SESSION['SESS_LEVEL']=="dealer"){
		if ($u_id=="") {
			header("Location: appt_edit.php?flag=".$flag."");
			exit();
		} else {
			header("Location: appt_edit.php?uid=".$appt_id."flag=".$flag."");
			exit();
		}
	} else {
		header("Location: appt_edit.php?uid=".$appt_id."flag=".$flag."");
...
<html>
	<head>
		...
		<title>Manage Appointment Duration | Look2 Home Marketing</title>
	...
	<div id="container">
		...
		<div id="right_column">
			...
			<div id="rc_content_container">
				<form action="appt_edit.php" method="post" name="apptedit" id="dedit" enctype="multipart/form-data">
					<div class="section_container">
						<div class="section_header">
							<svg xmlns="http://www.w3.org/2000/svg" class="section_icon" viewBox="0 0 800 800"><path d="M336 0c-8.8 0-16 7.2-16 16v32c0 8.8 7.2 16 16 16h24v42.4C186.9 126.3 52 273.7 52 452c0 191.9 156.1 348 348 348s348-156.1 348-348c.1-65.7-18.6-130-53.8-185.5l33.9-27.4 15.1 18.7c5.6 6.9 15.6 7.9 22.5 2.4l24.9-20.1c6.9-5.6 7.9-15.6 2.4-22.5L712.4 118c-5.6-6.9-15.6-7.9-22.5-2.4L665 135.7c-6.9 5.6-7.9 15.6-2.4 22.5l15.1 18.7-33.8 27.3c-53.9-53.1-125-88.8-204.1-97.9V64h24c8.8 0 16-7.2 16-16V16c0-8.8-7.2-16-16-16H336zm64 160c161.6 0 292 130.4 292 292S561.6 744 400 744 108 613.6 108 452s130.4-292 292-292zm0 59.5V452l185.9 139.6c52.9-70.4 61.4-164.7 22.1-243.5-39.4-78.8-119.9-128.5-208-128.6h0z"/></svg><h1>Appointment Duration - Set Duration Times</h1>
						</div>
						...
						<div class="section_titles">
							<div class="section_full"?>
								<h1><?php if($appt_dur_level=="dealer"){ ?>Dealer Default Times<? } else { ?>Photographer Times<?php } ?> - <?php echo $appt_dur_name;?></h1>
							</div>
						</div>
						<div class="section_result_row">
							<div class="section_form_20"><strong>Floor Plan 0-2000 SQFT</strong></div>
							<div class="section_form_20"><strong>Floor Plan 2001-4000 SQFT</strong></div>
							<div class="section_form_20"><strong>Floor Plan 4001-6000 SQFT</strong></div>
							<div class="section_form_20"><strong>Floor Plan 6001-8000 SQFT</strong></div>
							<div class="section_form_20"><strong>Floor Plan 8001+ SQFT</strong></div>
							<div class="clear"></div>
							<div class="section_form_20"><input name="fp1time" type="text" id="fp1time" value="<?php echo $appt_times['fp1']; ?>" size="6" />&nbsp;mins</div>
							<div class="section_form_20"><input name="fp2time" type="text" id="fp2time" value="<?php echo $appt_times['fp2']; ?>" size="6" />&nbsp;mins</div>
							<div class="section_form_20"><input name="fp3time" type="text" id="fp3time" value="<?php echo $appt_times['fp3']; ?>" size="6" />&nbsp;mins</div>
							<div class="section_form_20"><input name="fp4time" type="text" id="fp4time" value="<?php echo $appt_times['fp4']; ?>" size="6" />&nbsp;mins</div>
							<div class="section_form_20"><input name="fp5time" type="text" id="fp5time" value="<?php echo $appt_times['fp5']; ?>" size="6" />&nbsp;mins</div>
							<div class="clear"></div>
							<div class="section_form_20"><strong>Photos Only</strong></div>
							<div class="section_form_20"><strong>Zillow 3d</strong></div>
							...
							<div class="clear"></div>
							<div class="section_form_20"><input name="phototime" type="text" id="phototime" value="<?php echo $appt_times['photos']; ?>" size="6" />&nbsp;mins</div>
							<div class="section_form_20"><input name="zillowtime" type="text" id="zillowtime" value="<?php echo $appt_times['zillow_3d']; ?>" size="6" />&nbsp;mins</div>
							<div class="section_form_20"><input name="aphotostime" type="text" id="aphotostime" value="<?php echo $appt_times['aerial_photos']; ?>" size="6" />&nbsp;mins</div>
							<div class="section_form_20"><input name="avideotime" type="text" id="avideotime" value="<?php echo $appt_times['aerial_vid']; ?>" size="6" />&nbsp;mins</div>
							<div class="section_form_20"><input name="drawtime" type="text" id="drawtime" value="<?php echo $appt_times['draw']; ?>" size="6" />&nbsp;mins</div>
							<div class="clear"></div>
						</div>
						<div class="section_footer">
							<div class="section_navigate">
								<input name="appt_id" type="hidden" value="<?php echo $appt_dur_id; ?>" />
							</div>
							<div class="section_submit">
								<input class="section_submit_button_md" name="Submittime" type="submit" id="Submittime" value="Submit" />
							</div>
							<div class ="clear"></div>
						</div>
						<?php if($appt_dur_level=="dealer"){ 
							if(!($appt_da_sel = mysqli_query($link,"SELECT * FROM users WHERE level = 'd_assist' AND primary_deal = '$appt_dur_id' ORDER BY lastname ASC"))){
								printf("%s", sprintf("internal error %d:%s\n", mysqli_errno($link), mysqli_error($link)));
								exit();
							
							}
							$num_da = mysqli_num_rows($appt_da_sel);
							if($num_da!="0") {
						?>
						<div class="section_titles">
							<div class="section_full"?>
						...
						<?php }
							while($da_appt_list=mysqli_fetch_array($appt_da_sel)){ 
						?>
						<div class="section_result_row">
							<a href="appt_edit.php?uid=<?php echo $da_appt_list['id']; ?>">
								<div class="section_full">
									<?php echo $da_appt_list['lastname'].", ".$da_appt_list['firstname']; ?>
								</div>
							</a>
						</div>
						<?php } } ?>
