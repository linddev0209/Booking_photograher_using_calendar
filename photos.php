...
<html>
	<head>
		...
		<title>Upload Photos | Look2 Home Marketing</title>
...
<body>
				...
				<div class="section_container">
					...
					<form action="upload_photos.php" method="post" enctype="multipart/form-data">
						<div class="section_result_row">
							<div class="section_form_10"><strong>Floor Plan:</strong></div>
							<div class="section_form_36">
								<?php if(!($floorplan_db = mysqli_query($link,"SELECT DISTINCT * FROM floorplans where order_id = '$order_id' ORDER BY id ASC"))){
									printf("%s", sprintf("internal error %d:%s\n", mysqli_errno($link), mysqli_error($link))); 
									exit();
								} ?>
							...
							<div class="clear"></div>
							<div class="section_form_10">&nbsp;</div>
							<div class="clear"></div>
							<div class="dropzone" id="myDropzone"></div>
						</div>
						<div class="section_footer">
							<div class="section_full_ra"><input name="order_id" type="hidden" id="order_id" value="<?php echo $order_id; ?>" />
								<input class="section_submit_button_md" name="submit-all" type="submit" id="submit-all" value="Upload Photo(s)" />
							</div>
						</div>
					</form>
					<script>
						Dropzone.options.myDropzone= {
							...
							init: function() {
    							...
    							this.on("queuecomplete", function(file, serverFileName) {
        							window.location = 'photos.php?order_id=<?php echo $order_id; ?>';
	...
	<script type="text/javascript">
		...
		const lc = document.getElementById('left_column');
		const rc = document.getElementById('right_column');

		onresize(rc, function () {
