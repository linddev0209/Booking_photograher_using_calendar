...
if(!($p_count = mysqli_query($link,"SELECT DISTINCT * FROM orders WHERE status = 'pending' AND assigned = '$dealer_id'"))){
...
if(!($s_count = mysqli_query($link,"SELECT DISTINCT * FROM orders WHERE status = 'scheduled' AND assigned = '$dealer_id'"))){
	printf("%s", sprintf("internal error %d:%s\n", mysqli_errno($link), mysqli_error($link)));
	exit();
}
...
if(!($r_count = mysqli_query($link,"SELECT DISTINCT * FROM orders WHERE status = 'received' AND assigned = '$dealer_id'"))){
	printf("%s", sprintf("internal error %d:%s\n", mysqli_errno($link), mysqli_error($link)));
	exit();
}
...
<html>
	<head>
		...
		<title>Dealer Dashboard | Look2 Home Marketing</title>
		...
		<div id="right_column">
			...
			<div id="rc_content_container">
				<div class="section_container">
					...
					<?php
						if(!($p_order = mysqli_query($link,"SELECT * FROM orders where orders.status = 'pending' AND orders.assigned = '$dealer_id' ORDER BY ordered ASC LIMIT $p_offset, $totpage"))){
							printf("%s", sprintf("internal error %d:%s\n", mysqli_errno($link), mysqli_error($link)));
							exit();
						}
				...
				<div class="section_container">
					...
					<?php 
						if(!($s_order = mysqli_query($link,"SELECT * FROM orders where orders.status = 'scheduled' AND orders.assigned = '$dealer_id' ORDER BY scheduled ASC LIMIT $s_offset, $totpage"))) {
						printf("%s", sprintf("internal error %d:%s\n", mysqli_errno($link), mysqli_error($link)));
						exit();
						}
						...
						<div class="section_result_row">
							<a href="schedule.php?order_id=<?php echo $sched_order['id']; ?>">
								...
								<div class="section_photographer">
									<?php 
									if($agent_sid!="0") {
										if(!($as_schedorder = mysqli_query($link,"SELECT firstname, lastname FROM users where id = '$agent_sid'"))){
											printf("%s", sprintf("internal error %d:%s\n", mysqli_errno($link), mysqli_error($link)));
											exit();
										} 
										$s_agent = mysqli_fetch_array($as_schedorder);  echo $s_agent['lastname'].", ".$s_agent['firstname'];
									} else {
										echo "&nbsp;";
									}
								?>
				...
				<div class="section_container">
					...
					<?php if(!($r_order = mysqli_query($link,"SELECT * FROM orders where orders.status = 'received' AND orders.assigned = '$dealer_id' ORDER BY ordered ASC LIMIT $r_offset, $totpage"))){
						printf("%s", sprintf("internal error %d:%s\n", mysqli_errno($link), mysqli_error($link)));
						exit();
					}
					while($received_order = mysqli_fetch_array($r_order)){
						...
						$photographer_rid = $received_order['p_assist'];
						$orig_rec = strtotime($received_date);
					?>
						<div class="section_result_row">
							<a href="schedule.php?order_id=<?php echo $received_order['id']; ?>">
								<div class="section_date">
								...
								<div class="section_location">
									<?php if($photographer_rid!="0") {
											if(!($p_recorder = mysqli_query($link,"SELECT firstname, lastname FROM users where id = '$photographer_rid'"))){
												printf("%s", sprintf("internal error %d:%s\n", mysqli_errno($link), mysqli_error($link)));
												exit();
											} 
											$r_photographer = mysqli_fetch_array($p_recorder);  
											echo $r_photographer['lastname'].", ".$r_photographer['firstname'];
										} else {
											echo "&nbsp;";
										}
									?>
					...
					<?php } if($r_records=="0") { ?>
						<div class="section_result_row">
							<span class="no_results">No orders in processing were found</span>
						</div>
					<?php } ?>
					<div class="section_footer">
						<div class="section_navigate">
