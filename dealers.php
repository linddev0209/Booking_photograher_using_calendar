...
if($_GET['st']!="") { 
	$dealer_st = $_GET['st'];	
	$letter_menu = '<a href="dealers.php?st='.$dealer_st.'">All</a>';
	...
	if($num_count!="0") { 
		$letter_menu .= '<a href="dealers.php?st='.$dealer_st.'&letter=num">#</a>'; 
	} else { 
...
foreach($letter_sort as $letter){
	if($_GET['st']!="") { 
		...
		if($letter_count!="0") { 
			$letter_menu .= '<a href="dealers.php?st='.$dealer_st.'&letter='.$letter.'">'.$letter.'</a>'; 
		} else { 
...
if ($_GET['task'] == "remdealer"){
	if(!($dealer_rem = mysqli_query($link,"DELETE FROM users WHERE id = '$dealer_id'"))){
				printf("%s", sprintf("internal error %d:%s\n", mysqli_errno($link), mysqli_error($link)));
				exit();
			}
...
<html>
	<head>
		...
		<title>Dealers | Look2 Home Marketing</title>
		...
		<div id="right_column">
			...
			<div id="rc_content_container">
				<div class="section_full_ra">
					<a href="dealer_edit.php">Add New Dealer</a>	
				</div>
				<div class="section_container">
					...
					<div class="section_header">
						<svg xmlns="http://www.w3.org/2000/svg" class="section_icon" viewBox="0 0 36 36"><circle cx="16.86" cy="9.73" r="6.46"/><path d="M21 28h7v1.4h-7zm-6 2v3a1 1 0 0 0 1 1h17a1 1 0 0 0 1-1V23a1 1 0 0 0-1-1h-7v-1.47a1 1 0 0 0-2 0V22h-2v-3.58a32.12 32.12 0 0 0-5.14-.42 26 26 0 0 0-11 2.39 3.28 3.28 0 0 0-1.88 3V30zm17 2H17v-8h7v.42a1 1 0 0 0 2 0V24h6z"/></svg><h1>Dealer Accounts - Select State</h1>
					</div>
					<?php 
						...
						while($dealer_states=mysqli_fetch_array($state_sel)){ 
					?>
					<div class="section_result_row">
						<a href="dealers.php?st=<?php echo $dealer_states['state']; ?>">
							<div class="section_full">
								<?php $st = strtoupper($dealer_states['state']); echo $state_list[$st]; ?>
							</div>
						</a>
					</div>
					<?php } } else if($_GET['st']!="") { $dealer_st = $_GET['st']; ?>
					<div class="section_header">
						<svg xmlns="http://www.w3.org/2000/svg" class="section_icon" viewBox="0 0 36 36"><circle cx="16.86" cy="9.73" r="6.46"/><path d="M21 28h7v1.4h-7zm-6 2v3a1 1 0 0 0 1 1h17a1 1 0 0 0 1-1V23a1 1 0 0 0-1-1h-7v-1.47a1 1 0 0 0-2 0V22h-2v-3.58a32.12 32.12 0 0 0-5.14-.42 26 26 0 0 0-11 2.39 3.28 3.28 0 0 0-1.88 3V30zm17 2H17v-8h7v.42a1 1 0 0 0 2 0V24h6z"/></svg><h1>Dealer Accounts - Select Dealer</h1>
					</div>
					...
					<?php 
						if($_GET['letter']!="") {
							...
							if($dealer_letter!="num"){
								if(!($dealer_sel = mysqli_query($link,"SELECT id, firstname, lastname, cell_phone, email FROM users WHERE lastname LIKE '".$dealer_letter."%' AND state = '$dealer_st' AND level = 'dealer' ORDER BY lastname ASC"))){
									printf("%s", sprintf("internal error %d:%s\n", mysqli_errno($link), mysqli_error($link)));
									exit();
								} 
							} else {
								if(!($dealer_sel = mysqli_query($link,"SELECT id, firstname, lastname, cell_phone, email FROM users WHERE lastname REGEXP '^[0-9]' AND state = '$dealer_st' AND level = 'dealer' ORDER BY lastname ASC"))){
									printf("%s", sprintf("internal error %d:%s\n", mysqli_errno($link), mysqli_error($link)));
									exit();
								} 	
							}
						} else {
							if(!($dealer_sel = mysqli_query($link,"SELECT id, firstname, lastname, cell_phone, email FROM users WHERE level = 'dealer' AND state = '$dealer_st' ORDER BY lastname ASC"))){
								printf("%s", sprintf("internal error %d:%s\n", mysqli_errno($link), mysqli_error($link)));
								exit();
							}		
						}
						$num_dealers = mysqli_num_rows($dealer_sel);														
						while($select_dealer=mysqli_fetch_array($dealer_sel)) { 
					?>
					<div class="section_result_row">
						<a href="dealer_edit.php?dealer=<?php echo $select_dealer['id']; ?>">
							<div class="section_sel_company">
								<?php echo $select_dealer['lastname'].", ".$select_dealer['firstname']; ?>
							</div>
							<div class="section_phone">
								<?php echo $select_dealer['cell_phone']; ?>
							</div>							
						</a>
						<div class="section_phone">
							&nbsp;
						...
						<div class="section_edit">
							<a class="remove_rec" href="dealers.php?dealer=<?php echo $select_dealer['id']; ?>&task=remage" onClick="confirmCleanUp('dealer - <?php echo $select_dealer["lastname"].", ".$select_dealer["firstname"]; ?>'); return returnvalue"><svg xmlns="http://www.w3.org/2000/svg" class="section_icon" viewBox="0 0 800 800"><path d="M800 400c0 220.9-179.1 400-400 400S0 620.9 0 400 179.1 0 400 0s400 179.1 400 400zm-560-40c-22.1 0-40 17.9-40 40s17.9 40 40 40h320c22.1 0 40-17.9 40-40s-17.9-40-40-40H240z" fill-rule="evenodd"/></svg>Remove</a>
						</div>
						<div class="clear"></div>
					</div>
					<?php } } if($num_dealers=="0") { ?>
						<div class="section_result_row">
							<span class="no_results">No dealers found</span>
						</div>
					<?php } ?>
				</div>
			</div>
