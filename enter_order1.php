...
if($_POST['SubmitOrder']){
	...
	$s_name = filter_var($_POST['street_name'], FILTER_SANITIZE_STRING);
	...
	$instruct = filter_var($_POST['instruct'], FILTER_SANITIZE_STRING);
	$ord_id = $_POST['ord_id'];

	
	if($ord_id!="") {
		if(!($update_order = mysqli_query($link,"UPDATE orders SET company_id = '$comp_id', agent_id = '$age_id', assigned ='$assigned_id', status = '$status', disp_area = '$disp_area', instruct = '$instruct', pets = '$pets', sqft = '$a_sqft', owner = '$owner', owner_email = '$o_email', cell_phone1 = '$c_phone1', street_name = '$s_name', unit = '$unit', city = '$city', state = '$state', zip = '$zip', prop_type = '$prop_type', price = '$price', latitude = '$lat', longitude = '$lng', vacant = '$vacant', lockbox = '$lockbox' WHERE id = '$ord_id'"))){
				printf("%s", sprintf("internal error %d:%s\n", mysqli_errno($link), mysqli_error($link)));
				exit();
		}
		...
		if(!($insert_order = mysqli_query($link,"INSERT INTO orders SET company_id = '$comp_id', agent_id = '$age_id', assigned ='$assigned_id', ordered = '$order_date', status = '$status', disp_area = '$disp_area', instruct = '$instruct', pets = '$pets', sqft = '$a_sqft', owner = '$owner', owner_email = '$o_email', cell_phone1 = '$c_phone1', street_name = '$s_name', unit = '$unit', city = '$city', state = '$state', zip = '$zip', prop_type = '$prop_type', price = '$price', latitude = '$lat', longitude = '$lng', vacant = '$vacant', lockbox = '$lockbox'"))){			
				printf("%s", sprintf("internal error %d:%s\n", mysqli_errno($link), mysqli_error($link)));
				exit();
				
		}
		
		$ord_id = mysqli_insert_id($link); 
...
<html>
	<head>
		...
		<script src="scripts/address_complete.js" type="text/javascript"></script>
...
<body>
		...
		<div id="right_column">
			...
			<div id="rc_content_container">
				<div class="section_container">
					...
					<?php if($_SESSION['SESS_LEVEL']=="admin" || $_SESSION['SESS_LEVEL']=="staff" || $_SESSION['SESS_LEVEL']=="dealer") { ?>
						...
						<div class="section_result_row">
							<div class="section_half">
							 	<strong><?php echo $agents['lastname'].", ".$agents['firstname']; ?></strong><br />Office: <? echo $agents['office_phone']; ?>&nbsp;&nbsp;&nbsp;|&nbsp;&nbsp;Cell: <?php echo $agents['cell_phone']; ?><br /><a href="mailto:<?php echo $agents['email']; ?>"><?php echo $agents['email']; ?></a><br /><br />Company: <?php echo $agents['c_name']." - ".$agents['c_city']." ".$agents['c_state']; ?>
							</div>
							<div class="section_half">
								<span class="agent_pic_order_80">
									<?php if(file_exists("floorplan/profile/".$agent_id.".jpg")){ ?><img src="floorplan/profile/<?php echo $agent_id; ?>.jpg" width="80px" alt="<?php echo $agents['firstname']." ". $agents['lastname']; ?>"><?php } ?>
								</span>
							</div>
							<div class="clear"></div>
						</div>
					...
					<form action="enter_order1.php" method="post" enctype="multipart/form-data">
						<div class="section_result_row">
							<div class="section_form_10">Address:</div>
							<div class="section_form_36">
								<?php if($order_id!="") { if($order_deat['street_num']!="0" ) {?>
           	      					<input name="street_name" type="text" id="street_name" size="38" value="<?php echo $order_deat['street_num']; ?> <?php echo $order_deat['street_dir']; ?> <?php echo $order_deat['street_name']; ?>" /><? } else { ?><input name="street_name" type="text" size="38" id="street_name" value="<?php echo $order_deat['street_name']; ?>" /><? } } else { ?><input name="street_name" type="text" size="38" id="street_name" value="<?php echo $order_deat['street_name']; ?>" />
								<? } ?>
							</div>
							<div class="section_form_10">Apt#/Unit:</div>
							<div class="section_form_17"><input name="unit" type="text" id="unit" value="<?php echo $order_deat['unit']; ?>" size="8" maxlength="4"/></div>
							<div class="section_form_10">Property Type:</div>
							<div class="section_form_17">
								<select name="prop_type" id="prop_type">
									<option value="r" selected="selected">Residential</option>
									<option value="c" <?php if($order_deat['prop_type']=="c") { echo 'selected="selected"'; } ?>>Commercial</option>
								</select>
							</div>
							<div class="clear"></div>
							<div class="section_form_10">City:</div>
							<div class="section_form_36">
								<input name="city" type="text" id="city" value="<?php echo $order_deat['city']; ?>" />
								<input name="lat" type="hidden" id="lat" value="<?php echo $order_deat['latitude']; ?>" />
								<input name="lng" type="hidden"id="lng" value="<?php echo $order_deat['longitude']; ?>" />
							</div>
							<div class="section_form_10">State:</div>
							<div class="section_form_17"><input name="state" type="text" id="state" value="<?php echo $order_deat['state']; ?>" size="4" maxlength="2" /></div>
							<div class="section_form_10">Zip:</div>
							<div class="section_form_17">
								<input name="zip" type="text" id="zip" value="<?php echo $order_deat['zip']; ?>" size="10" maxlength="5" />
							</div>
							<div class="clear"></div>
							<div class="section_form_10">Approx. SQFT:</div>
							<div class="section_form_17"><input name="sqft" type="text" id="sqft" value="<?php echo $order_deat['sqft']; ?>" size="10" maxlength="5" /></div>
							<div class="section_form_6">Vacant:</div>
							<div class="section_form_17">
								<input name="vacant" type="radio" id="vacant" value="y" checked="checked" />Yes&nbsp;&nbsp;<input type="radio" name="vacant" id="vacant" value="n" <?php if($order_deat['vacant']=="n") echo "checked"; ?> />No
							</div>
							<div class="section_form_6">Pet(s):</div>
							<div class="section_form_17">
								<input name="pets" type="radio" id="pets" value="y" <?php if($order_deat['pets']=="y") echo "checked"; ?> />Yes&nbsp;&nbsp;<input name="pets" type="radio" id="pets" value="n" <?php if($order_deat['pets']!="y") echo "checked"; ?> />No
							</div>
							<div class="section_form_6">Lockbox:</div>
							<div class="section_form_17">
								<input name="lockbox" type="radio" id="lockbox" value="y" checked="checked" />Yes&nbsp;&nbsp;<input name="lockbox" type="radio" id="lockbox" value="n" <?php if($order_deat['lockbox']=="n") echo "checked"; ?> />No
							</div>
							<div class="clear"></div>
							...
							<div class="clear"></div>
							<div class="section_form_17">
								List Price<br /><em><strong>*Required for syndication*</strong></em>
							</div>
							<div class="section_form_24">
								<input name="list_price" type="text" id="list_price" value="<?php echo $order_deat['price']; ?>" /><br />ex. 159000 (not 159,000)
							</div>
							<div class="section_form_17">
								<input name="disp_area" type="checkbox" id="disp_area" value="y" <?php if($order_deat['disp_area']=="y") echo "checked"; ?> /><em>Don't display AREA online</em>
							</div>
							<div class="clear"></div>
							<div class="section_form_24">
								&nbsp;
							...
							<div class="section_form_24">
								<input name="owner" type="text" id="owner" value="<?php echo $order_deat['owner']; ?>" />
							</div>
							<div class="section_form_10">
								Owner Email
							</div>
							<div class="section_form_24">
								<input name="o_email" type="text" id="o_email" value="<?php echo $order_deat['owner_email']; ?>" />
							</div>
							<div class="section_form_8">
								Owner Phone
							</div>
							<div class="section_form_24">
								<input name="c_phone1" type="text" id="c_phone1" value="<?php echo $order_deat['cell_phone1']; ?>" size="24" maxlength="12" /><br />ex. 555-555-5555
							</div>
							<div class="clear"></div>
							<div class="section_form_17">
	...
	<script async
    	src="https://maps.googleapis.com/maps/api/js?key=AIzaSyByk08lPz3tTnvnxW-pLZJcUWcXw-zK7KI&libraries=places&callback=initAutocomplete">
	</script>
</body>
</html>