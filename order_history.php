...
if($c_id!="") {
	if(!($h_order = mysqli_query($link,"SELECT * FROM orders WHERE company_id = '$c_id' ORDER BY ordered DESC LIMIT $offset, $totpage"))){
		printf("%s", sprintf("internal error %d:%s\n", mysqli_errno($link), mysqli_error($link)));
		exit();
	}
	if(!($h_count = mysqli_query($link,"SELECT DISTINCT * FROM orders WHERE company_id = '$c_id' "))){
		printf("%s", sprintf("internal error %d:%s\n", mysqli_errno($link), mysqli_error($link)));
		exit();
	}

	$records = mysqli_num_rows($h_count);
} else {
	if(!($h_order = mysqli_query($link,"SELECT * FROM orders WHERE agent_id = '$a_id' ORDER BY ordered DESC LIMIT $offset, $totpage"))){
		printf("%s", sprintf("internal error %d:%s\n", mysqli_errno($link), mysqli_error($link)));
		exit();
	}
	if(!($h_count = mysqli_query($link,"SELECT DISTINCT * FROM orders WHERE agent_id = '$a_id'"))){
		printf("%s", sprintf("internal error %d:%s\n", mysqli_errno($link), mysqli_error($link)));
		exit();
	}
...
if ($_GET['task'] == "expire"){
	if($c_id!="") { 
		$order_nav = "company=".$c_id;
	} else {
		$order_nav = "agent=".$a_id;
	}
	$order_id = $_GET['oid'];
	if(!($order_change = mysqli_query($link,"UPDATE orders set status = 'expired' WHERE id = '$order_id'"))){
		printf("%s", sprintf("internal error %d:%s\n", mysqli_errno($link), mysqli_error($link)));
		exit();
	}
	header("Location: ".$_SERVER['PHP_SELF']."?".$order_nav);
}
...
if ($_GET['task'] == "relist"){
	...
	if(!($order_change = mysqli_query($link,"UPDATE orders set status = 'complete' WHERE id = '$order_id'"))){
		printf("%s", sprintf("internal error %d:%s\n", mysqli_errno($link), mysqli_error($link)));
		exit();
	}
	header("Location: ".$_SERVER['PHP_SELF']."?".$order_nav);
...
<html>
	<head>
		...
		<title>Order History | Look2 Home Marketing</title>
			...
			<div id="rc_content_container">
				<div class="section_container">
					<div class="section_header">
						...
						<h1>Order History - <?php if($c_id!="") { 
							if(!($company_sel = mysqli_query($link,"SELECT * FROM companies WHERE id = '$c_id'"))){
								printf("%s", sprintf("internal error %d:%s\n", mysqli_errno($link), mysqli_error($link)));
								exit();
							}
							$comp_detail = mysqli_fetch_array($company_sel); echo $comp_detail['name'];  }  
							if($a_id!="") { 
								if(!($agent_db = mysqli_query($link,"SELECT * FROM users WHERE id = '$a_id'"))){
									printf("%s", sprintf("internal error %d:%s\n", mysqli_errno($link), mysqli_error($link)));
									exit();
							}
							$agents = mysqli_fetch_array($agent_db);
							$agent_name = $agents['lastname'].", ".$agents['firstname']; ?><?php echo $agent_name; ?><?php } ?>
						</h1>
					</div>
					<?php 
						for($i=0; $i<mysqli_num_rows($h_order)/2; $i++) { 
					?>
						<div class="section_order_history">
							<?php 
								$list_history = mysqli_fetch_array($h_order); 
								$history_id = $list_history['id'];
									if(!($e_photo = mysqli_query($link,"SELECT * FROM photos where order_id = '$history_id' ORDER BY fp_id, sort ASC LIMIT 1"))){
										printf("%s", sprintf("internal error %d:%s\n", mysqli_errno($link), mysqli_error($link)));
										exit();
									}
								...
								if($list_history['street_num']=="0") {
    								$map_num = ""; 
								} else { 
    								$map_num = $list_history['street_num']." "; 
								}

								if($list_history['city']=="") {
									$order_city = "";
								} else {
									$order_city = $list_history['city'];
									$order_city_display = $list_history['city'].",";
								}

								if($list_history['state']=="") {
									$order_state = "";
								} else {
									$order_state = $list_history['state'];
								}

								if($list_history['zip']=="0") {
									$order_zip = "";
								} else {
									$order_zip = $list_history['zip'];
								}
							
								$map_address = "<b>".$map_num."".$list_history['street_name']."</b><br>".$order_city.", ".$order_state." ".$order_zip;

								if($list_history['longitude']!="0") {
    								$lat= $list_history['latitude']; 
    								$long = $list_history['longitude'];
    							} else { 
							...
							?>
							<div class="section_order_header">
								<div class="section_order_header_address">
									<h1>
										<?php if($list_history['street_num']!="0") { echo $list_history['street_num']; } ?> <?php if($list_history['street_dir']!="") { echo $list_history['street_dir']; } ?> <?php echo $list_history['street_name']; ?>&nbsp;<?php if($list_history['unit']!="") { echo $list_history['unit']; } ?>
									</h1><?php echo $list_history['city']; ?>, <?php echo $list_history['state']; ?> <?php echo $list_history['zip']; ?>
								</div>
								<?php if(file_exists("floorplan/".$history_id."/photos/".$history_id."_".$fp_assigned."_".$list_photo['id'].".jpg")) { ?><img class="history_order" src="includes/rezor.php?src=floorplan/<?php echo $history_id; ?>/photos/<?php echo $history_id; ?>_<?php echo $fp_assigned; ?>_<?php echo $list_photo['id']; ?>.jpg&amp;w=600&amp;zc=1&amp;q=90" alt="Order#<?php echo $history_id; ?>" /><?php } else { ?><div id="maps<?php echo $history_id; ?>" class="maps" data-gps="<?php echo $lat; ?>,<?php echo $long; ?>"></div><?php } ?>
								<div class="section_order_header_status" <?php if($list_history['status']=="complete") { echo'style="background: #0085B2;"'; } if($list_history['status']=="received") { echo'style="background: #7DB366;"'; } if($list_history['status']=="scheduled") { echo'style="background: #7d0052;"'; } if($list_history['status']=="pending") { echo'style="background: #b29300;"'; } if($list_history['status']=="archived") { echo'style="background: #333;"'; } if($list_history['status']=="canceled") { echo'style="background: #cd232b;"'; } if($list_history['status']=="expired") { echo'style="background: #666633;"'; } ?>>
									<?php echo ucfirst($list_history['status']); ?>
								</div>
							</div>
							<div class="section_order_links">
								<?php if($list_history['status']=="complete") { ?>
									<div class="section_order_links_row">
										<a href="<?php echo $_SERVER["PHP_SELF"]; ?>?task=expire&amp;oid=<?php echo $list_history['id']; ?>&amp;<?php echo $order_nav;?>"><svg xmlns="http://www.w3.org/2000/svg" class="section_icon" viewBox="0 0 800 800"><path d="M193.7 0h-79.4v38.1h57.1c-12.5 27.8-19 57.9-19 88.3 0 81.9 56.3 163.6 135 232.1 13.4 9.9 22 24.9 23.6 41.5-1.7 16.5-10.2 31.6-23.6 41.5-78.7 68.5-135 150.1-135 232 0 30.5 6.5 60.6 19 88.4h-57.1V800h571.4v-38.1H628c12.8-27.7 19.5-57.8 19.7-88.3 0-81.9-56.3-163.6-135-232.1-13.4-9.9-22-24.9-23.6-41.5 1.7-16.5 10.2-31.6 23.6-41.5 78.7-68.5 135-150.1 135-232-.1-30.5-6.8-60.7-19.7-88.4h57.8V0H193.7zm392.1 38.1c15.5 26.9 23.7 57.3 23.7 88.4 0 62.7-43.3 134.9-121.9 203.3-22.2 16.7-35.6 42.5-36.8 70.2a92.47 92.47 0 0 0 36.8 70.2c78.6 68.4 121.9 140.6 121.9 203.3a176.51 176.51 0 0 1-23.7 88.3H214.2a178.29 178.29 0 0 1-23.8-88.4c0-62.7 43.3-134.9 121.9-203.3 22.2-16.7 35.6-42.5 36.8-70.2a92.47 92.47 0 0 0-36.8-70.2c-78.6-68.4-121.9-140.6-121.9-203.3.1-31 8.3-61.5 23.7-88.4h371.7zM232.5 644.8c7.6-29.7 25.9-55.5 51.4-72.4l98.8-65.6c10.5-7 24.2-7 34.7 0l98.8 65.6c25.5 16.9 43.8 42.7 51.4 72.4 2.5 9.4 3.7 19.1 3.8 28.8 0 17.2-3.2 34.2-9.5 50.2H238.1c-6.2-16-9.4-33.1-9.5-50.3.1-9.7 1.4-19.4 3.9-28.7h0z"/></svg>Expire</a>
									</div>
									<div class="section_order_links_row">
										<a href="archive.php?oid=<?php echo $list_history['id']; ?>&amp;<?php echo $order_nav;?>"><svg xmlns="http://www.w3.org/2000/svg" class="section_icon" viewBox="0 0 800 800"><path d="M690 191.7c0-3.7-1.2-7.3-3.7-10.2L532.1 5.4l-.1-.1c-.9-1-2-1.9-3.1-2.6-.3-.2-.7-.4-1-.6-1-.5-2-1-3.1-1.3-.3-.1-.6-.2-.8-.3-1.2-.3-2.4-.4-3.6-.4H141.2c-17.3 0-31.4 14.1-31.4 31.4v737.3c0 17.3 14.1 31.4 31.4 31.4h517.6c17.3 0 31.4-14.1 31.4-31.4V193.5c0-.6-.1-1.2-.2-1.8zM353.8 760.3H205.9v-23.1l90.6-130.4v-1.1h-82.2v-35.6h138.1v24.8l-88.7 128.7v1.1h90.1v35.6h0zm70.3 0h-43.2V570.1h43.2v190.2zm156.4-87.2c-14.7 13.8-36.4 20-61.8 20-5.7 0-10.7-.3-14.7-.8v68h-42.6V572.6c13.3-2.3 31.9-4 58.1-4 26.5 0 45.4 5.1 58.2 15.2 12.1 9.6 20.3 25.4 20.3 44 .1 18.8-6.2 34.6-17.5 45.3zM141.2 532.7V31.4h187.6v-4.9h47.6v4.9h128.2v160.5c0 8.7 7 15.7 15.7 15.7h138.6v325.1H141.2zm380.6 68.4c-8.8 0-14.7.9-17.8 1.7V659c3.7.8 8.2 1.1 14.4 1.1 22.9 0 37-11.6 37-31.1 0-17.4-12.1-27.9-33.6-27.9zM376.4 58H424v19.6h-47.6z"/><path d="M328.8 31.4h47.6v14.7h-47.6zm0 56h47.6V107h-47.6zm47.6 32.1H424v19.6h-47.6zm0 62H424v19.6h-47.6zM328.8 150h47.6v19.6h-47.6zm47.6 67.2c-33.7 0-49.7 27.3-49.7 61l-11.3 102.2c0 33.7 27.3 61 61 61s61-27.3 61-61l-11.2-102.2c-.1-33.7-16.2-61-49.8-61zm20.2 197.4h-40.4v-85.3h40.4v85.3z"/></svg>Archive</a>
									</div>
									<?php if ($_SESSION['SESS_LEVEL']=="admin" || $_SESSION['SESS_LEVEL']=="staff") { ?> 
										<div class="section_order_links_row">
											<a href="complete.php?order_id=<?php echo $list_history['id']; ?>"><svg xmlns="http://www.w3.org/2000/svg" class="section_icon" viewBox="0 0 800 800"><path d="M741.3 8.4l-711 264.8-13.9 9.1c-13.9 14.3-13.6 37.1.7 51l273.6 132.8 185-190.1c14.3-13.4 36.4-13.7 51-.7 14.3 13.9 14.6 36.7.7 51l-185 190.1 140.2 269.9c14.3 13.9 37.1 13.6 51-.7l8.7-14.2L787.9 53.7c6.3-18.9-4-39.3-22.9-45.6-7.7-2.6-16.1-2.5-23.7.3z"/></svg>Resend</a>
										</div>
									<?php } ?>
								<?php } if($list_history['status']=="expired") { ?>
									<div class="section_order_links_row">
										<a href="<?php echo $_SERVER["PHP_SELF"]; ?>?task=relist&amp;oid=<?php echo $list_history['id']; ?>&amp;<?php echo $order_nav;?>"><svg xmlns="http://www.w3.org/2000/svg" class="section_icon" viewBox="0 0 800 800"><path d="M404.9 103V12.1c0-4.5-2.5-8.5-6.5-10.7-4-2.1-8.8-1.9-12.5.6l-251 169c-3.4 2.3-5.3 6-5.3 10.1 0 4 1.9 7.8 5.3 10.1L385.8 360c3.8 2.5 8.5 2.7 12.5.7 3.9-2.2 6.5-6.3 6.5-10.8v-97.7c39.5.3 72.1 2 99.4 5.2 130.5 15.5 165.5 93.9 166.9 97.2 1.8 4.5 6.4 7.4 11.2 7.4.7 0 1.6 0 2.3-.2 5.6-1.1 9.7-6.1 9.7-11.9 0-197.7-234.9-239.8-289.4-246.9zM665 608.8L414 439.9c-3.6-2.5-8.4-2.8-12.5-.8-3.9 2.2-6.4 6.3-6.4 10.8v97.7c-39.6-.3-72.1-2-99.3-5.3-130.6-15.5-165.5-93.8-166.9-97.2-1.9-4.5-6.4-7.4-11.2-7.4-.8 0-1.6 0-2.4.2-5.6 1.1-9.7 6.1-9.7 11.9 0 197.8 234.9 240 289.5 247.1v90.9c0 4.4 2.4 8.5 6.5 10.7 4 2.1 8.8 1.9 12.5-.6L665 629c3.4-2.3 5.3-6 5.3-10.1.1-4-1.9-7.9-5.3-10.1z"/></svg>Relist</a>
									</div>
									<div class="section_order_links_row">
										<a href="archive.php?oid=<?php echo $list_history['id']; ?>&amp;<?php echo $order_nav;?>"><svg xmlns="http://www.w3.org/2000/svg" class="section_icon" viewBox="0 0 800 800"><path d="M690 191.7c0-3.7-1.2-7.3-3.7-10.2L532.1 5.4l-.1-.1c-.9-1-2-1.9-3.1-2.6-.3-.2-.7-.4-1-.6-1-.5-2-1-3.1-1.3-.3-.1-.6-.2-.8-.3-1.2-.3-2.4-.4-3.6-.4H141.2c-17.3 0-31.4 14.1-31.4 31.4v737.3c0 17.3 14.1 31.4 31.4 31.4h517.6c17.3 0 31.4-14.1 31.4-31.4V193.5c0-.6-.1-1.2-.2-1.8zM353.8 760.3H205.9v-23.1l90.6-130.4v-1.1h-82.2v-35.6h138.1v24.8l-88.7 128.7v1.1h90.1v35.6h0zm70.3 0h-43.2V570.1h43.2v190.2zm156.4-87.2c-14.7 13.8-36.4 20-61.8 20-5.7 0-10.7-.3-14.7-.8v68h-42.6V572.6c13.3-2.3 31.9-4 58.1-4 26.5 0 45.4 5.1 58.2 15.2 12.1 9.6 20.3 25.4 20.3 44 .1 18.8-6.2 34.6-17.5 45.3zM141.2 532.7V31.4h187.6v-4.9h47.6v4.9h128.2v160.5c0 8.7 7 15.7 15.7 15.7h138.6v325.1H141.2zm380.6 68.4c-8.8 0-14.7.9-17.8 1.7V659c3.7.8 8.2 1.1 14.4 1.1 22.9 0 37-11.6 37-31.1 0-17.4-12.1-27.9-33.6-27.9zM376.4 58H424v19.6h-47.6z"/><path d="M328.8 31.4h47.6v14.7h-47.6zm0 56h47.6V107h-47.6zm47.6 32.1H424v19.6h-47.6zm0 62H424v19.6h-47.6zM328.8 150h47.6v19.6h-47.6zm47.6 67.2c-33.7 0-49.7 27.3-49.7 61l-11.3 102.2c0 33.7 27.3 61 61 61s61-27.3 61-61l-11.2-102.2c-.1-33.7-16.2-61-49.8-61zm20.2 197.4h-40.4v-85.3h40.4v85.3z"/></svg>Archive</a>
									</div>
							...
							<div class="clear"></div>
							<a href="order_detail.php?order_id=<?php echo $list_history['id']; ?>">
								<div class="section_order_foot">
									View Order
								</div>
							</a>
						</div>
						<?php $list_history = mysqli_fetch_array($h_order); if($list_history!="") { ?>
							<div class="section_order_history">
								<?php 
									$history_id = $list_history['id'];
										if(!($e_photo = mysqli_query($link,"SELECT * FROM photos where order_id = '$history_id' ORDER BY fp_id, sort ASC LIMIT 1"))){
								...
								<div class="section_order_header">
									<div class="section_order_header_address">
										...
										<?php echo $list_history['city']; ?>, <?php echo $list_history['state']; ?> <?php echo $list_history['zip']; ?>
									</div>
									<?php if(file_exists("floorplan/".$history_id."/photos/".$history_id."_".$fp_assigned."_".$list_photo['id'].".jpg")) { ?><img class="history_order" src="includes/rezor.php?src=floorplan/<?php echo $history_id; ?>/photos/<?php echo $history_id; ?>_<?php echo $fp_assigned; ?>_<?php echo $list_photo['id']; ?>.jpg&amp;w=600&amp;zc=1&amp;q=90" alt="Order#<?php echo $history_id; ?>" /><?php } else { ?><div id="maps<?php echo $history_id; ?>" class="maps" data-gps="<?php echo $lat; ?>,<?php echo $long; ?>"></div><?php } ?>
									<div class="section_order_header_status" <?php if($list_history['status']=="complete") { echo'style="background: #0085B2;"'; } if($list_history['status']=="received") { echo'style="background: #7DB366;"'; } if($list_history['status']=="scheduled") { echo'style="background: #7d0052;"'; } if($list_history['status']=="pending") { echo'style="background: #b29300;"'; } if($list_history['status']=="archived") { echo'style="background: #333;"'; } if($list_history['status']=="canceled") { echo'style="background: #cd232b;"'; } if($list_history['status']=="expired") { echo'style="background: #666633;"'; } ?>>
										<?php echo ucfirst($list_history['status']); ?>
					...
					<div class="section_footer">
						<div class="section_navigate">
							<?php 
								if($c_id!="") { 
									$history_nav = "company=".$c_id;	
								} else if($a_id!="") {
									$history_nav = "agent=".$a_id;
								}
								$prev = $offset-$totpage;
								if ($prev >= 0) $nav.="<a href='".$PHP_SELF."?offset=".$prev."&amp;".$history_nav."'><strong>&lt;&lt;&nbsp;Prev</strong></a> | "; 
								if($records!="0") {
									$nav.="Page";
								}
								if($records/$totpage < 10){
									$crumbmax = $records/$totpage;
								} else {
									$crumbmax = 10; 
								}
								$bottom = 0;
								if(($offset/$totpage) > $crumbmax/2) $bottom = ($offset/$totpage) - ceil($crumbmax/2);
								$top = $bottom+$crumbmax;
								if(ceil($records/$totpage) < ($offset/$totpage) + $crumbmax/2)	$top = ceil($records/$totpage);
								for($p=$bottom; $p<$top; $p++){
									if($offset/$totpage==$p){
										$nav.=" <strong>".($p+1)."</strong>"; 
									} else {  
										$nav.=" <a href='".$PHP_SELF."?offset=".($p)*$totpage."&amp;".$history_nav."'>".($p+1)."</a>"; 
	  								} 
								} 
    							$next = $offset+$totpage;
								if ($next < ($records))	$nav.=" | <a href='".$PHP_SELF."?offset=".$next."&amp;".$history_nav."'><b>Next&nbsp;&gt;&gt;</b></a>";
								echo $nav; 
							?>&nbsp;
						</div>
						<div class="clear"></div>
